...
const STAFF0 = [
  ...
  { id: "s12", first: "KGH", last: "Manager", name: "Manager, KGH", role: "Manager", pin: "0000", password: "KGHmanager1!", phone: "", email: "manager@kgh.com", on: true, homes: ["KGH"] },
...
const SKILLS_BY_RES = {
  ...
  r2: [{ name: "Microwave Use", steps: ["Open door", "Place food", "Close door", "Set time", "Press start", "Wait for beep", "Remove food"], startDate: "2026-02-01", pillCount: 20, expiryDate: "2027-02-01", nextCyclePillCount: 0, nextCycleStartDate: "" }, { name: "Table Setting", steps: ["Get placemat", "Place plate", "Add fork/knife", "Add napkin", "Add cup"] }]
};
...
function IndividualSupportRecord() {
  ...
  const isCCSStaff = user && user.role === "Staff" && (user.homes || []).includes("CCS") && !(user.homes || []).some((h) => h !== "CCS");
  ...
  const CCS_TIMES = ["6:00 AM","6:30 AM","7:00 AM","7:30 AM","8:00 AM","8:30 AM","9:00 AM","9:30 AM","10:00 AM","10:30 AM","11:00 AM","11:30 AM","12:00 PM","12:30 PM","1:00 PM","1:30 PM","2:00 PM","2:30 PM","3:00 PM","3:30 PM","4:00 PM","4:30 PM","5:00 PM","5:30 PM","6:00 PM","6:30 PM","7:00 PM","7:30 PM","8:00 PM","Off"];
  ...
  const [newMed, setNewMed] = useState({ name: "", dose: "", route: "By Mouth", freq: "Daily", times: "08:00", instr: "", doctor: "", rx: "", pillColor: "", pillDesc: "", reason: "", tabsPerDose: 1, startDate: "", pillCount: "", expiryDate: "", specialParam: false, specialParamInstr: "", specialParamTimes: "", specialParamVitals: [], isShortTerm: false, shortTermStart: "", shortTermEnd: "", shortTermStartTime: "08:00", shortTermEndTime: "08:00" });
  ...
  const mkErVisit = (res) => {
    ...
    return { id: "er" + Date.now(), resId: res ? res.id : "", visitType: "ER Visit", date: today, time: h + ":" + m, location: "", reason: "", transport: "", notified: [], companionedBy: gn(user.id), diagnosis: "", treatment: "", newMeds: "", admitted: "No", discharged: "", followUp: "", followUpAppt: "", restrictions: "", staffNotes: "", by: user.id };
  };
...
"severity":"low/medium/high/critical"}` }] }) });
  ...
  const [memos, setMemos] = useState([
    { id: "mm1", memoNum: "MEMO-2026-001", name: "Fire Drill Training", text: "All staff must complete fire drill training by March 5th.", by: "s1", d: "2026-02-26", memoType: "staff", acks: [] },
    { id: "mm2", memoNum: "MEMO-2026-002", name: "Visitor Sign-In Policy", text: "New visitor sign-in policy effective immediately. See binder.", by: "s1", d: "2026-02-27", memoType: "house", acks: [] },
    ...
    { id: "memo-health-alert1", d: "2026-03-14", t: "6:45 AM", by: "sa1", text: "\u{1F6A8} HEALTH ALERT \u2014 David Reyes\n\nDavid Reyes was transported to the Emergency Room on 03/14/2026 at approximately 4:30 AM after a fall in the hallway near his bedroom.\n\nDIAGNOSIS: Sprained right ankle (Grade II)\n\nTREATMENT: X-ray confirmed no fracture. Ankle wrapped with compression bandage. Discharged at 6:15 AM. Prescribed Ibuprofen 400mg as needed for pain.\n\nCURRENT STATUS: David has returned to the home. He is resting in his room with his leg elevated. He is walking with limited weight-bearing on the right foot.\n\nFOLLOW-UP INSTRUCTIONS:\n\u2022 Monitor for increased swelling, bruising, or pain\n\u2022 Ice ankle 20 min every 2 hours for first 24 hours\n\u2022 Keep leg elevated when resting\n\u2022 Administer Ibuprofen 400mg with food as needed \u2014 document each administration in MAR\n\u2022 Do NOT allow community outings or extended walking today\n\u2022 Follow-up appointment with PCP scheduled for 03/17/2026\n\nAll staff on duty must read and acknowledge this memo. Contact manager immediately if condition worsens.", acks: [], urgent: true, homeIds: ["TRAIN"] }
  ]);
  const [memoForm, setMemoForm] = useState({ name: "", text: "", type: "other", homeIds: [], resId: "", urgent: true });
  ...
  const dueMeds = (f) => {
    const a = meds.filter((m) => m.resId === selRes && m.status !== "dcd" && !(m.status === "hold" && isHeldNow(m)) && !isOnHV(m.resId));
    if (f === "all") return isAM ? a : a.filter((m) => m.freq === "PRN" || m.times.some((t) => isInMedWindow(t) || marRecs.find((mr) => mr.med === m.name && mr.d === today && mr.sTime === t)));
  ...
  const adminMed = (m, sTime) => {
    const allSpMedsForRes = meds.filter((x) => x.resId === m.resId && (x.status === "active" || x.status === "shortterm") && x.specialParam && x.specialParamInstr);
    const pendingSp = allSpMedsForRes.filter((sm) => !docs.some((d) => d.resId === m.resId && d.d === today && d.type === "Special Parameter" && (d.text || "").includes(sm.name)));
    if (pendingSp.length > 0 && !m.specialParam) {
      alert("🛑 MEDICATIONS LOCKED — Complete special parameter check-in for this resident before administering any medications.");
      return;
    }
    if (!isAM && sTime && !isInMedWindow(sTime)) {
      alert("This medication is outside the 1-hour administration window. Only medications within 1 hour before or after the prescribed time can be given.");
      return;
    }
    if (m.specialParam && m.specialParamInstr) {
      setSpecialParamForm({ med: m, sTime: sTime || "", type: "mar", response: "" });
  ...
  const genMARHTML = (archMonth) => {
    ...
    activeMeds.forEach((m) => {
      const cols = days + 5;
      const instrParts = [];
      const tabsLabel = m.tabsPerDose && m.tabsPerDose > 1 ? m.tabsPerDose + " tabs" : "1 tab";
      instrParts.push(tabsLabel + " " + (m.route || "by mouth").toLowerCase() + " " + (m.freq || "daily").toLowerCase());
      const dv2 = parseFloat(m.dose); const du2 = m.dose ? m.dose.replace(/^[\d.]+\s*/, "") : ""; const tabs2 = m.tabsPerDose || 1; const doseTimes2 = (m.times || []).length || 1;
      if (dv2 && tabs2 >= 1) { const perD2 = Math.round(dv2 * tabs2 * 100) / 100; const daily2 = Math.round(perD2 * doseTimes2 * 100) / 100; instrParts.push((tabs2 > 1 ? tabs2 + " tabs × " + dv2 + du2 + " = " + perD2 + du2 + "/dose" : "") + (doseTimes2 > 1 ? (tabs2 > 1 ? " | " : "") + doseTimes2 + "x/day = " + daily2 + du2 + "/day" : (tabs2 > 1 ? " | 1x/day = " + daily2 + du2 + "/day" : ""))); }
      if (m.reason) instrParts.push("Reason: " + m.reason);
      if (m.sideEffects && m.sideEffects.length) instrParts.push("Side effects: " + (Array.isArray(m.sideEffects) ? m.sideEffects.slice(0,3).join(", ") : m.sideEffects));
      if (m.instr) instrParts.push("Instructions: " + m.instr);
      if (m.rx) instrParts.push("Rx#: " + m.rx);
      if (m.doctor) instrParts.push("Dr: " + m.doctor);
      const instrRow = "<tr style='background:#f8f9ff'><td colspan='" + cols + "' style='font-size:7px;color:#444;padding:2px 6px;border-top:none;font-style:italic'>" + instrParts.join(" &nbsp;|&nbsp; ") + "</td></tr>";
      (m.times && m.times.length ? m.times : [""]).forEach((t, ti) => {
        const mtc = t ? getMTC(t) : null;
        const rowBg = mtc ? mtc.bgL : "";
        h += "<tr" + (rowBg ? " style='background:" + rowBg + "'" : "") + "><td class='med'" + (mtc ? " style='border-left:3px solid " + mtc.hex + "'" : "") + ">" + (ti === 0 ? (m.pill && m.pill.img ? "<img src='" + m.pill.img + "' style='width:18px;height:18px;object-fit:contain;vertical-align:middle;margin-right:3px;border-radius:3px'/>" : "") + m.name : "") + "</td><td class='dose'>" + (ti === 0 ? m.dose : "") + "</td><td class='dose'>" + (ti === 0 ? (m.route || "").replace("By Mouth", "PO") : "") + "</td><td class='dose'>" + (ti === 0 ? m.freq : "") + "</td><td class='time'" + (mtc ? " style='background:" + mtc.bg + ";color:" + mtc.hex + ";font-weight:700'" : "") + ">";
        h += (t || "") + "</td>";
    ...
    const pcp = r.doctors && r.doctors.find((d) => d.specialty === "PCP") ? r.doctors.find((d) => d.specialty === "PCP").name : "Not listed";
    h += "<div style='margin-top:12px;border:2px solid #000;padding:8px 12px;background:#f8f8f8'><div style='display:flex;justify-content:space-between;align-items:flex-start'><div><div style='font-size:11px;font-weight:700'>RESIDENT INFORMATION</div><div style='font-size:10px;margin-top:3px'>Name: <b>" + fullName(r.name) + "</b></div><div style='font-size:10px;margin-top:2px'>Date of Birth: <b>" + r.dob + "</b> &nbsp;|&nbsp; Age: <b>" + getAge(r.dob) + "</b></div><div style='font-size:10px;margin-top:2px'>PCP: <b>" + pcp + "</b></div></div><div style='text-align:right'><div style='font-size:11px;font-weight:700'>ALLERGIES</div><div style='font-size:10px;color:#c00;font-weight:700;margin-top:3px'>" + (r.allergies.length ? r.allergies.map((a) => a.name + " (" + a.sev + ")").join(" &nbsp;|&nbsp; ") : "NKDA") + "</div><div style='font-size:9px;color:#888;margin-top:2px'>MAR Period: <b>" + moName + " " + yr + "</b></div></div></div></div>";
  ...
  const printMedList = () => {
    ...
    h += "<h3 style='font-size:11px;margin:8px 0 3px'>Active Medications</h3><table><tr><th>Photo</th><th>Medication</th><th>Generic</th><th>Dose</th><th>Route</th><th>Frequency</th><th>Times</th><th>Instructions</th><th>Prescribing Doctor</th><th>Rx#</th><th>Reason</th></tr>";
    rm2.filter((m) => m.status === "active").forEach((m) => h += "<tr><td style='padding:2px;text-align:center;vertical-align:middle'>" + (m.pill && m.pill.img ? "<img src='" + m.pill.img + "' style='width:36px;height:36px;object-fit:contain;border-radius:4px;background:#f8f8f8' />" : "<span style='font-size:9px;color:#999'>\u2014</span>") + "</td><td class='b'>" + m.name + "</td><td>" + (m.gen || "") + "</td><td>" + m.dose + "</td><td>" + m.route + "</td><td>" + m.freq + "</td><td>" + (m.times || []).join(", ") + "</td><td style='font-size:8px'>" + (m.instr || "") + "</td><td>" + (m.doctor || "") + "</td><td>" + (m.rx || "") + "</td><td style='font-size:8px'>" + (m.reason || "") + "</td></tr>");
    h += "</table>";
  ...
  if (isCCSStaff && !ccsTimeAllowed()) {
    ...
    const myScheduledToday = Object.entries(ccsSchedules).map(([resId, daySched]) => {
      const ds = (daySched || {})[dayKey];
      if (!ds || !ds.time || ds.time === "Off" || !(ds.staffIds || []).includes(user.id)) return null;
      const res = residents.find((r) => r.id === resId);
      return { res, time: ds.time };
    }).filter(Boolean);
    return /* @__PURE__ */ React.createElement("div", { style: { fontFamily: C.font, minHeight: "100vh", background: "linear-gradient(170deg,#e8f0fe 0%,#f5f8ff 30%," + C.bg + " 60%)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 20 } },
      /* @__PURE__ */ React.createElement("style", null, "@keyframes pulse{0%,100%{transform:scale(1)}50%{transform:scale(1.05)}}"),
      /* @__PURE__ */ React.createElement("div", { style: { width: 80, height: 80, borderRadius: 24, background: "linear-gradient(135deg,#5856d6,#7c5cbf)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px", boxShadow: "0 8px 32px rgba(88,86,214,.35)" } }, /* @__PURE__ */ React.createElement(Ic, { n: "lock", s: 40, c: "#fff" })),
      /* @__PURE__ */ React.createElement("h1", { style: { fontSize: 22, fontWeight: 700, margin: "0 0 6px", color: "#5856d6" } }, "Outside Scheduled Hours"),
      /* @__PURE__ */ React.createElement("p", { style: { fontSize: 14, color: C.tx2, margin: "0 0 20px", textAlign: "center", maxWidth: 320, lineHeight: 1.5 } }, "Hi " + (user.first || "") + ". You can only access Team ISR during your scheduled session \u2014 10 minutes before and up to 15 minutes after your start time."),
      myScheduledToday.length > 0
        ? /* @__PURE__ */ React.createElement(Card, { style: { width: "100%", maxWidth: 340, padding: 16, marginBottom: 12 } },
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tx3, marginBottom: 8 } }, "YOUR SCHEDULE TODAY \u2014 " + dayKey.toUpperCase()),
            myScheduledToday.map(({ res, time }, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: i < myScheduledToday.length - 1 ? "0.5px solid " + C.sepL : "none" } },
              /* @__PURE__ */ React.createElement("div", { style: { width: 36, height: 36, borderRadius: 10, background: "#5856d615", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 } }, "\uD83D\uDC64"),
              /* @__PURE__ */ React.createElement("div", null,
                /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 700 } }, res ? fullName(res.name) : "Participant"),
                /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#5856d6", fontWeight: 600 } }, "\uD83D\uDD52 " + time)
              )
            ))
          )
        : /* @__PURE__ */ React.createElement(Card, { style: { width: "100%", maxWidth: 340, padding: 14, marginBottom: 12 } },
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: C.tx3, textAlign: "center" } }, "You are not scheduled for today (" + dayKey + ").")
          ),
      /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: C.tx3, textAlign: "center", marginBottom: 16 } }, "Current time: " + now.toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })),
      /* @__PURE__ */ React.createElement("button", { onClick: () => { setUser(null); }, style: { padding: "10px 28px", borderRadius: 10, border: "none", background: C.sepL, color: C.tx3, fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: C.font } }, "Sign Out")
    );
  }
  ...
  const homeContent = tab !== "home" ? null : (() => {
    ...
    return /* @__PURE__ */ React.createElement("div", null, (() => {
      const medStatus = residents.filter((rx) => userHomeIds.includes(rx.home) && !rx.archived).map((res) => {
      ...
      const assignedRes = isAM ? residents : residents.filter((r2) => (user.homes || []).includes(r2.home) && !r2.archived);
      ...
      const myRes = isAM ? residents : residents.filter((rx) => (user.homes || []).includes(rx.home) && !rx.archived);
  ...
  const ResDetail = () => {
    ...
    if (!r) return null;
    const rm = resMeds;
    // Auto-complete short-term meds whose end time has passed
    rm.forEach((m) => { if (m.status === "shortterm" && m.shortTermEnd) { const end = new Date(m.shortTermEnd + "T" + (m.shortTermEndTime || "23:59")); if (new Date() > end) setMeds((p) => p.map((x) => x.id === m.id ? { ...x, status: "complete" } : x)); } });
    // Auto-advance pill cycle when next cycle date has arrived
    rm.forEach((m) => { if (m.nextCycleStartDate && m.nextCyclePillCount && m.nextCycleStartDate <= today) setMeds((p) => p.map((x) => x.id === m.id ? { ...x, startDate: x.nextCycleStartDate, pillCount: x.nextCyclePillCount, nextCycleStartDate: "", nextCyclePillCount: 0 } : x)); });
    ...
    return /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement(Hdr, { title: fn, left: /* @__PURE__ */ React.createElement("button", { onClick: () => {
    ...
    }, style: { background: "none", border: "none", cursor: "pointer", color: C.blue, fontSize: 14, fontFamily: C.font, display: "flex", alignItems: "center" } }, /* @__PURE__ */ React.createElement(Ic, { n: "back", s: 16, c: C.blue }), " ", resSubTab !== "dash" ? fn.split(" ")[0] : "Back"), right: cm && (resSubTab === "mar" || resSubTab === "medlist") ? /* @__PURE__ */ React.createElement(Btn, { sm: true, onClick: () => resSubTab === "medlist" ? printMedList() : printMAR() }, /* @__PURE__ */ React.createElement(Ic, { n: "print", s: 12, c: "#fff" })) : null }), /* @__PURE__ */ React.createElement(AllergyBanner, { r }), resSubTab === "dash" && /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { padding: "14px 14px 6px", display: "flex", alignItems: "center", gap: 12, background: "linear-gradient(180deg, rgba(0,122,255,.04), transparent)", borderRadius: "0 0 16px 16px" } }, /* @__PURE__ */ React.createElement("div", { style: { position: "relative", flexShrink: 0 } }, r.photo ? /* @__PURE__ */ React.createElement("img", { src: r.photo, style: { width: 64, height: 64, borderRadius: "50%", objectFit: "cover", border: "3px solid " + C.blue + "40" } }) : /* @__PURE__ */ React.createElement(Av, { name: r.name, s: 64, cl: C.blue }), isAM && /* @__PURE__ */ React.createElement("label", { style: { position: "absolute", bottom: 0, right: 0, width: 22, height: 22, borderRadius: 11, background: C.blue, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: "0 1px 4px rgba(0,0,0,.25)" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: "#fff" } }, "\uD83D\uDCF7"), /* @__PURE__ */ React.createElement("input", { type: "file", accept: "image/*", style: { display: "none" }, onChange: (e) => { const file = e.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = (ev) => setResidents((p) => p.map((x) => x.id === selRes ? { ...x, photo: ev.target.result } : x)); reader.readAsDataURL(file); } }))), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 20, fontWeight: 700 } }, fn), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: C.tx3, marginTop: 2, display: "flex", flexDirection: "column", gap: 1 } }, /* @__PURE__ */ React.createElement("span", null, "\uD83C\uDF82 DOB: ", r.dob, " | Age: ", getAge(r.dob)), /* @__PURE__ */ React.createElement("span", null, "\uD83C\uDFE5 UCI: ", r.uci || "\u2014"), r.admDate && /* @__PURE__ */ React.createElement("span", null, "\uD83D\uDCC5 Admitted: ", r.admDate)), (() => {
      ...
      ...isS ? [
        ...
        { l: "Drs / Specialists", sub: (() => { const docs = (r.doctors || []); const specs = docs.filter((d) => d.specialty && d.specialty !== "PCP"); return docs.length + " providers, " + specs.length + " specialist" + (specs.length !== 1 ? "s" : ""); })(), e: "\u{1FA7A}", c: "#5856d6", v: "scheddr" },
        { l: "Activity Feed", sub: "Staff documentation", e: "\u{1F4DD}", c: C.ind, v: "activity" },
        { l: "Documents", sub: resFiles.filter((f) => f.resId === selRes).length + " files", e: "\u{1F4C1}", c: C.org, v: "docs" },
        { l: "Assignments", sub: (() => { const todayDA = (dailyAssigns[today] || {}); const cnt = Object.values(todayDA).reduce((s, sh) => s + Object.values(sh).reduce((s2, sids) => s2 + sids.length, 0), 0); return cnt + " assigned today"; })(), e: "\uD83D\uDCC5", c: C.pur, v: "assign" }
      ] : [
        { l: "MAR", sub: (() => {
          let dueCnt = 0;
          let givenCnt = 0;
        ...
        { l: "BIP", sub: (() => {
          const aBIP = bipDocs.find((b) => b.resId === selRes && b.active);
          return aBIP ? aBIP.filename.slice(0, 18) : "No active BIP";
        })(), e: "\u{1F9E0}", c: "#c0392b", v: "bip" },
        { l: "Documents", sub: resFiles.filter((f) => f.resId === selRes).length + " files", e: "\u{1F4C1}", c: C.org, v: "docs" },
        { l: "Activity", sub: todayDocs.length + " entries today", e: "\u{1F4DD}", c: C.ind, v: "activity" },
        ...r.home === "CCS" && (cm || isAM || isS) ? [{ l: "CCS Schedule", sub: (() => { const ds = ccsSchedules[selRes] || {}; const active = CCS_DAYS.filter((d) => ds[d] && ds[d].time && ds[d].time !== "Off"); return active.length + " days scheduled"; })(), e: "\uD83D\uDCC5", c: "#5856d6", v: "ccssched" }] : [],
        ...(cm || isAM || isS) ? [{ l: "Assignments", sub: (() => { const todayDA = (dailyAssigns[today] || {}); const cnt = Object.values(todayDA).reduce((s, sh) => s + Object.values(sh).reduce((s2, sids) => s2 + sids.length, 0), 0); return cnt + " assigned today"; })(), e: "\uD83D\uDCC5", c: C.pur, v: "assign" }] : []
      ]
      ...
      const specialParamMeds = rm.filter((mx) => (mx.status === "active" || mx.status === "shortterm") && mx.specialParam && mx.specialParamInstr);
      const spDoneToday = (med) => docs.some((d) => d.resId === selRes && d.d === today && d.type === "Special Parameter" && (d.text || "").includes(med.name));
      const pendingSpMeds = specialParamMeds.filter((mx) => !spDoneToday(mx));
      return /* @__PURE__ */ React.createElement(Card, { style: { marginBottom: 8 } }, /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 12px", background: C.blue + "06", borderBottom: "0.5px solid " + C.sepL } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 700, color: C.blue, textAlign: "center" } }, "MAR \u2014 ", moLabel)), pendingSpMeds.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { padding: "12px 14px", background: C.red + "08", borderBottom: "0.5px solid " + C.red + "25" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 8 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 22 } }, "\uD83D\uDED1"), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 800, color: C.red } }, "Medications Locked"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3, marginTop: 1 } }, "Complete check-in before dispensing any medications today"))), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 6 } }, pendingSpMeds.map((mx) => /* @__PURE__ */ React.createElement("div", { key: mx.id, style: { display: "flex", alignItems: "center", gap: 10, padding: "8px 10px", background: C.card, borderRadius: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 700, color: C.tx } }, mx.name, " \u2014 ", (mx.specialParamVitals || []).map((v) => v === "bp" ? "\u2764\uFE0F BP" : v === "hr" ? "\uD83D\uDC93 HR" : "\uD83E\uDE78 BS").join(", ") || "Task Required"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3, marginTop: 2 } }, mx.specialParamInstr)), /* @__PURE__ */ React.createElement("button", { onClick: () => { setSpecialParamForm({ med: mx, sTime: "", type: "checkin", response: "", bpSys: "", bpDia: "", hr: "", bs: "" }); setModal("specialParam"); }, style: { padding: "7px 14px", borderRadius: 9, border: "none", background: C.red, color: "#fff", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: C.font } }, "Check In")))), doctors.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { padding: "6px 12px", borderBottom: "0.5px solid " + C.sepL } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 600, color: C.tx3, marginBottom: 2 } }, "PHYSICIANS"), doctors.map((d, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { fontSize: 12, color: C.tx2, padding: "1px 0" } }, d)))));
    })(), /* @__PURE__ */ React.createElement(Card, null, isOnHV(selRes) && /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 12px", background: C.pur + "08" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, fontWeight: 700, color: C.pur } }, "HOME VISIT \u2014 Meds paused until ", (residents.find((x) => x.id === selRes)?.hvData?.toDate || "?") + " " + (residents.find((x) => x.id === selRes)?.hvData?.toTime || ""))), rm.filter((m) => m.status === "active" && m.freq !== "PRN" && !isOnHV(m.resId)).length === 0 && /* @__PURE__ */ React.createElement("div", { style: { padding: 16, textAlign: "center", color: C.tx3, fontSize: 13 } }, isOnHV(selRes) ? "Meds paused (home visit)" : "No active meds"), rm.filter((m) => { if (m.status === "shortterm") { if (!m.shortTermStart || !m.shortTermEnd) return false; const now = new Date(); const start = new Date(m.shortTermStart + "T" + (m.shortTermStartTime || "08:00")); const end = new Date(m.shortTermEnd + "T" + (m.shortTermEndTime || "08:00")); return now >= start && now <= end; } return m.status === "active" && m.freq !== "PRN" && !isOnHV(m.resId); }).map((m, i, arr) => {
      ...
      return /* @__PURE__ */ React.createElement("div", { key: m.id, style: { padding: "14px 12px", borderBottom: i < arr.length - 1 ? "2.5px solid " + C.sepL : "none", marginBottom: i < arr.length - 1 ? 0 : 0, background: tc ? tc.bgL : "transparent" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8 } }, cm ? /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 3 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 4, padding: "3px 8px", borderRadius: 6, background: C.grn + "12", border: "1.5px solid " + C.grn + "40" } }, /* @__PURE__ */ React.createElement("div", { style: { width: 10, height: 10, borderRadius: 5, background: C.grn } }), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 9, fontWeight: 700, color: C.grn } }, "Active")), /* @__PURE__ */ React.createElement("div", { onClick: () => {
        ...
        setViewMed(m.id);
        setModal("medD");
      } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap" } },
              /* @__PURE__ */ React.createElement("span", { style: { fontSize: 17, fontWeight: 800, letterSpacing: -0.3 } }, m.name, " ", /* @__PURE__ */ React.createElement("span", { style: { fontSize: 15, fontWeight: 700, color: C.tx2 } }, m.dose)),
              /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, fontWeight: 700, color: C.blue, background: C.blue + "10", padding: "1px 5px", borderRadius: 4 } }, "x", m.tabsPerDose || 1),
              m.specialParam && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 9, fontWeight: 700, color: "#fff", background: C.red, padding: "2px 6px", borderRadius: 4, display: "flex", alignItems: "center", gap: 3 } }, "🛑 TASK REQUIRED")),
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: C.tx3, marginTop: 2 } }, m.route, " | ", m.freq), (() => { if (!m.startDate || !m.pillCount) return null; const start = new Date(m.startDate); const dosesPerDay = (m.times || []).length || 1; const tabsPerDay = (m.tabsPerDose || 1) * dosesPerDay; const daysSince = Math.floor((new Date() - start) / 86400000); const pillsUsed = daysSince * tabsPerDay; const pillsLeft = Math.max(0, (m.pillCount || 0) - pillsUsed); const daysLeft = tabsPerDay > 0 ? Math.floor(pillsLeft / tabsPerDay) : 0; const nextStart = new Date(start); nextStart.setDate(start.getDate() + Math.ceil((m.pillCount || 0) / tabsPerDay)); const isShortTerm = m.isShortTerm; return /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8, padding: "8px 10px", background: pillsLeft <= 7 ? C.red + "08" : C.grn + "06", borderRadius: 10, border: "1.5px solid " + (pillsLeft <= 7 ? C.red : C.grn) + "30", display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" } },
          /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6 } },
            ...
            /* @__PURE__ */ React.createElement("div", null,
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 600, color: C.tx3, lineHeight: 1 } }, "PILLS REMAINING"),
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 22, fontWeight: 800, color: pillsLeft <= 7 ? C.red : C.grn, lineHeight: 1.1 } }, pillsLeft, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, fontWeight: 600, color: C.tx3, marginLeft: 3 } }, "pills")))),
          /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 12, flexWrap: "wrap", flex: 1 } },
            /* @__PURE__ */ React.createElement("div", null,
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 600, color: C.tx3 } }, "START DATE"),
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 700, color: C.tx } }, m.startDate)),
            /* @__PURE__ */ React.createElement("div", null,
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 600, color: C.tx3 } }, "NEXT FILL"),
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 700, color: C.pur } }, nextStart.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }))),
            pillsLeft <= 7 && /* @__PURE__ */ React.createElement("div", { style: { padding: "4px 10px", background: C.red, borderRadius: 8, alignSelf: "center" } },
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: "#fff" } }, "⚠️ Low — reorder now")),
            (() => { if (!m.expiryDate) return null; const exp = new Date(m.expiryDate); const daysLeft = Math.ceil((exp - new Date()) / 86400000); const isExpired = daysLeft <= 0; return /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, fontWeight: 600, color: C.tx3 } }, "EXPIRES"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 700, color: isExpired ? C.red : daysLeft <= 30 ? C.org : C.tx } }, isExpired ? "EXPIRED" : exp.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }), !isExpired && daysLeft <= 90 && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: daysLeft <= 30 ? C.red : C.org, marginLeft: 4 } }, "(" + daysLeft + "d)"))); })())); })(), isAM && /* @__PURE__ */ React.createElement("button", { onClick: (e) => { e.stopPropagation(); setMedUpdateForm({ ...m, origDose: m.dose, origFreq: m.freq, origTimes: (m.times || []).join(", "), origTabsPerDose: m.tabsPerDose || 1 }); setMedUpdateTab("info"); setModal("medUpdate"); }, style: { display: "flex", alignItems: "center", gap: 4, padding: "4px 10px", borderRadius: 7, border: "1.5px solid " + C.blue + "50", background: C.blue + "10", color: C.blue, fontSize: 10, fontWeight: 700, cursor: "pointer", fontFamily: C.font, marginTop: 4 } }, "✏️ Edit"),
          /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 4, marginTop: 4 } }, m.times.map((st, ti) => {
        ...
        }, style: { fontSize: 10, fontWeight: 700, color: C.grn, cursor: "pointer" } }, pureInit(gn(g.by)), " x", g.qty || 1) : isAM || isInMedWindow(st) ? /* @__PURE__ */ React.createElement("button", { onClick: (e) => {
          e.stopPropagation();
          adminMed(m, st);
        }, style: { fontSize: 9, fontWeight: 700, color: "#fff", background: C.grn, border: "none", borderRadius: 4, padding: "1px 6px", cursor: "pointer", fontFamily: C.font } }, "Give") : /* @__PURE__ */ React.createElement("span", { style: { fontSize: 8, color: C.tx3 } }, "Not due"));
      })))),
      /* @__PURE__ */ React.createElement("div", { style: { borderTop: "0.5px solid " + C.sepL, padding: "8px 12px", background: C.alt + "80" } },
        /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "flex-start" } },
          m.pill && m.pill.img && /* @__PURE__ */ React.createElement("img", { src: m.pill.img, style: { width: 44, height: 44, borderRadius: 8, objectFit: "contain", background: "#fff", border: "1px solid " + C.sepL, flexShrink: 0 } }),
          !(m.pill && m.pill.img) && /* @__PURE__ */ React.createElement("div", { style: { flexShrink: 0 } }, /* @__PURE__ */ React.createElement(PillVisual, { pill: m.pill, size: 44 })),
          /* @__PURE__ */ React.createElement("div", { style: { flex: 1, minWidth: 0 } },
            /* @__PURE__ */ React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: "3px 10px", fontSize: 11 } },
              /* @__PURE__ */ React.createElement("div", { style: { gridColumn: "1 / -1", background: "linear-gradient(135deg, #007aff, #0055cc)", borderRadius: 12, padding: "12px 14px", marginBottom: 8 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, fontWeight: 700, color: "rgba(255,255,255,.65)", textTransform: "uppercase", letterSpacing: 1, marginBottom: 5 } }, "✉ Instructions"), /* @__PURE__ */ React.createElement("div", { style: { fontWeight: 800, color: "#fff", fontSize: 15, lineHeight: 1.4, marginBottom: 6 } }, "Take " + (m.tabsPerDose || 1) + " " + (m.tabsPerDose > 1 ? "tabs" : "tab") + " " + (m.route || "by mouth").toLowerCase() + " " + (m.freq || "daily").toLowerCase()), /* @__PURE__ */ React.createElement("div", { style: { borderTop: "1px solid rgba(255,255,255,.2)", paddingTop: 6 } }, (() => {
                const dv = parseFloat(m.dose); const du = m.dose ? m.dose.replace(/^[\d.]+\s*/, "") : ""; const tabs = m.tabsPerDose || 1; const doseTimes = (m.times || []).length || 1;
                if (!dv) return null;
                const perD = Math.round(dv * tabs * 100) / 100; const daily = Math.round(perD * doseTimes * 100) / 100;
                return /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "rgba(255,255,255,.9)", fontWeight: 600, lineHeight: 1.6 } },
                  tabs > 1 ? tabs + " tabs × " + dv + du + " = " + perD + du + "/dose" : perD + du + "/dose",
                  doseTimes > 1 ? /* @__PURE__ */ React.createElement("span", { style: { color: C.tx3, fontWeight: 400 } }, "  |  " + doseTimes + "x/day = ") : /* @__PURE__ */ React.createElement("span", { style: { color: C.tx3, fontWeight: 400 } }, "  |  1x/day = "),
                  /* @__PURE__ */ React.createElement("b", null, daily + du + "/day")
                );
              }))),
              m.rx && /* @__PURE__ */ React.createElement("div", { style: { borderBottom: "0.5px solid #e0e0e0", paddingBottom: 5, marginBottom: 5 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, fontWeight: 700, color: "#888", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 2 } }, "Rx #"), /* @__PURE__ */ React.createElement("div", { style: { fontWeight: 800, color: "#000", fontSize: 13 } }, m.rx)),
              m.doctor && /* @__PURE__ */ React.createElement("div", { style: { borderBottom: "0.5px solid #e0e0e0", paddingBottom: 5, marginBottom: 5 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, fontWeight: 700, color: "#888", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 2 } }, "Prescribing Doctor"), /* @__PURE__ */ React.createElement("div", { style: { fontWeight: 800, color: "#000", fontSize: 13 } }, m.doctor)),
              m.reason && /* @__PURE__ */ React.createElement("div", { style: { gridColumn: "1 / -1", borderBottom: "0.5px solid #e0e0e0", paddingBottom: 5, marginBottom: 5 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, fontWeight: 700, color: "#888", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 2 } }, "Reason for Use"), /* @__PURE__ */ React.createElement("div", { style: { fontWeight: 800, color: "#000", fontSize: 13 } }, m.reason)),
              /* @__PURE__ */ React.createElement("div", { style: { gridColumn: "1 / -1", borderBottom: "0.5px solid #e0e0e0", paddingBottom: 5, marginBottom: 5 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, fontWeight: 700, color: "#888", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 2 } }, "Special Instructions"), /* @__PURE__ */ React.createElement("div", { style: { fontWeight: 800, color: m.instr ? "#000" : "#aaa", fontSize: 13, fontStyle: m.instr ? "normal" : "italic" } }, m.instr || "N/A")),
              m.sides && m.sides.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { gridColumn: "1 / -1" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, fontWeight: 700, color: "#888", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 5 } }, "Side Effects"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 4 } }, m.sides.slice(0, 4).map((s, si) => /* @__PURE__ */ React.createElement("span", { key: si, style: { fontSize: 11, padding: "3px 8px", borderRadius: 6, background: "#f0f0f0", color: "#000", fontWeight: 700, border: "1px solid #ccc" } }, s))))
            )
          )
        )
      ));
      ...
      return /* @__PURE__ */ React.createElement(Card, { key: m.id, style: { marginBottom: 10, overflow: "hidden", borderTop: "3px solid " + C.blue + "40" } },
      /* @__PURE__ */ React.createElement("div", { style: { padding: "14px 12px", display: "flex", alignItems: "center", gap: 8 } },
        /* @__PURE__ */ React.createElement(PillVisual, { pill: m.pill, size: 36 }),
        /* @__PURE__ */ React.createElement("div", { style: { flex: 1, minWidth: 0, cursor: "pointer" }, onClick: () => { setViewMed(m.id); setModal("medD"); } },
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 17, fontWeight: 800, letterSpacing: -0.3 } }, m.name, " ", /* @__PURE__ */ React.createElement("span", { style: { fontSize: 15, fontWeight: 700, color: C.tx2 } }, m.dose), " ", /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, fontWeight: 700, color: C.blue, background: C.blue + "10", padding: "1px 5px", borderRadius: 4 } }, "x", m.tabsPerDose || 1)),
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: C.tx3, marginTop: 2 } }, m.route, " | PRN")),
        /* @__PURE__ */ React.createElement(Btn, { sm: true, color: C.blue, onClick: () => adminPrn(m) }, "Give PRN")),
      /* @__PURE__ */ React.createElement("div", { style: { borderTop: "0.5px solid " + C.sepL, padding: "8px 12px", background: C.alt + "80" } },
        /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "flex-start" } },
          m.pill && m.pill.img && /* @__PURE__ */ React.createElement("img", { src: m.pill.img, style: { width: 44, height: 44, borderRadius: 8, objectFit: "contain", background: "#fff", border: "1px solid " + C.sepL, flexShrink: 0 } }),
          !(m.pill && m.pill.img) && /* @__PURE__ */ React.createElement("div", { style: { flexShrink: 0 } }, /* @__PURE__ */ React.createElement(PillVisual, { pill: m.pill, size: 44 })),
          /* @__PURE__ */ React.createElement("div", { style: { flex: 1, minWidth: 0 } },
            /* @__PURE__ */ React.createElement("div", { style: { gridColumn: "1 / -1", background: "#007aff", borderRadius: 10, padding: "10px 14px", marginBottom: 6 } },
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, fontWeight: 700, color: "rgba(255,255,255,.65)", textTransform: "uppercase", letterSpacing: 1, marginBottom: 5 } }, "✉ Instructions"),
              /* @__PURE__ */ React.createElement("div", { style: { fontWeight: 800, color: "#fff", fontSize: 15, lineHeight: 1.4, marginBottom: 6 } }, "Take " + (m.tabsPerDose || 1) + " " + (m.tabsPerDose > 1 ? "tabs" : "tab") + " " + (m.route || "by mouth").toLowerCase() + " as needed"),
              /* @__PURE__ */ React.createElement("div", { style: { borderTop: "1px solid rgba(255,255,255,.2)", paddingTop: 6 } }, (() => { const dv = parseFloat(m.dose); const du = m.dose ? m.dose.replace(/^[d.]+s*/, "") : ""; const tabs = m.tabsPerDose || 1; if (!dv) return null; const perD = Math.round(dv * tabs * 100) / 100; return /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "rgba(255,255,255,.9)", fontWeight: 600 } }, tabs > 1 ? tabs + " tabs × " + dv + du + " = " + perD + du + "/dose" : perD + du + "/dose", "  |  PRN"); })())),
            /* @__PURE__ */ React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4px 10px", fontSize: 11 } },
              m.rx && /* @__PURE__ */ React.createElement("div", { style: { borderBottom: "0.5px solid #e0e0e0", paddingBottom: 5, marginBottom: 5 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, fontWeight: 700, color: "#888", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 2 } }, "Rx #"), /* @__PURE__ */ React.createElement("div", { style: { fontWeight: 800, color: "#000", fontSize: 13 } }, m.rx)),
              m.doctor && /* @__PURE__ */ React.createElement("div", { style: { borderBottom: "0.5px solid #e0e0e0", paddingBottom: 5, marginBottom: 5 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, fontWeight: 700, color: "#888", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 2 } }, "Prescribing Doctor"), /* @__PURE__ */ React.createElement("div", { style: { fontWeight: 800, color: "#000", fontSize: 13 } }, m.doctor)),
              m.reason && /* @__PURE__ */ React.createElement("div", { style: { gridColumn: "1 / -1", borderBottom: "0.5px solid #e0e0e0", paddingBottom: 5, marginBottom: 5 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, fontWeight: 700, color: "#888", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 2 } }, "Reason for Use"), /* @__PURE__ */ React.createElement("div", { style: { fontWeight: 800, color: "#000", fontSize: 13 } }, m.reason)),
              /* @__PURE__ */ React.createElement("div", { style: { gridColumn: "1 / -1", borderBottom: "0.5px solid #e0e0e0", paddingBottom: 5, marginBottom: 5 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, fontWeight: 700, color: "#888", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 2 } }, "Special Instructions"), /* @__PURE__ */ React.createElement("div", { style: { fontWeight: 800, color: m.instr ? "#000" : "#aaa", fontSize: 13, fontStyle: m.instr ? "normal" : "italic" } }, m.instr || "N/A")),
              m.sides && m.sides.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { gridColumn: "1 / -1", marginBottom: 5 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, fontWeight: 700, color: "#888", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 5 } }, "Side Effects"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 4 } }, m.sides.slice(0, 4).map((s, si) => /* @__PURE__ */ React.createElement("span", { key: si, style: { fontSize: 11, padding: "3px 8px", borderRadius: 6, background: "#f0f0f0", color: "#000", fontWeight: 700, border: "1px solid #ccc" } }, s))))))),
      (() => { if (!m.startDate || !m.pillCount) return null; const tabsPerDay = m.tabsPerDose || 1; const daysSince = Math.floor((new Date() - new Date(m.startDate)) / 86400000); const pillsLeft = Math.max(0, (m.pillCount || 0) - daysSince * tabsPerDay); const nextStart = new Date(m.startDate); nextStart.setDate(new Date(m.startDate).getDate() + Math.ceil((m.pillCount || 0) / tabsPerDay)); const expiry = m.expiryDate ? new Date(m.expiryDate) : null; const daysToExpiry = expiry ? Math.ceil((expiry - new Date()) / 86400000) : null; const isExpired = daysToExpiry !== null && daysToExpiry <= 0; return /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 12px", background: (pillsLeft <= 7 || isExpired) ? C.red + "06" : C.grn + "04", borderTop: "0.5px solid " + C.sepL } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 18 } }, "💊"), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, fontWeight: 600, color: C.tx3 } }, "PILLS REMAINING"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 22, fontWeight: 800, color: pillsLeft <= 7 ? C.red : C.grn, lineHeight: 1.1 } }, pillsLeft, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, fontWeight: 500, color: C.tx3, marginLeft: 3 } }, "pills")))), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 12, flexWrap: "wrap", flex: 1 } }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, fontWeight: 600, color: C.tx3 } }, "START DATE"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 700, color: C.tx } }, m.startDate)), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, fontWeight: 600, color: C.tx3 } }, "NEXT FILL"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 700, color: C.pur } }, nextStart.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }))), expiry && /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, fontWeight: 600, color: C.tx3 } }, "EXPIRES"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 700, color: isExpired ? C.red : daysToExpiry <= 30 ? C.org : C.tx } }, isExpired ? "EXPIRED" : expiry.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }), !isExpired && daysToExpiry <= 90 && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: daysToExpiry <= 30 ? C.red : C.org, marginLeft: 4 } }, "(" + daysToExpiry + "d)"))), (pillsLeft <= 7 || isExpired) && /* @__PURE__ */ React.createElement("div", { style: { padding: "4px 10px", background: C.red, borderRadius: 8, alignSelf: "center" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: "#fff" } }, isExpired ? "⚠️ Expired" : "⚠️ Low — reorder")))), isAM && /* @__PURE__ */ React.createElement("button", { onClick: (e) => { e.stopPropagation(); setMedUpdateForm({ ...m, origDose: m.dose, origFreq: m.freq, origTimes: (m.times || []).join(", "), origTabsPerDose: m.tabsPerDose || 1 }); setMedUpdateTab("pills"); setModal("medUpdate"); }, style: { display: "flex", alignItems: "center", gap: 4, padding: "5px 10px", borderRadius: 7, border: "1.5px solid " + C.blue + "50", background: C.blue + "10", color: C.blue, fontSize: 10, fontWeight: 700, cursor: "pointer", fontFamily: C.font, marginTop: 6, width: "100%" } }, "✏️ Update Pill Count / Expiry")); })(), todayGiven.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { borderTop: "0.5px solid " + C.sepL, padding: "6px 12px", background: C.blue + "04" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 600, color: C.blue, marginBottom: 2 } }, "Today (", todayGiven.length, "x)"), todayGiven.map((rec, i) => /* @__PURE__ */ React.createElement("div", { key: rec.id, style: { display: "flex", justifyContent: "space-between", fontSize: 11, padding: "2px 0" } }, /* @__PURE__ */ React.createElement("span", null, rec.t, " \u2014 ", rec.reason), /* @__PURE__ */ React.createElement("span", { style: { fontWeight: 600, color: rec.followUp ? C.grn : C.org } }, rec.followUp ? pureInit(gn(rec.by)) + " \u2713" : "F/U pending"))))));
    ...
    }))), resSubTab === "medlist" && /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px" } }, /* @__PURE__ */ React.createElement(Card, { style: { marginBottom: 8, border: "1.5px solid " + C.sepL } }, /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 12px", background: C.blue + "06", borderBottom: "1.5px solid " + C.sepL, display: "flex", alignItems: "center" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13, fontWeight: 600, color: C.blue, flex: 1 } }, "Current Medications"), /* @__PURE__ */ React.createElement("button", { onClick: () => printMedList(), style: { background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 3, color: C.blue, fontSize: 11, fontWeight: 600, fontFamily: C.font } }, /* @__PURE__ */ React.createElement(Ic, { n: "print", s: 14, c: C.blue }), " Print")), rm.filter((m) => m.status === "active" || m.status === "shortterm").length === 0 ? /* @__PURE__ */ React.createElement("div", { style: { padding: 16, textAlign: "center", color: C.tx3 } }, "No active medications") : rm.filter((m) => m.status === "active" || m.status === "shortterm").map((m, i, arr) => /* @__PURE__ */ React.createElement("div", { key: m.id, style: { padding: "14px 12px", borderBottom: i < arr.length - 1 ? "2px solid " + C.sepL : "none" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 10 } }, /* @__PURE__ */ React.createElement(PillVisual, { pill: m.pill, size: 34 }), /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 17, fontWeight: 800, letterSpacing: -0.3 } }, m.name, " ", /* @__PURE__ */ React.createElement("span", { style: { fontSize: 15, fontWeight: 700, color: C.tx2 } }, m.dose)), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3 } }, m.route, " | ", m.freq, m.times.length ? /* @__PURE__ */ React.createElement(React.Fragment, null, " | ", m.times.map((t, ti) => {
      const tc2 = getMTC(t);
      return /* @__PURE__ */ React.createElement("span", { key: ti, style: { display: "inline-block", padding: "1px 5px", borderRadius: 4, background: tc2 ? tc2.bg : "#eee", color: tc2 ? tc2.hex : C.tx3, fontWeight: 600, fontSize: 10, marginLeft: ti > 0 ? 2 : 0 } }, t);
    })) : ""), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx2, marginTop: 2 } }, m.instr)), /* @__PURE__ */ React.createElement(Pill, { text: m.freq === "PRN" ? "PRN" : "Sched", cl: m.freq === "PRN" ? C.blue : C.grn })), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3, marginTop: 4, paddingLeft: 40 } }, m.doctor, " | Rx: ", m.rx, m.reason ? " | " + m.reason : "", m.isShortTerm ? /* @__PURE__ */ React.createElement("span", { style: { marginLeft: 4, fontSize: 9, padding: "1px 5px", borderRadius: 4, background: C.org + "15", color: C.org, fontWeight: 700 } }, "⏰ SHORT-TERM") : null), (() => { if (!m.startDate || !m.pillCount) return null; const dosesPerDay = (m.times || []).length || 1; const tabsPerDay = (m.tabsPerDose || 1) * dosesPerDay; const daysSince = Math.floor((new Date() - new Date(m.startDate)) / 86400000); const pillsLeft = Math.max(0, (m.pillCount || 0) - daysSince * tabsPerDay); const nextDate = new Date(m.startDate); nextDate.setDate(nextDate.getDate() + Math.ceil((m.pillCount || 0) / tabsPerDay)); return /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8, marginLeft: 40, padding: "8px 10px", background: pillsLeft <= 7 ? C.red + "08" : C.grn + "06", borderRadius: 10, border: "1.5px solid " + (pillsLeft <= 7 ? C.red : C.grn) + "30", display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" } },
          /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6 } },
            /* @__PURE__ */ React.createElement("span", { style: { fontSize: 20 } }, "💊"),
            /* @__PURE__ */ React.createElement("div", null,
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 600, color: C.tx3, lineHeight: 1 } }, "PILLS REMAINING"),
          ...
          /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 12, flexWrap: "wrap", flex: 1 } },
            ...
            /* @__PURE__ */ React.createElement("div", null,
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 600, color: C.tx3 } }, "NEXT FILL"),
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 700, color: C.pur } }, nextDate.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }))),
            pillsLeft <= 7 && /* @__PURE__ */ React.createElement("div", { style: { padding: "4px 10px", background: C.red, borderRadius: 8, alignSelf: "center" } },
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: "#fff" } }, "⚠️ Low — reorder now")),
              (() => { if (!m.expiryDate) return null; const exp = new Date(m.expiryDate); const daysLeft = Math.ceil((exp - new Date()) / 86400000); const isExpired = daysLeft <= 0; return /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, fontWeight: 600, color: C.tx3 } }, "EXPIRES"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 700, color: isExpired ? C.red : daysLeft <= 30 ? C.org : C.tx } }, isExpired ? "EXPIRED" : exp.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }), !isExpired && daysLeft <= 90 && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: daysLeft <= 30 ? C.red : C.org, marginLeft: 4 } }, "(" + daysLeft + "d)"))); })())); })(),
          (() => { const dv = parseFloat(m.dose); const du = m.dose ? m.dose.replace(/^[\d.]+\s*/, "") : ""; const tabs = m.tabsPerDose || 1; const doseTimes = (m.times || []).length || 1; if (!dv) return null; const perD = Math.round(dv * tabs * 100) / 100; const daily = Math.round(perD * doseTimes * 100) / 100; return /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, marginTop: 3, paddingLeft: 40, padding: "3px 6px 3px 40px", background: "#5856d606", borderRadius: 6, color: "#5856d6", fontWeight: 600 } }, tabs > 1 ? tabs + " tabs × " + dv + du + " = " + perD + du + "/dose" : dv + du + "/dose", /* @__PURE__ */ React.createElement("span", { style: { color: C.tx3, fontWeight: 400 } }, "  |  " + doseTimes + "x/day = "), /* @__PURE__ */ React.createElement("b", null, daily + du + "/day")); })(), isAM && /* @__PURE__ */ React.createElement("label", { style: { display: "flex", alignItems: "center", gap: 4, padding: "5px 10px", borderRadius: 8, background: C.teal + "10", cursor: "pointer", marginTop: 4, width: "fit-content", border: "1px solid " + C.teal + "30" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13 } }, "📷"), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: C.teal, fontWeight: 600 } }, m.pill && m.pill.img ? "Change Photo" : "Add Pill Photo"), /* @__PURE__ */ React.createElement("input", { type: "file", accept: "image/*", capture: "environment", style: { display: "none" }, onChange: (e2) => {
    ...
    } }))))), rm.filter((m) => m.status === "hold").length > 0 && /* @__PURE__ */ React.createElement(Card, { style: { marginBottom: 8 } }, /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 12px", background: C.yel + "08", borderBottom: "0.5px solid " + C.sepL } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13, fontWeight: 600, color: C.yel } }, "On Hold")), rm.filter((m) => m.status === "hold").map((m, i, arr) => /* @__PURE__ */ React.createElement("div", { key: m.id, style: { padding: "8px 12px", borderBottom: i < arr.length - 1 ? "0.5px solid " + C.sepL : "none", opacity: 0.7 } }, /* @__PURE__ */ React.createElement("span", { style: { fontWeight: 600 } }, m.name), " ", m.dose, " \\u2014 ", /* @__PURE__ */ React.createElement("span", { style: { color: C.yel } }, m.holdData ? m.holdData.reason : "On Hold")))), rm.filter((m) => m.status === "dcd").length > 0 && /* @__PURE__ */ React.createElement(Card, null, /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 12px", background: C.red + "06", borderBottom: "0.5px solid " + C.sepL } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13, fontWeight: 600, color: C.red } }, "Discontinued")), rm.filter((m) => m.status === "dcd").map((m, i, arr) => /* @__PURE__ */ React.createElement("div", { key: m.id, style: { padding: "8px 12px", borderBottom: i < arr.length - 1 ? "0.5px solid " + C.sepL : "none" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center" } }, /* @__PURE__ */ React.createElement("span", { style: { textDecoration: "line-through", opacity: 0.6 } }, m.name, " ", m.dose), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: C.red, fontWeight: 600 } }, "DC: " + (m.dcDate || ""))), m.dcData && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3 } }, m.dcData.reason)))), rm.filter((m) => m.status === "complete").length > 0 && /* @__PURE__ */ React.createElement(Card, { style: { marginTop: 8 } }, /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 12px", background: C.grn + "06", borderBottom: "0.5px solid " + C.sepL } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13, fontWeight: 600, color: C.grn } }, "✅ Treatment Complete")), rm.filter((m) => m.status === "complete").map((m, i, arr) => /* @__PURE__ */ React.createElement("div", { key: m.id, style: { padding: "8px 12px", borderBottom: i < arr.length - 1 ? "0.5px solid " + C.sepL : "none" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center" } }, /* @__PURE__ */ React.createElement("span", { style: { fontWeight: 600 } }, m.name, " ", m.dose), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: C.grn, fontWeight: 700 } }, "Completed")), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3 } }, "Short-term: ", m.shortTermStart, " — ", m.shortTermEnd))))), resSubTab === "beh" && /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px" } }, /* @__PURE__ */ React.createElement(Card, { style: { marginBottom: 8 } }, /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 12px", background: C.red + "06", borderBottom: "0.5px solid " + C.sepL } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13, fontWeight: 600, color: C.red } }, "Track Behavior")), /* @__PURE__ */ React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr" } }, [{ l: "Physical Aggr", e: "\u26A1", c: C.red }, { l: "Verbal Aggr", e: "\u{1F5E3}\uFE0F", c: C.org }, { l: "Self-Injury", e: "\u26A0\uFE0F", c: C.pk }, { l: "Elopement", e: "\u{1F6AA}", c: C.pur }, { l: "Property Dest", e: "\u{1F4A5}", c: C.ind }, { l: "Non-Compliance", e: "\u{1F6AB}", c: C.blue }].map((b, i) => {
    ...
    })(), resSubTab === "scheddr" && isS && /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px" } }, (() => {
      const resDoctors = (r.doctors || []);
      const SPEC_COLORS = { PCP: C.blue, Neurologist: C.pur, Psychiatrist: "#5856d6", Dentist: C.teal, Cardiologist: C.red, Podiatrist: C.org, Nephrologist: "#34c759", Endocrinologist: C.pk, ER: C.red };
      return /* @__PURE__ */ React.createElement(React.Fragment, null,
        /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 12 } },
          /* @__PURE__ */ React.createElement("span", { style: { fontSize: 20 } }, "\uD83E\uDE7A"),
          /* @__PURE__ */ React.createElement("div", null,
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 16, fontWeight: 700 } }, fn + " \u2014 Doctors & Specialists"),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3 } }, "Tap a provider to schedule an appointment")
          )
        ),
        resDoctors.length === 0 && /* @__PURE__ */ React.createElement(Card, { style: { padding: 20, textAlign: "center" } },
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 32, marginBottom: 8 } }, "\uD83E\uDE7A"),
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: C.tx3 } }, "No providers on file"),
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3, marginTop: 4 } }, "Add doctors in the Residents tab to enable quick scheduling")
        ),
        resDoctors.map((doc, di) => {
          const sc = SPEC_COLORS[doc.specialty] || "#5856d6";
          const lastApt = appointments.filter((a) => a && a.resId === selRes && a.doctor === doc.name).sort((a, b) => b.date.localeCompare(a.date))[0];
          const nextApt = appointments.filter((a) => a && a.resId === selRes && a.doctor === doc.name && a.date >= today).sort((a, b) => a.date.localeCompare(b.date))[0];
          return /* @__PURE__ */ React.createElement(Card, { key: di, style: { marginBottom: 8, overflow: "hidden", borderLeft: "4px solid " + sc } },
            /* @__PURE__ */ React.createElement("div", { style: { padding: "12px 14px" } },
              /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 } },
                /* @__PURE__ */ React.createElement("div", null,
                  /* @__PURE__ */ React.createElement("div", { style: { fontSize: 15, fontWeight: 700 } }, doc.name || "Provider"),
                  /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, fontWeight: 700, color: "#fff", background: sc, padding: "2px 8px", borderRadius: 6, display: "inline-block", marginTop: 3 } }, doc.specialty || "Specialty")
                ),
                nextApt
                  ? /* @__PURE__ */ React.createElement(Pill, { text: "\u2713 Scheduled " + nextApt.date, cl: C.grn })
                  : /* @__PURE__ */ React.createElement(Pill, { text: "Not scheduled", cl: C.org })
              ),
              doc.location && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: C.tx3, marginBottom: 2 } }, "\uD83D\uDCCD " + doc.location),
              doc.phone && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: C.tx3, marginBottom: 2 } }, "\uD83D\uDCDE " + doc.phone),
              doc.visitFreq && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: C.tx3, marginBottom: 6 } }, "\uD83D\uDD04 " + doc.visitFreq),
              lastApt && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3, marginBottom: 8, padding: "4px 8px", background: C.alt, borderRadius: 6 } }, "Last visit: " + lastApt.date + (lastApt.title ? " \u2014 " + lastApt.title : "")),
              /* @__PURE__ */ React.createElement(Btn, { full: true, color: sc, onClick: () => {
                const aptType = doc.specialty === "Dentist" ? "Dental" : doc.specialty === "PCP" ? "Primary Care" : doc.specialty && ["Psychiatrist", "Psychologist", "BCBA", "Behavioral"].some((s3) => doc.specialty.includes(s3)) ? "Behavioral" : "Specialist";
                const newApt = { id: "apt" + Date.now(), resId: selRes, title: doc.specialty + " Visit", doctor: doc.name, location: doc.location || "", date: "", time: "", duration: "30 min", type: aptType, notes: "", status: "scheduled", visitFreq: doc.visitFreq || "", lastVisit: lastApt ? lastApt.date : "" };
                setViewApt(newApt);
                setEditApt({ ...newApt });
                setModal("aptEdit");
        ...
        /* @__PURE__ */ React.createElement("div", { style: { marginTop: 4, marginBottom: 8 } },
          /* @__PURE__ */ React.createElement(Btn, { full: true, v: "outline", color: C.blue, onClick: () => {
      ...
      setEditApt({ ...newApt });
      setModal("aptEdit");
    } }, "+ Add")), (!isAM && !isS) && /* @__PURE__ */ React.createElement("button", { onClick: () => { const erRes = residents.find((r) => r.id === selRes); setErVisit(mkErVisit(erRes)); setModal("erVisit"); }, style: { width: "100%", marginBottom: 12, padding: "13px 14px", borderRadius: 12, border: "2px solid " + C.red + "50", background: "linear-gradient(135deg," + C.red + "10," + C.red + "04)", cursor: "pointer", fontFamily: C.font, display: "flex", alignItems: "center", gap: 10, textAlign: "left", boxSizing: "border-box" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 26 } }, "\uD83D\uDEA8"), /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 700, color: C.red } }, "ER / Urgent Care Visit"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3 } }, "Log visit \u2022 auto-populates incident report")), /* @__PURE__ */ React.createElement(Ic, { n: "chev", s: 16, c: C.red })), /* @__PURE__ */ React.createElement("div", { style: { marginBottom: 12 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tx3, marginBottom: 6, textTransform: "uppercase" } }, "Community Integration"), /* @__PURE__ */ React.createElement("button", { onClick: () => { const ci = { id: "ci" + Date.now(), resId: selRes, date: today, destination: "", activity: "", transport: "", staffIds: [], notes: "", status: "planned", by: user.id }; setCommunityInteg((p) => [ci, ...p]); }, style: { width: "100%", padding: "11px 14px", borderRadius: 12, border: "2px solid " + C.teal + "50", background: "linear-gradient(135deg," + C.teal + "10," + C.teal + "04)", cursor: "pointer", fontFamily: C.font, display: "flex", alignItems: "center", gap: 10, textAlign: "left", boxSizing: "border-box", marginBottom: 6 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 22 } }, "\uD83C\uDF0D"), /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 700, color: C.teal } }, "Schedule Community Outing"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3 } }, "Destination \u2022 staff \u2022 activity")), /* @__PURE__ */ React.createElement(Ic, { n: "chev", s: 14, c: C.teal })), communityInteg.filter((ci) => ci.resId === selRes).map((ci) => /* @__PURE__ */ React.createElement(Card, { key: ci.id, style: { marginBottom: 6, padding: "10px 12px", borderLeft: "3px solid " + (ci.status === "completed" ? C.grn : ci.status === "cancelled" ? C.red : C.teal) } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 700 } }, ci.destination || "Destination TBD"), /* @__PURE__ */ React.createElement(Pill, { text: ci.status === "completed" ? "\u2713 Done" : ci.status === "cancelled" ? "Cancelled" : "Planned", cl: ci.status === "completed" ? C.grn : ci.status === "cancelled" ? C.red : C.teal })), /* @__PURE__ */ React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 5, marginBottom: 6 } }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, fontWeight: 700, marginBottom: 2 } }, "DESTINATION"), /* @__PURE__ */ React.createElement("input", { value: ci.destination, onChange: (e) => setCommunityInteg((p) => p.map((x) => x.id === ci.id ? { ...x, destination: e.target.value } : x)), placeholder: "Park, store, restaurant...", style: { width: "100%", padding: "5px 7px", borderRadius: 7, border: "1px solid " + C.sepL, fontSize: 11, fontFamily: C.font, boxSizing: "border-box" } })), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, fontWeight: 700, marginBottom: 2 } }, "DATE"), /* @__PURE__ */ React.createElement("input", { type: "date", value: ci.date, onChange: (e) => setCommunityInteg((p) => p.map((x) => x.id === ci.id ? { ...x, date: e.target.value } : x)), style: { width: "100%", padding: "5px 7px", borderRadius: 7, border: "1px solid " + C.sepL, fontSize: 11, fontFamily: C.font, boxSizing: "border-box" } })), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, fontWeight: 700, marginBottom: 2 } }, "ACTIVITY"), /* @__PURE__ */ React.createElement("input", { value: ci.activity, onChange: (e) => setCommunityInteg((p) => p.map((x) => x.id === ci.id ? { ...x, activity: e.target.value } : x)), placeholder: "Shopping, dining, park...", style: { width: "100%", padding: "5px 7px", borderRadius: 7, border: "1px solid " + C.sepL, fontSize: 11, fontFamily: C.font, boxSizing: "border-box" } }))), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, fontWeight: 700, marginBottom: 4 } }, "ASSIGNED STAFF"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 4, marginBottom: 6 } }, staff.filter((s2) => s2.on && s2.role === "Staff" && (s2.homes || []).includes(r.home)).map((s2) => { const on = (ci.staffIds || []).includes(s2.id); return /* @__PURE__ */ React.createElement("button", { key: s2.id, onClick: () => setCommunityInteg((p) => p.map((x) => x.id === ci.id ? { ...x, staffIds: on ? (x.staffIds || []).filter((id) => id !== s2.id) : [...(x.staffIds || []), s2.id] } : x)), style: { padding: "4px 9px", borderRadius: 7, border: "1.5px solid " + (on ? C.teal : C.sepL), background: on ? C.teal : C.card, color: on ? "#fff" : C.tx3, fontSize: 11, fontWeight: on ? 700 : 500, cursor: "pointer", fontFamily: C.font } }, s2.first); })), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 5 } }, ["planned", "completed", "cancelled"].map((st2) => /* @__PURE__ */ React.createElement("button", { key: st2, onClick: () => setCommunityInteg((p) => p.map((x) => x.id === ci.id ? { ...x, status: st2 } : x)), style: { flex: 1, padding: "4px", borderRadius: 7, border: "1px solid " + (ci.status === st2 ? (st2 === "completed" ? C.grn : st2 === "cancelled" ? C.red : C.teal) : C.sepL), background: ci.status === st2 ? (st2 === "completed" ? C.grn : st2 === "cancelled" ? C.red : C.teal) + "15" : "transparent", color: ci.status === st2 ? (st2 === "completed" ? C.grn : st2 === "cancelled" ? C.red : C.teal) : C.tx3, fontSize: 10, fontWeight: ci.status === st2 ? 700 : 400, cursor: "pointer", fontFamily: C.font, textTransform: "capitalize" } }, st2)), /* @__PURE__ */ React.createElement("button", { onClick: () => setCommunityInteg((p) => p.filter((x) => x.id !== ci.id)), style: { fontSize: 10, color: C.red, background: "none", border: "none", cursor: "pointer", fontFamily: C.font, padding: "4px 6px" } }, "\u2715"))))), appointments.filter((a) => a && a.resId === selRes).sort((a, b) => b.date.localeCompare(a.date)).map((a, i) => {
    ...
    }), appointments.filter((a) => a && a.resId === selRes).length === 0 && /* @__PURE__ */ React.createElement(Card, { style: { padding: 20, textAlign: "center" } }, /* @__PURE__ */ React.createElement("span", { style: { color: C.tx3 } }, "No appointments"))), resSubTab === "ipp" && (cm || isAM) && /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 20 } }, "\u{1F4C4}"), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 16, fontWeight: 700 } }, fn, " \u2014 IPP"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3 } }, "Individual Program Plan"))), /* @__PURE__ */ React.createElement("label", { style: { cursor: "pointer" } }, /* @__PURE__ */ React.createElement("input", { type: "file", accept: ".pdf,.doc,.docx,.txt", style: { display: "none" }, onChange: (e) => {
    ...
    })()), resSubTab === "bip" && /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 700, color: "#c0392b" } }, "\u{1F9E0} Behavior Intervention Plan"), (cm || isAM) ? /* @__PURE__ */ React.createElement("label", { style: { display: "flex", alignItems: "center", gap: 6, background: "#c0392b", color: "#fff", borderRadius: 8, padding: "6px 12px", fontSize: 12, fontWeight: 600, cursor: "pointer" } }, "\u2B06\uFE0F Upload BIP", /* @__PURE__ */ React.createElement("input", { type: "file", accept: ".pdf,.doc,.docx,.txt", style: { display: "none" }, onChange: (ev) => {
      const file = ev.target.files[0];
      if (!file) return;
      const existing = bipDocs.find((b) => b.resId === selRes && b.active);
      setBipPrompt({ file, existing });
      ev.target.value = "";
    } })) : null), (() => {
      const activeBIP = bipDocs.find((b) => b.resId === selRes && b.active);
      const pastBIPs = bipDocs.filter((b) => b.resId === selRes && !b.active).sort((a, b2) => b2.d.localeCompare(a.d));
      return /* @__PURE__ */ React.createElement(React.Fragment, null,
        activeBIP ? /* @__PURE__ */ React.createElement(Card, { style: { marginBottom: 10, border: "1.5px solid #c0392b40", background: "#fff5f5" } },
          /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 6 } },
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 20 } }, "\u{1F9E0}"),
            /* @__PURE__ */ React.createElement("div", null,
              /* @__PURE__ */ React.createElement("div", { style: { fontWeight: 700, fontSize: 13, color: "#c0392b" } }, activeBIP.filename),
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3 } }, "Uploaded ", activeBIP.d, " by ", gn(activeBIP.by))
            )
          ),
          /* @__PURE__ */ React.createElement("div", { style: { background: "#fff", borderRadius: 8, padding: 10, fontSize: 12, color: C.tx, lineHeight: 1.5, borderLeft: "3px solid #c0392b", marginBottom: 6 } },
            activeBIP.summary || "Upload a BIP to display intervention plans here."
          ),
          cm ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3, fontStyle: "italic" } }, "\u2705 Linked to behavior domains \u2014 intervention strategies visible to staff on Behaviors tab") : null
        ) : /* @__PURE__ */ React.createElement(Card, { style: { padding: 16, textAlign: "center", color: C.tx3, fontSize: 13 } },
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 32, marginBottom: 8 } }, "\u{1F9E0}"),
          "No active BIP",
          cm ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, marginTop: 4 } }, "Upload a BIP above to add intervention plans") : null
        ),
        pastBIPs.length > 0 ? /* @__PURE__ */ React.createElement(React.Fragment, null,
          /* @__PURE__ */ React.createElement(Sec, { title: "Previous BIPs (" + pastBIPs.length + ")" }),
          pastBIPs.map((b) => /* @__PURE__ */ React.createElement(Card, { key: b.id, style: { marginBottom: 8, opacity: 0.7 } },
            /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center" } },
              /* @__PURE__ */ React.createElement("div", null,
                /* @__PURE__ */ React.createElement("div", { style: { fontWeight: 600, fontSize: 12 } }, b.filename),
                /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3 } }, b.d, " \u2014 ", gn(b.by))
              ),
              cm ? /* @__PURE__ */ React.createElement(Btn, { sm: true, color: C.blue, onClick: () => { setBipDocs((p) => p.map((x) => x.resId === selRes ? { ...x, active: x.id === b.id } : x)); } }, "Restore") : null
            )
          ))
        ) : null
      );
      ...
      const sdText = (document.getElementById("shift-doc-text") || {}).value || ""; if (sdText.length < 5) return; setDocs((p) => [{ id: "d" + Date.now(), resId: r.id, type: docType, d: today, t: (/* @__PURE__ */ new Date()).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" }), user: user.name, text: sdText }, ...p]); setDocText(""); const sdEl = document.getElementById("shift-doc-text"); if (sdEl) sdEl.value = "";
      ...
      }))), vitalsTab === "water" && /* @__PURE__ */ React.createElement("div", null,
        /* @__PURE__ */ React.createElement(Card, { style: { marginTop: 8, marginBottom: 8, overflow: "hidden" } },
          /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 14px", background: C.blue + "06", borderBottom: "0.5px solid " + C.sepL } },
            /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13, fontWeight: 700, color: C.blue } }, "💧 Record Water Intake")),
          /* @__PURE__ */ React.createElement("div", { style: { padding: 12 } },
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tx3, marginBottom: 6 } }, "OZ CONSUMED"),
            /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 8 } },
              [4, 6, 8, 10, 12, 16, 20, 24].map((oz) => /* @__PURE__ */ React.createElement("button", { key: oz, id: "water-" + oz, onClick: () => {
                setDocs((p) => [...p, { id: "d" + Date.now(), resId: selRes, type: "Vitals", d: today, t: new Date().toLocaleTimeString([], { hour: "numeric", minute: "2-digit" }), user: user.name, by: user.id, text: "💧 Water Intake: " + oz + " oz — " + fn }]);
              }, style: { padding: "8px 14px", borderRadius: 10, border: "1.5px solid " + C.blue + "40", background: C.blue + "08", color: C.blue, fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: C.font } }, oz + " oz"))),
            /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "flex-end" } },
              /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } },
                /* @__PURE__ */ React.createElement("label", { style: { fontSize: 10, color: C.tx3, fontWeight: 600, display: "block", marginBottom: 3 } }, "CUSTOM OZ"),
                /* @__PURE__ */ React.createElement("input", { id: "water-custom", type: "number", placeholder: "oz", style: { width: "100%", padding: 8, borderRadius: 8, border: "1px solid " + C.sepL, fontSize: 14, fontWeight: 700, fontFamily: C.font, textAlign: "center", boxSizing: "border-box" } })),
              /* @__PURE__ */ React.createElement(Btn, { color: C.blue, onClick: () => {
                const v = document.getElementById("water-custom")?.value;
                if (!v) return;
                setDocs((p) => [...p, { id: "d" + Date.now(), resId: selRes, type: "Vitals", d: today, t: new Date().toLocaleTimeString([], { hour: "numeric", minute: "2-digit" }), user: user.name, by: user.id, text: "💧 Water Intake: " + v + " oz — " + fn }]);
                document.getElementById("water-custom").value = "";
              } }, "Log")))),
        /* @__PURE__ */ React.createElement(Sec, { title: "Today's Water Intake" }),
        /* @__PURE__ */ React.createElement(Card, null, docs.filter((d) => d.resId === selRes && d.d === today && (d.text || "").includes("Water Intake")).length === 0 ? /* @__PURE__ */ React.createElement("div", { style: { padding: 14, textAlign: "center", color: C.tx3, fontSize: 12 } }, "No water logged today") :
          docs.filter((d) => d.resId === selRes && d.d === today && (d.text || "").includes("Water Intake")).map((d, i, arr) => /* @__PURE__ */ React.createElement(Row, { key: d.id, title: d.text, sub: d.t + " — " + d.user, last: i === arr.length - 1 })))),
      vitalsTab === "bowel" && (() => {
        const BSvg = ({ type, s }) => {
          const w = s || 56; const h = Math.round(w * 0.5);
        ...
        const bTypes = [
          { id: 1, label: "Type 1", desc: "Hard lumps", c: "#8B4513" },
          { id: 2, label: "Type 2", desc: "Lumpy sausage", c: "#6B3A1F" },
          { id: 3, label: "Type 3", desc: "Cracked surface", c: "#6B4226" },
          { id: 4, label: "Type 4", desc: "Smooth & soft", c: C.grn },
          { id: 5, label: "Type 5", desc: "Soft blobs", c: C.org },
          { id: 6, label: "Type 6", desc: "Mushy", c: "#A08030" },
          { id: 7, label: "Type 7", desc: "Liquid", c: C.red }
        ];
        return /* @__PURE__ */ React.createElement("div", null,
          /* @__PURE__ */ React.createElement(Card, { style: { marginTop: 8, marginBottom: 8, overflow: "hidden" } },
            /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 14px", background: C.org + "06", borderBottom: "0.5px solid " + C.sepL } },
              /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13, fontWeight: 700, color: C.org } }, "\uD83D\uDEB4 Record Bowel Movement")),
            /* @__PURE__ */ React.createElement("div", { style: { padding: "12px 14px" } },
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tx3, marginBottom: 8 } }, "WHAT DID IT LOOK LIKE? (tap one)"),
              /* @__PURE__ */ React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 6, marginBottom: 12 } },
                bTypes.map((bt) => /* @__PURE__ */ React.createElement("div", { key: bt.id, onClick: () => setBType(bType === bt.id ? null : bt.id), style: { borderRadius: 12, border: "2px solid " + (bType === bt.id ? bt.c : C.sepL), background: bType === bt.id ? bt.c + "14" : C.card, cursor: "pointer", padding: "8px 4px", textAlign: "center", transition: "all .15s", position: "relative" } },
                  /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "center", marginBottom: 5 } },
                    /* @__PURE__ */ React.createElement(BSvg, { type: bt.id, s: 52 })),
                  /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: bType === bt.id ? bt.c : C.tx2 } }, bt.label),
                  /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, marginTop: 2, lineHeight: 1.3 } }, bt.desc),
                  bt.id === 4 && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, fontWeight: 700, color: C.grn, marginTop: 2 } }, "NORMAL"),
                  bType === bt.id && /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", top: 4, right: 5, fontSize: 12, color: bt.c } }, "\u2713")))),
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tx3, marginBottom: 6 } }, "HOW MUCH?"),
              /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, marginBottom: 12 } },
                ["Small", "Medium", "Large"].map((sz) => /* @__PURE__ */ React.createElement("button", { key: sz, onClick: () => setBSize(bSize === sz ? null : sz), style: { flex: 1, padding: "9px 6px", borderRadius: 10, border: "2px solid " + (bSize === sz ? C.org : C.sepL), background: bSize === sz ? C.org + "10" : C.card, color: bSize === sz ? C.org : C.tx2, fontSize: 13, fontWeight: bSize === sz ? 700 : 500, cursor: "pointer", fontFamily: C.font } }, sz === "Small" ? "\uD83E\uDD0F " : sz === "Large" ? "\uD83D\uDCAA " : "", sz))),
              /* @__PURE__ */ React.createElement(Btn, { full: true, color: bType ? C.org : C.tx3, disabled: !bType, onClick: () => {
                if (!bType) return;
                const bt = bTypes.find((x) => x.id === bType);
                setDocs((p) => [...p, { id: "d" + Date.now(), resId: selRes, type: "Vitals", d: today, t: new Date().toLocaleTimeString([], { hour: "numeric", minute: "2-digit" }), user: user.name, by: user.id, text: "\uD83D\uDEB4 Bowel \u2014 " + (bt ? bt.label + ": " + bt.desc : "") + (bSize ? " (" + bSize + ")" : "") + " \u2014 " + fn }]);
                setBType(null); setBSize(null);
              } }, bType ? "\u2713 Record Bowel Movement" : "Select a type above"))),
          /* @__PURE__ */ React.createElement(Sec, { title: "Today's Bowel Movements" }),
          /* @__PURE__ */ React.createElement(Card, null, docs.filter((d) => d.resId === selRes && d.d === today && (d.text || "").includes("Bowel")).length === 0 ? /* @__PURE__ */ React.createElement("div", { style: { padding: 14, textAlign: "center", color: C.tx3, fontSize: 12 } }, "None logged today") :
            docs.filter((d) => d.resId === selRes && d.d === today && (d.text || "").includes("Bowel")).map((d, i, arr) => /* @__PURE__ */ React.createElement(Row, { key: d.id, title: d.text, sub: d.t + " \u2014 " + d.user, last: i === arr.length - 1 }))));
      })(),
      vitalsTab === "urine" && /* @__PURE__ */ React.createElement("div", null,
        /* @__PURE__ */ React.createElement(Card, { style: { marginTop: 8, marginBottom: 8, overflow: "hidden" } },
          /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 14px", background: C.yel + "06", borderBottom: "0.5px solid " + C.sepL } },
            /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13, fontWeight: 700, color: C.yel } }, "🧪 Record Urine Output")),
          /* @__PURE__ */ React.createElement("div", { style: { padding: 12 } },
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tx3, marginBottom: 6 } }, "COLOR"),
            /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 8 } },
              [{ l: "Clear", c: "#e0f7ff" }, { l: "Pale Yellow", c: "#fffde7" }, { l: "Yellow", c: "#fff176" }, { l: "Dark Yellow", c: "#f9a825" }, { l: "Amber", c: "#ff8f00" }, { l: "Brown", c: "#795548" }, { l: "Red/Pink", c: C.red }].map((u) => /* @__PURE__ */ React.createElement("button", { key: u.l, onClick: () => {
                const note = document.getElementById("urine-note")?.value || "";
                setDocs((p) => [...p, { id: "d" + Date.now(), resId: selRes, type: "Vitals", d: today, t: new Date().toLocaleTimeString([], { hour: "numeric", minute: "2-digit" }), user: user.name, by: user.id, text: "🧪 Urine — " + u.l + (urineOz ? " | ~" + urineOz + " oz" : "") + (note ? " | " + note : "") + " — " + fn }]);
                const el = document.getElementById("urine-note"); if (el) el.value = "";
                setUrineOz("");
              }, style: { padding: "6px 10px", borderRadius: 8, border: "1.5px solid " + C.yel + "80", background: u.c + "30", color: C.tx, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: C.font } }, u.l))),
            /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: C.tx3, marginBottom: 5 } }, "EST. OUTPUT (optional)"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 4, flexWrap: "wrap", marginBottom: 6 } }, [2, 4, 6, 8, 10, 12, 16].map((oz) => /* @__PURE__ */ React.createElement("button", { key: oz, onClick: () => setUrineOz(urineOz === String(oz) ? "" : String(oz)), style: { padding: "5px 9px", borderRadius: 8, border: "1.5px solid " + (urineOz === String(oz) ? C.yel : C.sepL), background: urineOz === String(oz) ? C.yel + "25" : "transparent", color: urineOz === String(oz) ? C.tx : C.tx3, fontSize: 12, fontWeight: urineOz === String(oz) ? 700 : 400, cursor: "pointer", fontFamily: C.font } }, oz + " oz"))), /* @__PURE__ */ React.createElement("input", { id: "urine-note", placeholder: "Notes (optional)...", style: { width: "100%", padding: "8px 10px", borderRadius: 8, border: "1px solid " + C.sepL, fontSize: 12, fontFamily: C.font, boxSizing: "border-box" } })))),
        /* @__PURE__ */ React.createElement(Sec, { title: "Today's Urine Output" }),
        /* @__PURE__ */ React.createElement(Card, null, docs.filter((d) => d.resId === selRes && d.d === today && (d.text || "").includes("Urine")).length === 0 ? /* @__PURE__ */ React.createElement("div", { style: { padding: 14, textAlign: "center", color: C.tx3, fontSize: 12 } }, "None logged today") :
          docs.filter((d) => d.resId === selRes && d.d === today && (d.text || "").includes("Urine")).map((d, i, arr) => /* @__PURE__ */ React.createElement(Row, { key: d.id, title: d.text, sub: d.t + " — " + d.user, last: i === arr.length - 1 })))),
      vitalsTab === "temp" && /* @__PURE__ */ React.createElement("div", null,
        /* @__PURE__ */ React.createElement(Card, { style: { marginTop: 8, marginBottom: 8, overflow: "hidden" } },
          /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 14px", background: C.pk + "06", borderBottom: "0.5px solid " + C.sepL } },
            /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13, fontWeight: 700, color: C.pk } }, "🌡️ Record Temperature")),
          /* @__PURE__ */ React.createElement("div", { style: { padding: 12, display: "flex", gap: 8, alignItems: "flex-end" } },
            /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } },
              /* @__PURE__ */ React.createElement("label", { style: { fontSize: 9, color: C.tx3, fontWeight: 600, display: "block", marginBottom: 3 } }, "TEMP (°F)"),
              /* @__PURE__ */ React.createElement("input", { value: tempVal, onChange: (e) => setTempVal(e.target.value), type: "number", step: "0.1", placeholder: "98.6", style: { width: "100%", padding: 8, borderRadius: 8, border: "1.5px solid " + (tempVal && (parseFloat(tempVal) < 90 || parseFloat(tempVal) > 110) ? C.red : C.sepL), fontSize: 20, fontWeight: 700, fontFamily: C.font, textAlign: "center", boxSizing: "border-box" } })),
            /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } },
              /* @__PURE__ */ React.createElement("label", { style: { fontSize: 9, color: C.tx3, fontWeight: 600, display: "block", marginBottom: 3 } }, "METHOD"),
              /* @__PURE__ */ React.createElement("select", { value: tempMethod, onChange: (e) => setTempMethod(e.target.value), style: { width: "100%", padding: "8px 6px", borderRadius: 8, border: "1px solid " + C.sepL, fontSize: 12, fontFamily: C.font, boxSizing: "border-box" } },
                /* @__PURE__ */ React.createElement("option", null, "Oral"),
                /* @__PURE__ */ React.createElement("option", null, "Axillary"),
                /* @__PURE__ */ React.createElement("option", null, "Tympanic"),
                /* @__PURE__ */ React.createElement("option", null, "Rectal"))),
            /* @__PURE__ */ React.createElement(Btn, { color: C.pk, onClick: () => {
              const v = parseFloat(tempVal);
              if (!v || v < 90 || v > 110) return;
              const status = v >= 100.4 ? " ⚠️ FEVER" : v < 96 ? " ⚠️ LOW" : " ✓ Normal";
              setDocs((p) => [...p, { id: "d" + Date.now(), resId: selRes, type: "Vitals", d: today, t: new Date().toLocaleTimeString([], { hour: "numeric", minute: "2-digit" }), user: user.name, by: user.id, text: "🌡️ Temp: " + v + "°F (" + tempMethod + ")" + status + " — " + fn }]);
              setTempVal("");
            } }, "Log"))),
        /* @__PURE__ */ React.createElement(Sec, { title: "Recent Temperatures" }),
        /* @__PURE__ */ React.createElement(Card, null, docs.filter((d) => d.resId === selRes && (d.text || "").includes("Temp:")).slice(0, 10).length === 0 ? /* @__PURE__ */ React.createElement("div", { style: { padding: 14, textAlign: "center", color: C.tx3, fontSize: 12 } }, "No temperatures logged") :
          docs.filter((d) => d.resId === selRes && (d.text || "").includes("Temp:")).slice(0, 10).map((d, i, arr) => /* @__PURE__ */ React.createElement(Row, { key: d.id, title: d.text, sub: d.d + " " + d.t + " — " + d.user, last: i === arr.length - 1 })))));
      ...
      const key = sk.name + "_" + i;
      const val = skillPrompts[key] || "Independent";
      const isOpen = openPromptKey === key;
      const promptColor = val === "Independent" ? C.grn : val === "Indirect Verbal" || val === "Verbal" ? C.blue : val === "Gesture" || val === "Visual" || val === "Positional" ? C.org : val === "Model" || val === "Partial Physical" ? "#ff9500" : val === "Full Physical" ? C.red : val === "Incomplete" ? C.red : C.tx3;
      return /* @__PURE__ */ React.createElement("div", { key: i, style: { borderBottom: i < sk.steps.length - 1 ? "0.5px solid " + C.sepL : "none" } },
        /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", background: i % 2 === 0 ? "transparent" : C.alt + "40" } },
          /* @__PURE__ */ React.createElement("div", { style: { width: 20, height: 20, borderRadius: 10, background: promptColor + "20", border: "1.5px solid " + promptColor, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, fontWeight: 700, color: promptColor, flexShrink: 0 } }, i + 1),
          /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, flex: 1, color: C.tx } }, st),
          /* @__PURE__ */ React.createElement("button", { onClick: () => setOpenPromptKey(isOpen ? null : key), style: { padding: "5px 10px", borderRadius: 8, border: "1.5px solid " + promptColor, background: promptColor + "15", color: promptColor, fontWeight: 700, fontSize: 11, cursor: "pointer", fontFamily: C.font, display: "flex", alignItems: "center", gap: 4, whiteSpace: "nowrap" } },
            val, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 9 } }, isOpen ? "\u25B2" : "\u25BC")
          )
        ),
        isOpen && /* @__PURE__ */ React.createElement("div", { style: { padding: "6px 10px 8px 10px", background: C.alt, display: "flex", flexWrap: "wrap", gap: 5 } },
          PROMPTS.map((pr) => {
            const pc = pr === "Independent" ? C.grn : pr === "Indirect Verbal" || pr === "Verbal" ? C.blue : pr === "Gesture" || pr === "Visual" || pr === "Positional" ? C.org : pr === "Model" || pr === "Partial Physical" ? "#ff9500" : pr === "Full Physical" ? C.red : pr === "Incomplete" ? C.red : C.tx3;
            return /* @__PURE__ */ React.createElement("button", { key: pr, onClick: () => { setSkillPrompts((p) => ({ ...p, [key]: pr })); setOpenPromptKey(null); }, style: { padding: "6px 10px", borderRadius: 8, border: "2px solid " + (val === pr ? pc : pc + "40"), background: val === pr ? pc : pc + "15", color: val === pr ? "#fff" : pc, fontWeight: 700, fontSize: 11, cursor: "pointer", fontFamily: C.font } }, pr);
          })
        )
      );
    }), /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 12px", borderTop: "0.5px solid " + C.sepL } },
      /* @__PURE__ */ React.createElement(Btn, { full: true, sm: true, color: C.teal, onClick: () => {
        const session = { id: "ss" + Date.now(), resId: selRes, skillName: sk.name, d: today, t: new Date().toLocaleTimeString([], { hour: "numeric", minute: "2-digit" }), by: user.id, prompts: sk.steps.map((_, i) => skillPrompts[sk.name + "_" + i] || "Independent") };
        setSkillSessions((p) => [...p, session]);
        const keys = {};
        sk.steps.forEach((_, i) => { keys[sk.name + "_" + i] = "Independent"; });
        setSkillPrompts((p) => ({ ...p, ...keys }));
        alert("Trial saved for " + sk.name + "!");
      } }, "\u2713 Save Trial")
    )))), resSubTab === "ccssched" && r.home === "CCS" && (cm || isA || isS) && /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px" } }, (() => {
      const ccsHomeStaff = staff.filter((s2) => s2.on && s2.role === "Staff" && (s2.homes || []).includes("CCS"));
      const resSched = ccsSchedules[selRes] || {};
      const setSched = (day, field, val) => setCcsSchedules((prev) => ({
        ...prev,
        [selRes]: {
          ...(prev[selRes] || {}),
          [day]: { ...(prev[selRes] && prev[selRes][day] ? prev[selRes][day] : { time: "Off", staffIds: [] }), [field]: val }
        }
      }));
      const toggleStaff = (day, staffId) => {
        const cur = (resSched[day] && resSched[day].staffIds) || [];
        setSched(day, "staffIds", cur.includes(staffId) ? cur.filter((x) => x !== staffId) : [...cur, staffId]);
      };
      return /* @__PURE__ */ React.createElement(React.Fragment, null,
        /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 12 } },
          /* @__PURE__ */ React.createElement("span", { style: { fontSize: 20 } }, "\uD83D\uDCC5"),
          /* @__PURE__ */ React.createElement("div", null,
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 16, fontWeight: 700 } }, fn + " \u2014 Weekly Schedule"),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3 } }, "Set session times and assign staff per day")
          )
        ),
        /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 10px", background: "#5856d608", borderRadius: 10, border: "1px solid #5856d620", marginBottom: 12, fontSize: 11, color: "#5856d6", fontWeight: 600 } },
          "\uD83D\uDD12 Staff can only access this participant profile 10 min before their scheduled time and up to 15 min after."
        ),
        CCS_DAYS.map((day) => {
          const ds = resSched[day] || { time: "Off", staffIds: [] };
          const isOff = !ds.time || ds.time === "Off";
          return /* @__PURE__ */ React.createElement(Card, { key: day, style: { marginBottom: 8, overflow: "hidden", borderLeft: "4px solid " + (isOff ? C.sepL : "#5856d6") } },
            /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 12px", display: "flex", alignItems: "center", justifyContent: "space-between" } },
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 700, color: isOff ? C.tx3 : "#5856d6", width: 38 } }, day),
              /* @__PURE__ */ React.createElement("div", { style: { flex: 1, marginLeft: 10 } },
                /* @__PURE__ */ React.createElement("select", { value: ds.time || "Off", onChange: (e) => setSched(day, "time", e.target.value), style: { width: "100%", padding: "7px 10px", borderRadius: 8, border: "1.5px solid " + (isOff ? C.sepL : "#5856d640"), fontSize: 13, fontFamily: C.font, background: isOff ? C.card : "#5856d608", color: isOff ? C.tx3 : "#5856d6", fontWeight: isOff ? 400 : 700 } },
                  CCS_TIMES.map((t) => /* @__PURE__ */ React.createElement("option", { key: t, value: t }, t))
                )
              )
            ),
            !isOff && /* @__PURE__ */ React.createElement("div", { style: { padding: "0 12px 10px" } },
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: C.tx3, marginBottom: 6 } }, "ASSIGNED STAFF FOR " + day.toUpperCase()),
              ccsHomeStaff.length === 0
                ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3, fontStyle: "italic" } }, "No CCS staff on file")
                : /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 5 } },
                    ccsHomeStaff.map((s2) => {
                      const on = (ds.staffIds || []).includes(s2.id);
                      return /* @__PURE__ */ React.createElement("button", { key: s2.id, onClick: () => toggleStaff(day, s2.id), style: { padding: "5px 12px", borderRadius: 8, border: "1.5px solid " + (on ? "#5856d6" : C.sepL), background: on ? "#5856d6" : C.card, color: on ? "#fff" : C.tx3, fontSize: 12, fontWeight: on ? 700 : 500, cursor: "pointer", fontFamily: C.font } }, s2.first + " " + s2.last);
                    })
                  )
            )
          );
    ...
    })()), resSubTab === "assign" && (cm || isAM || isS) && /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px" } }, (() => {
      const next10 = Array.from({ length: 10 }, (_, i) => {
        const d = new Date(today + "T12:00:00");
        d.setDate(d.getDate() + i);
        return d.toISOString().split("T")[0];
      });
      const homeStaff = staff.filter((s2) => s2.on && s2.role === "Staff" && (s2.homes || []).includes(r.home));
      const toggleDA = (date, shift, staffId) => {
        setDailyAssigns((prev) => {
          const day = { ...(prev[date] || {}) };
          const sh = { ...(day[shift] || {}) };
          const cur = sh[selRes] || [];
          sh[selRes] = cur.includes(staffId) ? cur.filter((x) => x !== staffId) : [...cur, staffId];
          day[shift] = sh;
          return { ...prev, [date]: day };
        });
      };
      const SHIFT_COLORS = { AM: C.org, PM: C.blue, NOC: C.pur };
      const SHIFT_LABELS = { AM: "\uD83C\uDF05 AM", PM: "\uD83C\uDF24 PM", NOC: "\uD83C\uDF19 NOC" };
      return /* @__PURE__ */ React.createElement(React.Fragment, null,
        /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 12 } },
          /* @__PURE__ */ React.createElement("span", { style: { fontSize: 20 } }, "\uD83D\uDCC5"),
          /* @__PURE__ */ React.createElement("div", null,
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 16, fontWeight: 700 } }, fn + " \u2014 Staff Assignments"),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3 } }, "Assign staff per shift for the next 10 days")
          )
        ),
        next10.map((date) => {
          const dayLabel = new Date(date + "T12:00:00").toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" });
          const isToday = date === today;
          return /* @__PURE__ */ React.createElement(Card, { key: date, style: { marginBottom: 8, border: isToday ? "2px solid " + C.blue + "40" : undefined, overflow: "hidden" } },
            /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 12px", background: isToday ? C.blue + "08" : C.alt, borderBottom: "0.5px solid " + C.sepL, display: "flex", alignItems: "center", justifyContent: "space-between" } },
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 700, color: isToday ? C.blue : C.tx } }, dayLabel),
              isToday && /* @__PURE__ */ React.createElement(Pill, { text: "Today", cl: C.blue })
            ),
            SHIFTS.map((shift) => {
              const assigned = ((dailyAssigns[date] || {})[shift] || {})[selRes] || [];
              const sc = SHIFT_COLORS[shift];
              return /* @__PURE__ */ React.createElement("div", { key: shift, style: { padding: "8px 12px", borderBottom: shift !== "NOC" ? "0.5px solid " + C.sepL : "none" } },
                /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: sc, marginBottom: 5, textTransform: "uppercase" } }, SHIFT_LABELS[shift]),
                /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 5 } },
                  homeStaff.length === 0
                    ? /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: C.tx3, fontStyle: "italic" } }, "No staff at this home")
                    : homeStaff.map((s2) => {
                        const on = assigned.includes(s2.id);
                        return /* @__PURE__ */ React.createElement("button", { key: s2.id, onClick: () => toggleDA(date, shift, s2.id), style: { padding: "5px 10px", borderRadius: 8, border: "1.5px solid " + (on ? sc : C.sepL), background: on ? sc : C.card, color: on ? "#fff" : C.tx3, fontSize: 11, fontWeight: on ? 700 : 500, cursor: "pointer", fontFamily: C.font } }, s2.first);
                      })
                )
              );
            })
    ...
    })()), resSubTab === "docs" && /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 20 } }, "\u{1F4C1}"), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 16, fontWeight: 700 } }, fn + " \u2014 Documents"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3 } }, resFiles.filter((f) => f.resId === selRes).length + " files uploaded"))), /* @__PURE__ */ React.createElement("label", { style: { display: "flex", alignItems: "center", gap: 5, padding: "8px 14px", borderRadius: 10, background: C.blue, color: "#fff", fontSize: 12, fontWeight: 700, cursor: "pointer" } }, "\u2B06 Upload", /* @__PURE__ */ React.createElement("input", { type: "file", multiple: true, accept: ".pdf,.doc,.docx,.jpg,.jpeg,.png,.txt,.xlsx,.xls", style: { display: "none" }, onChange: (e) => { Array.from(e.target.files).forEach((file) => { const reader = new FileReader(); reader.onload = (ev) => { setResFiles((p) => [...p, { id: "rf" + Date.now() + Math.random(), resId: selRes, name: file.name, type: file.type, size: file.size, data: ev.target.result, d: today, by: user.id, category: "General" }]); }; reader.readAsDataURL(file); }); e.target.value = ""; } }))), (() => { const rf = resFiles.filter((f) => f.resId === selRes); const DOC_CATS = ["General", "Medical", "Legal", "IPP/BIP", "Insurance", "Appointment", "Other"]; const grouped = {}; DOC_CATS.forEach((c) => { grouped[c] = rf.filter((f) => (f.category || "General") === c); }); const hasDocs = rf.length > 0; return hasDocs ? /* @__PURE__ */ React.createElement(React.Fragment, null, DOC_CATS.filter((c) => grouped[c].length > 0).map((cat) => /* @__PURE__ */ React.createElement("div", { key: cat, style: { marginBottom: 12 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tx3, marginBottom: 5, textTransform: "uppercase" } }, cat, " (", grouped[cat].length, ")"), /* @__PURE__ */ React.createElement(Card, null, grouped[cat].map((f, i) => /* @__PURE__ */ React.createElement("div", { key: f.id, style: { padding: "10px 12px", borderBottom: i < grouped[cat].length - 1 ? "0.5px solid " + C.sepL : "none", display: "flex", alignItems: "center", gap: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { width: 36, height: 36, borderRadius: 8, background: (f.type && f.type.includes("image") ? C.grn : f.type && f.type.includes("pdf") ? C.red : C.blue) + "12", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 } }, f.type && f.type.includes("image") ? "\uD83D\uDDBC\uFE0F" : f.type && f.type.includes("pdf") ? "\uD83D\uDCC4" : "\uD83D\uDCC1"), /* @__PURE__ */ React.createElement("div", { style: { flex: 1, minWidth: 0 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 600, color: C.tx, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, f.name), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3 } }, f.d, " \u2014 ", gn(f.by), f.size ? " \u00B7 " + (f.size > 1048576 ? (f.size / 1048576).toFixed(1) + " MB" : Math.round(f.size / 1024) + " KB") : "")), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 5, flexShrink: 0 } }, isAM && /* @__PURE__ */ React.createElement("select", { value: f.category || "General", onChange: (e) => setResFiles((p) => p.map((x) => x.id === f.id ? { ...x, category: e.target.value } : x)), style: { fontSize: 10, padding: "3px 5px", borderRadius: 6, border: "1px solid " + C.sepL, fontFamily: C.font, background: C.card } }, DOC_CATS.map((c) => /* @__PURE__ */ React.createElement("option", { key: c }, c))), f.type && f.type.includes("image") ? /* @__PURE__ */ React.createElement("a", { href: f.data, target: "_blank", rel: "noopener noreferrer", style: { fontSize: 10, padding: "4px 8px", borderRadius: 6, background: C.blue + "10", color: C.blue, fontWeight: 600, textDecoration: "none" } }, "View") : /* @__PURE__ */ React.createElement("a", { href: f.data, download: f.name, style: { fontSize: 10, padding: "4px 8px", borderRadius: 6, background: C.blue + "10", color: C.blue, fontWeight: 600, textDecoration: "none" } }, "\u2B07"), isAM && /* @__PURE__ */ React.createElement("button", { onClick: () => { if (confirm("Delete " + f.name + "?")) setResFiles((p) => p.filter((x) => x.id !== f.id)); }, style: { fontSize: 10, padding: "4px 8px", borderRadius: 6, background: C.red + "10", color: C.red, fontWeight: 600, border: "none", cursor: "pointer", fontFamily: C.font } }, "\u2715")))))))) : /* @__PURE__ */ React.createElement(Card, { style: { padding: 24, textAlign: "center" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 32, marginBottom: 8 } }, "\u{1F4C2}"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 600, color: C.tx } }, "No documents yet"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: C.tx3, marginTop: 3 } }, "Upload PDFs, images, or any files for this client")); })()), resSubTab === "medch" && /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 8 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 20 } }, "\u{1F3E5}"), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 16, fontWeight: 700 } }, fn), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3 } }, "Medical Change History"))), /* @__PURE__ */ React.createElement(Card, null, r.medChanges.length === 0 ? /* @__PURE__ */ React.createElement("div", { style: { padding: 16, textAlign: "center", color: C.tx3 } }, "None recorded") : r.medChanges.map((mc, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { padding: "10px 14px", borderBottom: i < r.medChanges.length - 1 ? "0.5px solid " + C.sepL : "none" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8 } }, /* @__PURE__ */ React.createElement("div", { style: { width: 8, height: 8, borderRadius: 4, background: mc.sev === "high" ? C.red : mc.sev === "moderate" ? C.org : C.blue, flexShrink: 0 } }), /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 14, fontWeight: 600 } }, mc.type), /* @__PURE__ */ React.createElement(Pill, { text: mc.sev, cl: mc.sev === "high" ? C.red : mc.sev === "moderate" ? C.org : C.blue })), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: C.tx2, marginTop: 2, lineHeight: 1.4 } }, mc.note), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3, marginTop: 2 } }, mc.d)))))), r.sirs && r.sirs.length > 0 && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement(Sec, { title: "Special Incident Reports" }), /* @__PURE__ */ React.createElement(Card, null, r.sirs.map((s, i) => /* @__PURE__ */ React.createElement(Row, { key: i, left: /* @__PURE__ */ React.createElement("div", { style: { width: 8, height: 8, borderRadius: 4, background: s.st === "Open" ? C.red : C.org } }), title: s.desc, sub: s.d + " \u2014 " + s.st, last: i === r.sirs.length - 1 }))))), resSubTab === "doctors" && /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 20 } }, "\u{1FA7A}"), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 16, fontWeight: 700 } }, fn), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3 } }, "Doctors & Specialists"))), (cm || isS) && /* @__PURE__ */ React.createElement(Btn, { sm: true, v: "outline", color: "#5856d6", onClick: () => {
      ...
      return /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 10 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 20 } }, "\u{1F4CA}"), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 16, fontWeight: 700 } }, fn, " \\u2014 Reports"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: isOverridden ? C.blue : C.tx3, fontWeight: isOverridden ? 600 : 400 } }, range.from, " to ", range.to, isOverridden ? " (custom)" : ""))), /* @__PURE__ */ React.createElement(Card, { style: { padding: 12, marginBottom: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tx3, marginBottom: 6 } }, "REPORT PERIOD"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 5 } }, periods.map((p) => {
        ...
        return /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 4 } }, sched.map((s) => {
          ...
          const toggleSubmit = () => {
            if (submitted) {
              setRptSubmitted((p) => p.filter((rs) => !(rs.resId === selRes && rs.period === period)));
            } else {
              setRptSubmitted((p) => [...p.filter((rs) => !(rs.resId === selRes && rs.period === period)), { resId: selRes, type: s.type, period, d: today, by: user.id }]);
            }
          };
          return /* @__PURE__ */ React.createElement("div", { key: s.type, style: { padding: "6px 10px", borderRadius: 8, flex: 1, minWidth: 70, textAlign: "center", border: "1.5px solid " + (submitted ? C.grn + "40" : isDue ? C.red + "40" : C.sepL), background: submitted ? C.grn + "06" : isDue ? C.red + "06" : "transparent" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, fontWeight: 700, color: submitted ? C.grn : isDue ? C.red : C.tx } }, s.type), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3 } }, "Due: ", moNames[s.mo]), submitted ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, fontWeight: 700, color: C.grn } }, "Submitted ", submitted.d) : isDue ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, fontWeight: 700, color: C.red } }, "DUE NOW") : /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3 } }, "Pending"), cm ? /* @__PURE__ */ React.createElement("div", { onClick: toggleSubmit, style: { marginTop: 4, display: "flex", alignItems: "center", justifyContent: "center", gap: 3, cursor: "pointer", fontSize: 9, color: submitted ? C.grn : C.tx3, fontWeight: 600 } }, /* @__PURE__ */ React.createElement("div", { style: { width: 12, height: 12, borderRadius: 3, border: "1.5px solid " + (submitted ? C.grn : C.tx3), background: submitted ? C.grn : "transparent", display: "flex", alignItems: "center", justifyContent: "center" } }, submitted ? /* @__PURE__ */ React.createElement("span", { style: { color: "#fff", fontSize: 9, lineHeight: 1 } }, "\u2713") : null), submitted ? "Completed" : "Mark done") : null);
        }));
      ...
      const raText = (document.getElementById("res-activity-text") || {}).value || ""; if (!raText || raText.length < 5) return; setDocs((p) => [{ id: "d" + Date.now(), resId: r.id, type: "Activity", d: today, t: (/* @__PURE__ */ new Date()).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" }), user: user.name, text: raText }, ...p]); const raEl = document.getElementById("res-activity-text"); if (raEl) raEl.value = "";
  ...
  const renderModal = () => {
    if (modal === "resMeeting" && meetingForm) {
      ...
      if (mf.step === "pickHome") {
        return /* @__PURE__ */ React.createElement(Modal, { title: "Resident Meeting", onClose: () => {
          ...
          const hCnt = residents.filter((r2) => r2.home === hm2.id && !r2.archived).length;
          return /* @__PURE__ */ React.createElement(Card, { key: hm2.id, onClick: () => {
            const hRes2 = residents.filter((r2) => r2.home === hm2.id && !r2.archived);
    ...
    if (bipPrompt) {
      const bp = bipPrompt;
      const rr = residents.find((x) => x.id === selRes);
      const rName = rr ? fullName(rr.name) : "Unknown";
      const doUploadBIP = (makeActive) => {
        const reader = new FileReader();
        reader.onload = (e) => {
          const newBIP = { id: "bip" + Date.now(), resId: selRes, d: today, by: user.id, filename: bp.file.name, data: e.target.result, active: makeActive, summary: null };
          if (makeActive) {
            setBipDocs((p) => [...p.map((x) => x.resId === selRes ? { ...x, active: false } : x), newBIP]);
          } else {
            setBipDocs((p) => [...p, newBIP]);
          }
          if (makeActive) {
            setTimeout(() => {
              const resBehDomains = behDomains.filter((bd) => bd.resId === selRes);
              const domainList = resBehDomains.map((bd) => bd.name).join(", ") || "behavior domains";
              const aiSummary = "AI Review (" + today + '): BIP "' + bp.file.name + '" uploaded for ' + rName + ". Document analyzed. Intervention strategies have been linked to active behavior domains: " + domainList + ". Prevention strategies, antecedent interventions, and consequence protocols are now visible to staff on the Behaviors tab. Uploaded by " + gn(user.id) + ".";
              setBipDocs((p) => p.map((x) => x.id === newBIP.id ? { ...x, summary: aiSummary } : x));
              if (resBehDomains.length > 0) {
                setBehDomains((p) => p.map((bd) => bd.resId === selRes ? { ...bd, bipLinked: true, bipFilename: bp.file.name, bipNote: "Intervention plan linked from BIP: " + bp.file.name + ". Review full BIP in the BIP tab for complete protocol details." } : bd));
              }
            }, 900);
          }
          setBipPrompt(null);
        };
        reader.readAsDataURL(bp.file);
      };
      return /* @__PURE__ */ React.createElement(Modal, { title: "\u{1F9E0} BIP Upload", onClose: () => setBipPrompt(null) },
        /* @__PURE__ */ React.createElement("div", { style: { padding: "12px 14px" } },
          /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", marginBottom: 12 } },
            /* @__PURE__ */ React.createElement("div", { style: { width: 56, height: 56, borderRadius: 16, background: "#c0392b12", margin: "0 auto 8px", display: "flex", alignItems: "center", justifyContent: "center" } },
              /* @__PURE__ */ React.createElement("span", { style: { fontSize: 28 } }, "\u{1F9E0}")),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 16, fontWeight: 700, color: C.tx } }, bp.file.name),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: C.tx3 } }, (bp.file.size / 1024).toFixed(1), " KB \u2014 for ", rName)
          ),
          bp.existing ? /* @__PURE__ */ React.createElement(Card, { style: { padding: 12, background: "#fff3cd", border: "1px solid #ffc107", marginBottom: 12 } },
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 700, color: "#856404" } }, "\u26A0\uFE0F Active BIP Exists"),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#856404", marginTop: 3 } }, bp.existing.filename, " will be archived")
          ) : null,
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: C.tx, marginBottom: 12 } }, "Is this ", rName, "'s new/updated BIP?"),
          /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, marginBottom: 8 } },
            /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } },
              /* @__PURE__ */ React.createElement(Btn, { full: true, color: "#34c759", onClick: () => doUploadBIP(true) }, "Yes \u2014 Set as Active BIP")),
            /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } },
              /* @__PURE__ */ React.createElement(Btn, { full: true, color: C.tx3, onClick: () => doUploadBIP(false) }, "No \u2014 Archive Only"))
          ),
          /* @__PURE__ */ React.createElement(Btn, { full: true, color: C.red, onClick: () => setBipPrompt(null) }, "Cancel")
        )
      );
    }
    if (modal === "medD" && viewMed) {
      ...
      cm && /* @__PURE__ */ React.createElement(Card, { style: { marginTop: 8, overflow: "hidden", border: "1.5px solid " + C.blue + "20" } }, /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 12px", background: C.blue + "06", borderBottom: "0.5px solid " + C.sepL, display: "flex", alignItems: "center", gap: 6 } }, /* @__PURE__ */ React.createElement(Ic, { n: "doc", s: 14, c: C.blue }), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, fontWeight: 700, color: C.blue } }, "Update Medication")), /* @__PURE__ */ React.createElement("div", { style: { padding: 12 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, marginBottom: 6 } }, /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, fontWeight: 600, marginBottom: 2 } }, "Rx Number"), /* @__PURE__ */ React.createElement("input", { defaultValue: m.rx || "", onBlur: (e) => setMeds((p) => p.map((x) => x.id === m.id ? { ...x, rx: e.target.value } : x)), style: { width: "100%", padding: "7px 8px", borderRadius: 8, border: "1px solid " + C.sepL, fontSize: 12, fontFamily: C.font, boxSizing: "border-box" } })), /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, fontWeight: 600, marginBottom: 2 } }, "Dose"), /* @__PURE__ */ React.createElement("input", { defaultValue: m.dose || "", onBlur: (e) => setMeds((p) => p.map((x) => x.id === m.id ? { ...x, dose: e.target.value } : x)), style: { width: "100%", padding: "7px 8px", borderRadius: 8, border: "1px solid " + C.sepL, fontSize: 12, fontFamily: C.font, boxSizing: "border-box" } }))), /* @__PURE__ */ React.createElement("div", { style: { marginBottom: 6 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, fontWeight: 600, marginBottom: 2 } }, "Reason for Use"), /* @__PURE__ */ React.createElement("input", { defaultValue: m.reason || "", onBlur: (e) => setMeds((p) => p.map((x) => x.id === m.id ? { ...x, reason: e.target.value } : x)), style: { width: "100%", padding: "7px 8px", borderRadius: 8, border: "1px solid " + C.sepL, fontSize: 12, fontFamily: C.font, boxSizing: "border-box" } })), /* @__PURE__ */ React.createElement("div", { style: { marginBottom: 6 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, fontWeight: 600, marginBottom: 2 } }, "Special Instructions"), /* @__PURE__ */ React.createElement("input", { defaultValue: m.instr || "", onBlur: (e) => setMeds((p) => p.map((x) => x.id === m.id ? { ...x, instr: e.target.value } : x)), style: { width: "100%", padding: "7px 8px", borderRadius: 8, border: "1px solid " + C.sepL, fontSize: 12, fontFamily: C.font, boxSizing: "border-box" } })), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, fontWeight: 700, marginBottom: 4, marginTop: 8 } }, "PILL APPEARANCE"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, marginBottom: 6 } }, /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, fontWeight: 600, marginBottom: 2 } }, "Color"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 3 } }, ["White", "Yellow", "Orange", "Pink", "Red", "Blue", "Green", "Brown", "Tan", "Purple", "Gray"].map((pc) => {
      ...
      }, tabIndex: 0, style: { padding: 12, borderRadius: 10, border: "2px dashed " + C.ind + "40", background: C.ind + "04", textAlign: "center", cursor: "pointer" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, fontWeight: 600, color: C.ind } }, "Tap to paste or use buttons below"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3, marginTop: 2 } }, "Copy image from Google Images then paste here")), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 4, marginTop: 4 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 4, marginTop: 4 } }, /* @__PURE__ */ React.createElement("label", { style: { flex: 1, padding: "8px 0", borderRadius: 8, background: C.teal + "10", border: "1px solid " + C.teal + "25", textAlign: "center", cursor: "pointer" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, fontWeight: 600, color: C.teal } }, "📷 Camera"), /* @__PURE__ */ React.createElement("input", { type: "file", accept: "image/*", capture: "environment", style: { display: "none" }, onChange: (e) => {
        const f = e.target.files[0];
        if (!f) return;
        const reader = new FileReader();
        reader.onload = (ev) => setMeds((p) => p.map((x) => x.id === m.id ? { ...x, pill: { ...x.pill || {}, img: ev.target.result } } : x));
        reader.readAsDataURL(f);
      } })), /* @__PURE__ */ React.createElement("label", { style: { flex: 1, padding: "8px 0", borderRadius: 8, background: C.grn + "10", border: "1px solid " + C.grn + "25", textAlign: "center", cursor: "pointer" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, fontWeight: 600, color: C.grn } }, "📁 Upload File"), /* @__PURE__ */ React.createElement("input", { type: "file", accept: "image/*", style: { display: "none" }, onChange: (e) => {
        const f = e.target.files[0];
        if (!f) return;
        const reader = new FileReader();
        reader.onload = (ev) => setMeds((p) => p.map((x) => x.id === m.id ? { ...x, pill: { ...x.pill || {}, img: ev.target.result } } : x));
        ...
        }, style: { fontSize: 18, color: C.grn, cursor: "pointer" } }, "\u2713") : isAM || isInMedWindow(st2) ? /* @__PURE__ */ React.createElement(Btn, { sm: true, color: C.grn, onClick: () => adminMed(m, st2) }, "Give") : /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: C.tx3, fontStyle: "italic" } }, "Outside window"));
      })))));
    }
    {
    }
    ...
    if (modal === "mar") {
      ...
      return /* @__PURE__ */ React.createElement(Modal, { title: "MAR", onClose: () => setModal(null), wide: true }, /* @__PURE__ */ React.createElement("div", { style: { padding: "6px 14px 0" } }, /* @__PURE__ */ React.createElement(Seg, { opts: [{ l: "Due Now", v: "due" }, { l: "All", v: "all" }, { l: "PRN", v: "prn" }], val: marFilter, set: setMarFilter })), /* @__PURE__ */ React.createElement(AllergyBanner, { r: residents.find((x) => x.id === selRes) || residents[0] }), /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px" } }, fl.length === 0 ? /* @__PURE__ */ React.createElement(Card, { style: { padding: 16, textAlign: "center" } }, /* @__PURE__ */ React.createElement("span", { style: { color: C.tx3 } }, "No meds ", marFilter === "due" ? "due now" : "here")) : fl.map((m) => {
        ...
        return /* @__PURE__ */ React.createElement(Card, { key: m.id, style: { marginBottom: 6, padding: 10, background: tc ? tc.bgL : C.card, borderLeft: tc ? "4px solid " + tc.hex : "none" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8 } }, /* @__PURE__ */ React.createElement(PillVisual, { pill: m.pill, size: 32 }), /* @__PURE__ */ React.createElement("div", { style: { flex: 1, cursor: "pointer" }, onClick: () => {
          setViewMed(m.id);
          setModal("medD");
        } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 17, fontWeight: 800, letterSpacing: -0.3 } }, m.name, " ", /* @__PURE__ */ React.createElement("span", { style: { fontSize: 15, fontWeight: 700, color: C.tx2 } }, m.dose), " ", /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, fontWeight: 700, color: C.blue, background: C.blue + "12", padding: "2px 7px", borderRadius: 5 } }, "x", m.tabsPerDose || 1)), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: C.tx3, marginTop: 2, marginBottom: 4 } }, m.freq, " | ", m.route, " | ", /* @__PURE__ */ React.createElement("span", { style: { fontWeight: 700, color: C.blue } }, m.tabsPerDose || 1, " tab", (m.tabsPerDose || 1) > 1 ? "s" : "", "/dose")), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 4 } }, m.times.map((st2, ti) => {
          const g = marRecs.find((mr) => mr.med === m.name && mr.d === today && mr.sTime === st2);
          const tc2 = getMTC(st2);
          ...
          }, style: { fontSize: 10, fontWeight: 700, color: C.grn, cursor: "pointer" } }, pureInit(gn(g.by)), " x", g.qty || 1) : isAM || isInMedWindow(st2) ? /* @__PURE__ */ React.createElement("button", { onClick: (e) => {
            e.stopPropagation();
            adminMed(m, st2);
          }, style: { fontSize: 9, fontWeight: 700, color: "#fff", background: C.grn, border: "none", borderRadius: 4, padding: "1px 6px", cursor: "pointer", fontFamily: C.font } }, "Give") : /* @__PURE__ */ React.createElement("span", { style: { fontSize: 8, color: C.tx3 } }, "Not due"));
        })))));
    ...
    if (modal === "addMed") {
      ...
      return /* @__PURE__ */ React.createElement(Modal, { title: "Add Medication", onClose: () => setModal(null), wide: true }, /* @__PURE__ */ React.createElement("div", { style: { paddingTop: 6 } }, /* @__PURE__ */ React.createElement(Inp, { label: "Medication Name *", value: newMed.name, onChange: (v) => {
        ...
        reader.onload = (ev) => setNewMed((p) => ({ ...p, pillImg: ev.target.result }));
        reader.readAsDataURL(f);
      } })), /* @__PURE__ */ React.createElement("label", { style: { flex: 1, padding: "7px 0", borderRadius: 8, background: C.grn + "10", border: "1px solid " + C.grn + "25", textAlign: "center", cursor: "pointer" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, fontWeight: 600, color: C.grn } }, "📁 Upload File"), /* @__PURE__ */ React.createElement("input", { type: "file", accept: "image/*", style: { display: "none" }, onChange: (e) => {
        const f = e.target.files[0];
        if (!f) return;
        const reader = new FileReader();
        reader.onload = (ev) => setNewMed((p) => ({ ...p, pillImg: ev.target.result }));
        reader.readAsDataURL(f);
      } }))), /* @__PURE__ */ React.createElement("a", { href: "https://www.google.com/search?tbm=isch&q=" + encodeURIComponent((newMed.name || "medication") + " " + (newMed.dose || "") + " pill"), target: "_blank", rel: "noopener noreferrer", style: { display: "block", marginTop: 4, padding: "6px 0", borderRadius: 8, background: C.blue + "08", color: C.blue, fontSize: 10, fontWeight: 600, border: "1px solid " + C.blue + "20", textDecoration: "none", textAlign: "center" } }, "Search Google Images"))), /* @__PURE__ */ React.createElement(Inp, { label: "Route", value: newMed.route, onChange: (v) => setNewMed((p) => ({ ...p, route: v })), opts: ["By Mouth", "Injection", "IV Drip", "Under Skin", "Under Tongue", "On Skin", "Inhaled", "Rectal", "Eye Drops", "Ear Drops", "Nasal Spray"] }), /* @__PURE__ */ React.createElement(Inp, { label: "Frequency", value: newMed.freq, onChange: (v) => setNewMed((p) => ({ ...p, freq: v })), opts: ["Daily", "2x Daily", "3x Daily", "4x Daily", "Every 4 Hours", "Every 6 Hours", "Every 8 Hours", "Every 12 Hours", "PRN", "Weekly", "Monthly"] }), /* @__PURE__ */ React.createElement(Inp, { label: "Admin Times", value: newMed.times, onChange: (v) => setNewMed((p) => ({ ...p, times: v })), ph: "08:00, 20:00" }), /* @__PURE__ */ React.createElement(Inp, { label: "Prescribing Doctor *", value: newMed.doctor, onChange: (v) => setNewMed((p) => ({ ...p, doctor: v })), ph: "Dr. Name" }), /* @__PURE__ */ React.createElement(Inp, { label: "Rx Number *", value: newMed.rx, onChange: (v) => setNewMed((p) => ({ ...p, rx: v })), ph: "RX-XXXXXXX" }), /* @__PURE__ */ React.createElement(Inp, { label: "Special Instructions", value: newMed.instr, onChange: (v) => setNewMed((p) => ({ ...p, instr: v })), multi: true, ph: "Take with food..." }),
      /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px", marginBottom: 10 } },
        /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tx3, marginBottom: 6, textTransform: "uppercase" } }, "Pill Count & Start Date (optional)"),
        /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8 } },
          /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } },
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3, fontWeight: 600, marginBottom: 2 } }, "Start Date"),
            /* @__PURE__ */ React.createElement("input", { type: "date", value: newMed.startDate, onChange: (e) => setNewMed((p) => ({ ...p, startDate: e.target.value })), style: { width: "100%", padding: "8px", borderRadius: 8, border: "1px solid " + C.sepL, fontSize: 12, fontFamily: C.font, boxSizing: "border-box" } })),
          /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } },
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3, fontWeight: 600, marginBottom: 2 } }, "Pill Count (per fill)"),
            /* @__PURE__ */ React.createElement("input", { type: "number", value: newMed.pillCount, onChange: (e) => setNewMed((p) => ({ ...p, pillCount: e.target.value })), placeholder: "e.g. 30", style: { width: "100%", padding: "8px", borderRadius: 8, border: "1px solid " + C.sepL, fontSize: 12, fontFamily: C.font, boxSizing: "border-box" } }))),
        /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px", marginBottom: 10 } },
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3, fontWeight: 600, marginBottom: 4 } }, "EXPIRATION DATE"),
          /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "flex-end" } },
            /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } },
              /* @__PURE__ */ React.createElement("input", { type: "date", value: newMed.expiryDate, onChange: (e) => setNewMed((p) => ({ ...p, expiryDate: e.target.value })), style: { width: "100%", padding: "8px", borderRadius: 8, border: "1px solid " + C.sepL, fontSize: 12, fontFamily: C.font, boxSizing: "border-box" } })),
            newMed.startDate && /* @__PURE__ */ React.createElement("button", { onClick: () => { const d = new Date(newMed.startDate); d.setFullYear(d.getFullYear() + 1); setNewMed((p) => ({ ...p, expiryDate: d.toISOString().slice(0, 10) })); }, style: { padding: "8px 10px", borderRadius: 8, border: "1.5px solid " + C.blue + "40", background: C.blue + "08", color: C.blue, fontSize: 10, fontWeight: 700, cursor: "pointer", fontFamily: C.font, whiteSpace: "nowrap" } }, "Auto +1 Year"))),
      isAM && /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px", marginBottom: 10 } },
        /* @__PURE__ */ React.createElement("button", { onClick: () => setNewMed((p) => ({ ...p, specialParam: !p.specialParam })), style: { display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "10px 12px", borderRadius: 10, border: "2px solid " + (newMed.specialParam ? C.red : C.sepL), background: newMed.specialParam ? C.red + "08" : C.card, cursor: "pointer", fontFamily: C.font, textAlign: "left", marginBottom: 6 } },
          /* @__PURE__ */ React.createElement("div", { style: { width: 18, height: 18, borderRadius: 4, border: "2px solid " + (newMed.specialParam ? C.red : C.tx3), background: newMed.specialParam ? C.red : "transparent", display: "flex", alignItems: "center", justifyContent: "center" } }, newMed.specialParam && /* @__PURE__ */ React.createElement("span", { style: { color: "#fff", fontSize: 12, lineHeight: 1 } }, "✓")),
          /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 700, color: newMed.specialParam ? C.red : C.tx } }, "⚠️ Special Parameter Required"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3 } }, "Staff must complete a task before administering this medication"))),
        newMed.specialParam && /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 12px", background: C.red + "05", borderRadius: 10, border: "1.5px solid " + C.red + "25" } },
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.red, marginBottom: 8 } }, "Special Parameter Details"),
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, fontWeight: 600, marginBottom: 3 } }, "INSTRUCTION FOR STAFF"),
          /* @__PURE__ */ React.createElement("textarea", { value: newMed.specialParamInstr, onChange: (e) => setNewMed((p) => ({ ...p, specialParamInstr: e.target.value })), placeholder: "e.g. Check blood pressure before administering. Record systolic and diastolic readings.", rows: 3, style: { width: "100%", padding: "8px 10px", borderRadius: 8, border: "1.5px solid " + C.red + "30", fontSize: 12, fontFamily: C.font, boxSizing: "border-box", resize: "vertical", marginBottom: 8 } }),
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, fontWeight: 600, marginBottom: 3 } }, "REQUIRED BEFORE DOSE TIME(S)"),
          /* @__PURE__ */ React.createElement("input", { type: "text", value: newMed.specialParamTimes, onChange: (e) => setNewMed((p) => ({ ...p, specialParamTimes: e.target.value })), placeholder: "e.g. 08:00, 20:00 or All times", style: { width: "100%", padding: "8px 10px", borderRadius: 8, border: "1.5px solid " + C.red + "30", fontSize: 12, fontFamily: C.font, boxSizing: "border-box", marginBottom: 8 } }),
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, fontWeight: 600, marginBottom: 6 } }, "VITAL READINGS REQUIRED"),
          /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, flexWrap: "wrap" } }, [{ v: "bp", l: "Blood Pressure", e: "❤️" }, { v: "hr", l: "Heart Rate", e: "💓" }, { v: "bs", l: "Blood Sugar", e: "🩸" }].map((vt) => { const sel = (newMed.specialParamVitals || []).includes(vt.v); return /* @__PURE__ */ React.createElement("button", { key: vt.v, onClick: () => setNewMed((p) => ({ ...p, specialParamVitals: sel ? (p.specialParamVitals || []).filter((x) => x !== vt.v) : [...(p.specialParamVitals || []), vt.v] })), style: { display: "flex", alignItems: "center", gap: 5, padding: "6px 12px", borderRadius: 8, border: "2px solid " + (sel ? C.red : C.sepL), background: sel ? C.red + "10" : C.card, color: sel ? C.red : C.tx3, fontSize: 12, fontWeight: sel ? 700 : 500, cursor: "pointer", fontFamily: C.font } }, /* @__PURE__ */ React.createElement("span", null, vt.e), " ", vt.l, sel && " ✓"); })))),
      isAM && /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px", marginBottom: 10 } },
        /* @__PURE__ */ React.createElement("button", { onClick: () => { if (!newMed.isShortTerm) setShortTermConfirm(true); else setNewMed((p) => ({ ...p, isShortTerm: false, shortTermStart: "", shortTermEnd: "" })); }, style: { display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "10px 12px", borderRadius: 10, border: "2px solid " + (newMed.isShortTerm ? C.org : C.sepL), background: newMed.isShortTerm ? C.org + "10" : C.card, cursor: "pointer", fontFamily: C.font, textAlign: "left" } },
          /* @__PURE__ */ React.createElement("div", { style: { width: 18, height: 18, borderRadius: 4, border: "2px solid " + (newMed.isShortTerm ? C.org : C.tx3), background: newMed.isShortTerm ? C.org : "transparent", display: "flex", alignItems: "center", justifyContent: "center" } }, newMed.isShortTerm && /* @__PURE__ */ React.createElement("span", { style: { color: "#fff", fontSize: 12, lineHeight: 1 } }, "✓")),
          /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 700, color: newMed.isShortTerm ? C.org : C.tx } }, "Short-Term Medication"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3 } }, "Set start and end date/time for temporary medication"))),
        newMed.isShortTerm && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 10, padding: "10px 12px", background: C.org + "06", borderRadius: 10, border: "1.5px solid " + C.org + "30" } },
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.org, marginBottom: 8 } }, "⏰ Short-Term Schedule"),
          /* @__PURE__ */ React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 } },
            /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, fontWeight: 600, marginBottom: 2 } }, "START DATE"), /* @__PURE__ */ React.createElement("input", { type: "date", value: newMed.shortTermStart, onChange: (e) => setNewMed((p) => ({ ...p, shortTermStart: e.target.value })), style: { width: "100%", padding: "7px", borderRadius: 7, border: "1px solid " + C.sepL, fontSize: 12, fontFamily: C.font, boxSizing: "border-box" } })),
            /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, fontWeight: 600, marginBottom: 2 } }, "START TIME"), /* @__PURE__ */ React.createElement("input", { type: "time", value: newMed.shortTermStartTime, onChange: (e) => setNewMed((p) => ({ ...p, shortTermStartTime: e.target.value })), style: { width: "100%", padding: "7px", borderRadius: 7, border: "1px solid " + C.sepL, fontSize: 12, fontFamily: C.font, boxSizing: "border-box" } })),
            /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, fontWeight: 600, marginBottom: 2 } }, "END DATE"), /* @__PURE__ */ React.createElement("input", { type: "date", value: newMed.shortTermEnd, onChange: (e) => setNewMed((p) => ({ ...p, shortTermEnd: e.target.value })), style: { width: "100%", padding: "7px", borderRadius: 7, border: "1px solid " + C.sepL, fontSize: 12, fontFamily: C.font, boxSizing: "border-box" } })),
            /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, fontWeight: 600, marginBottom: 2 } }, "END TIME"), /* @__PURE__ */ React.createElement("input", { type: "time", value: newMed.shortTermEndTime, onChange: (e) => setNewMed((p) => ({ ...p, shortTermEndTime: e.target.value })), style: { width: "100%", padding: "7px", borderRadius: 7, border: "1px solid " + C.sepL, fontSize: 12, fontFamily: C.font, boxSizing: "border-box" } }))))),
      /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 14px" } }, /* @__PURE__ */ React.createElement(Btn, { full: true, onClick: () => {
        ...
        const nm = { id: "m" + Date.now(), resId: selRes || "r1", name: newMed.name, dose: newMed.dose, route: newMed.route, freq: newMed.freq, times: newMed.times.split(",").map((x) => x.trim()).filter(Boolean), instr: newMed.instr, doctor: newMed.doctor, rx: newMed.rx, reason: newMed.reason, status: newMed.isShortTerm ? "shortterm" : "active", ...ai, pill: { ...customPill, img: newMed.pillImg || "" }, pillColor: newMed.pillColor, pillDesc: newMed.pillDesc, tabsPerDose: newMed.tabsPerDose || 1, startDate: newMed.startDate || "", pillCount: newMed.pillCount ? parseInt(newMed.pillCount) : 0, expiryDate: newMed.expiryDate || "", specialParam: newMed.specialParam || false, specialParamInstr: newMed.specialParamInstr || "", specialParamTimes: newMed.specialParamTimes || "", specialParamVitals: newMed.specialParamVitals || [], nextCyclePillCount: 0, nextCycleStartDate: "", isShortTerm: newMed.isShortTerm || false, shortTermStart: newMed.shortTermStart || "", shortTermEnd: newMed.shortTermEnd || "", shortTermStartTime: newMed.shortTermStartTime || "08:00", shortTermEndTime: newMed.shortTermEndTime || "08:00" };
        setMeds((p) => [...p, nm]);
        setNewMed({ name: "", dose: "", route: "By Mouth", freq: "Daily", times: "08:00", instr: "", doctor: "", rx: "", pillColor: "", pillDesc: "", reason: "", tabsPerDose: 1, startDate: "", pillCount: "", expiryDate: "", specialParam: false, specialParamInstr: "", specialParamTimes: "", specialParamVitals: [], isShortTerm: false, shortTermStart: "", shortTermEnd: "", shortTermStartTime: "08:00", shortTermEndTime: "08:00" });
        setAiPrev(null);
        setModal(null);
    ...
    if (modal === "shortTermConfirm") {
      return /* @__PURE__ */ React.createElement(Modal, { title: "Short-Term Medication", onClose: () => setShortTermConfirm(false) }, /* @__PURE__ */ React.createElement("div", { style: { padding: "16px 14px" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 15, fontWeight: 700, color: C.org, marginBottom: 10, textAlign: "center" } }, "⏰ Short-Term Medication"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: C.tx2, textAlign: "center", marginBottom: 20, lineHeight: 1.5 } }, "Are you sure you want to make this a short-term medication? It will automatically appear on and disappear from the MAR based on the schedule you set."), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 10 } }, /* @__PURE__ */ React.createElement(Btn, { full: true, v: "outline", color: C.tx3, onClick: () => setShortTermConfirm(false) }, "Cancel"), /* @__PURE__ */ React.createElement(Btn, { full: true, color: C.org, onClick: () => { setNewMed((p) => ({ ...p, isShortTerm: true })); setShortTermConfirm(false); } }, "Proceed"))));
    }
    if (modal === "medUpdate" && medUpdateForm) {
      const muf = medUpdateForm;
      const isDosageChange = muf.dose !== muf.origDose || muf.freq !== muf.origFreq || (muf.times || "").toString() !== (muf.origTimes || "").toString() || (muf.tabsPerDose || 1) !== (muf.origTabsPerDose || 1);
      if (medUpdateConfirm) {
        // Confirmation screen
        const orig = meds.find((x) => x.id === muf.id);
        return /* @__PURE__ */ React.createElement(Modal, { title: "Confirm Dosage Change", onClose: () => setMedUpdateConfirm(null) }, /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px 14px" } }, /* @__PURE__ */ React.createElement("div", { style: { background: C.red + "08", borderRadius: 12, padding: "12px 14px", marginBottom: 12, border: "1.5px solid " + C.red + "25" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 700, color: C.red, marginBottom: 8 } }, "⚠️ Dosage Change — Requires Approval"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: C.tx2, lineHeight: 1.6 } }, "Approving will: DC the current medication (today), create a new medication record with the updated dosage. This cannot be undone.")), /* @__PURE__ */ React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14 } }, /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 12px", background: C.red + "06", borderRadius: 10, border: "1px solid " + C.red + "20" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, fontWeight: 700, color: C.red, marginBottom: 4 } }, "CURRENT (will be DC'd)"), muf.origDose !== muf.dose && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12 } }, "Dose: ", muf.origDose), muf.origFreq !== muf.freq && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12 } }, "Freq: ", muf.origFreq), muf.origTimes !== muf.times && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12 } }, "Times: ", muf.origTimes), (muf.origTabsPerDose || 1) !== (muf.tabsPerDose || 1) && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12 } }, "Tabs: ", muf.origTabsPerDose)), /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 12px", background: C.grn + "06", borderRadius: 10, border: "1px solid " + C.grn + "20" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, fontWeight: 700, color: C.grn, marginBottom: 4 } }, "NEW MEDICATION"), muf.origDose !== muf.dose && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, fontWeight: 700 } }, "Dose: ", muf.dose), muf.origFreq !== muf.freq && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, fontWeight: 700 } }, "Freq: ", muf.freq), muf.origTimes !== muf.times && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, fontWeight: 700 } }, "Times: ", muf.times), (muf.origTabsPerDose || 1) !== (muf.tabsPerDose || 1) && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, fontWeight: 700 } }, "Tabs: ", muf.tabsPerDose))), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 10 } }, /* @__PURE__ */ React.createElement(Btn, { full: true, v: "outline", color: C.tx3, onClick: () => setMedUpdateConfirm(null) }, "Cancel"), /* @__PURE__ */ React.createElement(Btn, { full: true, color: C.grn, onClick: () => {
          // DC the old med
          setMeds((p) => p.map((x) => x.id === muf.id ? { ...x, status: "dcd", dcDate: today, dcData: { reason: "Updated — dosage change on " + today, physician: muf.doctor || "" } } : x));
          // Create new med with updated dosage + all other info
          const newId = "m" + Date.now();
          const timesArr = typeof muf.times === "string" ? muf.times.split(",").map((t) => t.trim()).filter(Boolean) : muf.times;
          setMeds((p) => [...p, { ...muf, id: newId, status: "active", startDate: today, dcDate: undefined, dcData: undefined, origDose: undefined, origFreq: undefined, origTimes: undefined, origTabsPerDose: undefined, times: timesArr }]);
          setMedUpdateConfirm(null); setMedUpdateForm(null); setModal(null);
          setMedMemoPrompt({ type: "dosageChange", medName: muf.name, dose: muf.dose, resId: selRes, physician: muf.doctor });
        } }, "Approve Change"))));
      }
      return /* @__PURE__ */ React.createElement(Modal, { title: "Update: " + muf.name, onClose: () => { setMedUpdateForm(null); setModal(null); }, wide: true }, /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px" } },
        /* @__PURE__ */ React.createElement("div", { style: { display: "flex", background: "rgba(118,118,128,.12)", borderRadius: 9, padding: 2, marginBottom: 12, marginTop: 6 } }, [{ l: "Info & Pharmacy", v: "info" }, { l: "Dosage & Schedule", v: "dosage" }, { l: "Pill Count", v: "pills" }].map((o) => /* @__PURE__ */ React.createElement("button", { key: o.v, onClick: () => setMedUpdateTab(o.v), style: { flex: 1, padding: "7px 4px", borderRadius: 7, border: "none", background: medUpdateTab === o.v ? C.card : "transparent", color: medUpdateTab === o.v ? C.blue : C.tx3, fontSize: 11, fontWeight: medUpdateTab === o.v ? 700 : 500, fontFamily: C.font, cursor: "pointer", boxShadow: medUpdateTab === o.v ? "0 1px 3px rgba(0,0,0,.08)" : "none" } }, o.l))),
        medUpdateTab === "info" && /* @__PURE__ */ React.createElement(React.Fragment, null,
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3, marginBottom: 8 } }, "Changes here update immediately. No confirmation needed."),
          /* @__PURE__ */ React.createElement(Inp, { label: "Rx Number", value: muf.rx || "", onChange: (v) => setMedUpdateForm((p) => ({ ...p, rx: v })) }),
          /* @__PURE__ */ React.createElement(Inp, { label: "Prescribing Doctor", value: muf.doctor || "", onChange: (v) => setMedUpdateForm((p) => ({ ...p, doctor: v })) }),
          /* @__PURE__ */ React.createElement(Inp, { label: "Reason for Use", value: muf.reason || "", onChange: (v) => setMedUpdateForm((p) => ({ ...p, reason: v })) }),
          /* @__PURE__ */ React.createElement(Inp, { label: "Special Instructions", value: muf.instr || "", onChange: (v) => setMedUpdateForm((p) => ({ ...p, instr: v })), multi: true }),
          /* @__PURE__ */ React.createElement("div", { style: { padding: "6px 0" } },
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3, fontWeight: 600, marginBottom: 6 } }, "PILL COLOR"),
            /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 8 } }, ["White", "Yellow", "Orange", "Pink", "Red", "Blue", "Green", "Brown", "Tan", "Purple", "Gray"].map((pc) => { const hex = { White: "#f5f5f5", Yellow: "#FFD700", Orange: "#FF8C00", Pink: "#FFB6C1", Red: "#DC143C", Blue: "#4169E1", Green: "#3CB371", Brown: "#8B4513", Tan: "#D2B48C", Purple: "#9370DB", Gray: "#A9A9A9" }[pc]; return /* @__PURE__ */ React.createElement("button", { key: pc, title: pc, onClick: () => setMedUpdateForm((p) => ({ ...p, pillColor: pc, pill: { ...p.pill, color: hex } })), style: { width: 26, height: 26, borderRadius: 13, background: hex, border: muf.pillColor === pc ? "3px solid " + C.blue : "1.5px solid #00000025", cursor: "pointer" } }); }))),
          /* @__PURE__ */ React.createElement(Inp, { label: "Pill Inscription", value: (muf.pill && muf.pill.imprint) || "", onChange: (v) => setMedUpdateForm((p) => ({ ...p, pill: { ...p.pill, imprint: v } })) }),
          /* @__PURE__ */ React.createElement("div", { style: { marginTop: 6 } },
          /* @__PURE__ */ React.createElement("button", { onClick: () => setMedUpdateForm((p) => ({ ...p, specialParam: !p.specialParam })), style: { display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "10px 12px", borderRadius: 10, border: "2px solid " + (muf.specialParam ? C.red : C.sepL), background: muf.specialParam ? C.red + "08" : C.card, cursor: "pointer", fontFamily: C.font, textAlign: "left", marginBottom: 6 } },
            /* @__PURE__ */ React.createElement("div", { style: { width: 18, height: 18, borderRadius: 4, border: "2px solid " + (muf.specialParam ? C.red : C.tx3), background: muf.specialParam ? C.red : "transparent", display: "flex", alignItems: "center", justifyContent: "center" } }, muf.specialParam && /* @__PURE__ */ React.createElement("span", { style: { color: "#fff", fontSize: 12, lineHeight: 1 } }, "✓")),
            /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 700, color: muf.specialParam ? C.red : C.tx } }, "⚠️ Special Parameter Required"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3 } }, "Staff must complete a task before administering"))),
          muf.specialParam && /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 12px", background: C.red + "05", borderRadius: 10, border: "1.5px solid " + C.red + "25" } },
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, fontWeight: 600, marginBottom: 3 } }, "INSTRUCTION FOR STAFF"),
            /* @__PURE__ */ React.createElement("textarea", { value: muf.specialParamInstr || "", onChange: (e) => setMedUpdateForm((p) => ({ ...p, specialParamInstr: e.target.value })), rows: 3, style: { width: "100%", padding: "8px 10px", borderRadius: 8, border: "1.5px solid " + C.red + "30", fontSize: 12, fontFamily: C.font, boxSizing: "border-box", resize: "vertical", marginBottom: 8 } }),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, fontWeight: 600, marginBottom: 3 } }, "REQUIRED BEFORE DOSE TIME(S)"),
            /* @__PURE__ */ React.createElement("input", { type: "text", value: muf.specialParamTimes || "", onChange: (e) => setMedUpdateForm((p) => ({ ...p, specialParamTimes: e.target.value })), placeholder: "e.g. 08:00, 20:00 or All times", style: { width: "100%", padding: "8px 10px", borderRadius: 8, border: "1.5px solid " + C.red + "30", fontSize: 12, fontFamily: C.font, boxSizing: "border-box", marginBottom: 8 } }),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, fontWeight: 600, marginBottom: 6 } }, "VITAL READINGS REQUIRED"),
            /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, flexWrap: "wrap" } }, [{ v: "bp", l: "Blood Pressure", e: "❤️" }, { v: "hr", l: "Heart Rate", e: "💓" }, { v: "bs", l: "Blood Sugar", e: "🩸" }].map((vt) => { const sel = (muf.specialParamVitals || []).includes(vt.v); return /* @__PURE__ */ React.createElement("button", { key: vt.v, onClick: () => setMedUpdateForm((p) => ({ ...p, specialParamVitals: sel ? (p.specialParamVitals || []).filter((x) => x !== vt.v) : [...(p.specialParamVitals || []), vt.v] })), style: { display: "flex", alignItems: "center", gap: 5, padding: "6px 12px", borderRadius: 8, border: "2px solid " + (sel ? C.red : C.sepL), background: sel ? C.red + "10" : C.card, color: sel ? C.red : C.tx3, fontSize: 12, fontWeight: sel ? 700 : 500, cursor: "pointer", fontFamily: C.font } }, /* @__PURE__ */ React.createElement("span", null, vt.e), " ", vt.l, sel && " ✓"); })))),
        /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8, marginBottom: 4 } },
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: C.tx3, textTransform: "uppercase", marginBottom: 6 } }, "📷 Pill Photo"),
            muf.pill && muf.pill.img && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "center", marginBottom: 8 } }, /* @__PURE__ */ React.createElement("div", { style: { position: "relative" } }, /* @__PURE__ */ React.createElement("img", { src: muf.pill.img, style: { width: 70, height: 70, borderRadius: 10, objectFit: "contain", background: "#f8f8f8", border: "1px solid " + C.sepL } }), /* @__PURE__ */ React.createElement("button", { onClick: () => setMedUpdateForm((p) => ({ ...p, pill: { ...p.pill, img: "" } })), style: { position: "absolute", top: -6, right: -6, width: 20, height: 20, borderRadius: 10, background: C.red, border: "none", color: "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer" } }, "✕"))),
            /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8 } },
              /* @__PURE__ */ React.createElement("label", { style: { flex: 1, padding: "10px 0", borderRadius: 10, background: C.teal + "10", border: "1.5px solid " + C.teal + "30", textAlign: "center", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 } },
                /* @__PURE__ */ React.createElement("span", { style: { fontSize: 18 } }, "📷"),
                /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13, fontWeight: 700, color: C.teal } }, "Camera"),
                /* @__PURE__ */ React.createElement("input", { type: "file", accept: "image/*", capture: "environment", style: { display: "none" }, onChange: (e) => { const f = e.target.files[0]; if (!f) return; const reader = new FileReader(); reader.onload = (ev) => setMedUpdateForm((p) => ({ ...p, pill: { ...p.pill || {}, img: ev.target.result } })); reader.readAsDataURL(f); } })),
              /* @__PURE__ */ React.createElement("label", { style: { flex: 1, padding: "10px 0", borderRadius: 10, background: C.blue + "10", border: "1.5px solid " + C.blue + "30", textAlign: "center", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 } },
                /* @__PURE__ */ React.createElement("span", { style: { fontSize: 18 } }, "📁"),
                /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13, fontWeight: 700, color: C.blue } }, "Upload File"),
                /* @__PURE__ */ React.createElement("input", { type: "file", accept: "image/*", style: { display: "none" }, onChange: (e) => { const f = e.target.files[0]; if (!f) return; const reader = new FileReader(); reader.onload = (ev) => setMedUpdateForm((p) => ({ ...p, pill: { ...p.pill || {}, img: ev.target.result } })); reader.readAsDataURL(f); } })))),
          /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 0" } }, /* @__PURE__ */ React.createElement(Btn, { full: true, color: C.grn, onClick: () => { setMeds((p) => p.map((x) => x.id === muf.id ? { ...x, rx: muf.rx, doctor: muf.doctor, reason: muf.reason, instr: muf.instr, pillColor: muf.pillColor, pill: muf.pill, specialParam: muf.specialParam, specialParamInstr: muf.specialParamInstr, specialParamTimes: muf.specialParamTimes, specialParamVitals: muf.specialParamVitals || [] } : x)); setMedUpdateForm(null); setModal(null); } }, "✓ Save Info Changes"))),
        medUpdateTab === "dosage" && /* @__PURE__ */ React.createElement(React.Fragment, null,
          /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 10px", background: C.org + "08", borderRadius: 10, marginBottom: 10, border: "1px solid " + C.org + "25" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.org } }, "⚠️ Dosage changes require approval"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3, marginTop: 2 } }, "The current medication will be DC'd and a new one created.")),
          /* @__PURE__ */ React.createElement(Inp, { label: "Dose (e.g. 250mg)", value: muf.dose || "", onChange: (v) => setMedUpdateForm((p) => ({ ...p, dose: v })) }),
          /* @__PURE__ */ React.createElement(Inp, { label: "Frequency", value: muf.freq || "", onChange: (v) => setMedUpdateForm((p) => ({ ...p, freq: v })), opts: ["Daily", "2x Daily", "3x Daily", "4x Daily", "Every 4 Hours", "Every 6 Hours", "Every 8 Hours", "Every 12 Hours", "PRN", "Weekly", "Monthly"] }),
          /* @__PURE__ */ React.createElement(Inp, { label: "Admin Times (comma separated)", value: typeof muf.times === "string" ? muf.times : (muf.times || []).join(", "), onChange: (v) => setMedUpdateForm((p) => ({ ...p, times: v })), ph: "08:00, 20:00" }),
          /* @__PURE__ */ React.createElement(Inp, { label: "Tabs Per Dose", value: String(muf.tabsPerDose || 1), onChange: (v) => setMedUpdateForm((p) => ({ ...p, tabsPerDose: parseInt(v) || 1 })) }),
          /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 0" } }, /* @__PURE__ */ React.createElement(Btn, { full: true, color: isDosageChange ? C.org : C.tx3, disabled: !isDosageChange, onClick: () => { if (isDosageChange) setMedUpdateConfirm(true); } }, isDosageChange ? "⚠️ Review & Approve Changes" : "No Dosage Changes Made"))),
        medUpdateTab === "pills" && /* @__PURE__ */ React.createElement(React.Fragment, null,
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3, marginBottom: 10 } }, "Set pill count and start date. The system will calculate daily countdown and auto-generate next fill date."),
          /* @__PURE__ */ React.createElement(Inp, { label: "Start Date", value: muf.startDate || "", onChange: (v) => setMedUpdateForm((p) => ({ ...p, startDate: v })), type: "date" }),
          /* @__PURE__ */ React.createElement(Inp, { label: "Pill Count (per fill)", value: String(muf.pillCount || ""), onChange: (v) => setMedUpdateForm((p) => ({ ...p, pillCount: parseInt(v) || 0 })) }),
          muf.pillCount && muf.startDate && /* @__PURE__ */ React.createElement(React.Fragment, null, (() => { const dosesPerDay = (typeof muf.times === "string" ? muf.times.split(",").filter(Boolean) : muf.times || []).length || 1; const tabsPerDay = (muf.tabsPerDose || 1) * dosesPerDay; const daysSupply = tabsPerDay > 0 ? Math.floor((muf.pillCount || 0) / tabsPerDay) : 0; const nextDate = new Date(muf.startDate); nextDate.setDate(nextDate.getDate() + daysSupply); return /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 12px", background: C.blue + "06", borderRadius: 10, marginBottom: 8, border: "1px solid " + C.blue + "20" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.blue, marginBottom: 4 } }, "📅 Auto-calculated"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: C.tx2 } }, "Days supply: " + daysSupply + " days"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: C.tx2 } }, "Next fill date: " + nextDate.toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3, marginTop: 4 } }, "Based on " + (muf.tabsPerDose || 1) + " tab(s) × " + dosesPerDay + " dose(s)/day = " + tabsPerDay + " tabs/day")); })(),
          /* @__PURE__ */ React.createElement(Inp, { label: "Next Cycle Pill Count", value: String(muf.nextCyclePillCount || ""), onChange: (v) => setMedUpdateForm((p) => ({ ...p, nextCyclePillCount: parseInt(v) || 0 })), ph: "Enter count for next fill" }),
          /* @__PURE__ */ React.createElement("div", { style: { padding: "6px 0" } },
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3, fontWeight: 600, marginBottom: 4 } }, "EXPIRATION DATE"),
            /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "center" } },
              /* @__PURE__ */ React.createElement("input", { type: "date", value: muf.expiryDate || "", onChange: (e) => setMedUpdateForm((p) => ({ ...p, expiryDate: e.target.value })), style: { flex: 1, padding: "8px", borderRadius: 8, border: "1px solid " + C.sepL, fontSize: 12, fontFamily: C.font } }),
              muf.startDate && /* @__PURE__ */ React.createElement("button", { onClick: () => { const d = new Date(muf.startDate); d.setFullYear(d.getFullYear() + 1); setMedUpdateForm((p) => ({ ...p, expiryDate: d.toISOString().slice(0, 10) })); }, style: { padding: "8px 10px", borderRadius: 8, border: "1.5px solid " + C.blue + "40", background: C.blue + "08", color: C.blue, fontSize: 10, fontWeight: 700, cursor: "pointer", fontFamily: C.font, whiteSpace: "nowrap" } }, "Auto +1 Year")))),
          /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 0" } }, /* @__PURE__ */ React.createElement(Btn, { full: true, color: C.grn, onClick: () => { setMeds((p) => p.map((x) => x.id === muf.id ? { ...x, startDate: muf.startDate, pillCount: muf.pillCount, expiryDate: muf.expiryDate || x.expiryDate || "", nextCyclePillCount: muf.nextCyclePillCount, nextCycleStartDate: muf.nextCycleStartDate } : x)); setMedUpdateForm(null); setModal(null); } }, "✓ Save Pill Count")))));
    }
    if (modal === "bowel") {
    ...
    if (modal === "addStaff") {
      ...
      }, style: { padding: "8px 12px", borderRadius: 10, border: "1px solid " + C.org, background: C.org + "08", color: C.org, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: C.font } }, "Generate")), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.org, marginTop: 3 } }, "Give this temp password to the staff member. They will create their own on first login.")), /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px 6px" } }, /* @__PURE__ */ React.createElement("label", { style: { fontSize: 12, color: C.tx3, fontWeight: 500, display: "block", marginBottom: 4 } }, "Homes *"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 6 } }, HOMES.map((h) => {
        const on = nStaff.homes.includes(h.id);
        return /* @__PURE__ */ React.createElement("button", { key: h.id, onClick: () => setNStaff((p) => ({ ...p, homes: on ? p.homes.filter((x) => x !== h.id) : [...p.homes, h.id] })), style: { display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderRadius: 10, border: "2px solid " + (on ? h.color : C.sepL), background: on ? h.color + "10" : "transparent", color: on ? h.color : C.tx3, fontSize: 13, fontWeight: on ? 700 : 500, cursor: "pointer", fontFamily: C.font, textAlign: "left" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 18, lineHeight: 1, color: on ? h.color : C.sepL } }, on ? "\u2611" : "\u2610"), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: on ? 700 : 500 } }, h.full), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3, fontWeight: 400 } }, h.name)));
    ...
    if (modal === "editStaff" && editStaff) {
      ...
      } }, "Reset Password"))), /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px 6px" } }, /* @__PURE__ */ React.createElement("label", { style: { fontSize: 12, color: C.tx3, fontWeight: 500, display: "block", marginBottom: 4 } }, "Homes"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 6 } }, HOMES.map((h) => {
        const on = (es.homes || []).includes(h.id);
        return /* @__PURE__ */ React.createElement("button", { key: h.id, onClick: () => setEditStaff((p) => ({ ...p, homes: on ? (p.homes || []).filter((x) => x !== h.id) : [...p.homes || [], h.id] })), style: { display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderRadius: 10, border: "2px solid " + (on ? h.color : C.sepL), background: on ? h.color + "10" : "transparent", color: on ? h.color : C.tx3, fontSize: 13, fontWeight: on ? 700 : 500, cursor: "pointer", fontFamily: C.font, textAlign: "left" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 18, lineHeight: 1, color: on ? h.color : C.sepL } }, on ? "\u2611" : "\u2610"), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: on ? 700 : 500 } }, h.full), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3, fontWeight: 400 } }, h.name)));
    ...
    if (modal === "editRes" && editRes) {
      ...
      return /* @__PURE__ */ React.createElement(Modal, { title: "Edit: " + fullName(er.name), onClose: () => {
        ...
        setEditRes(null);
        setModal(null);
      } }, "Save Changes"), /* @__PURE__ */ React.createElement(Btn, { full: true, v: "outline", color: "#ff9500", onClick: () => {
        const isArch = er.archived;
        if (isArch) {
          setResidents((p) => p.map((r) => r.id === er.id ? { ...r, archived: false, archiveDate: null, archiveReason: null, archivedBy: null } : r));
          setEditRes(null); setModal(null);
        } else {
          const reason = prompt("Reason for archiving " + fullName(er.name) + ":\n(e.g. Discharged, Transferred, Deceased, Moved to other facility)");
          if (reason !== null) {
            setResidents((p) => p.map((r) => r.id === er.id ? { ...r, archived: true, archiveDate: new Date().toISOString().split("T")[0], archiveReason: reason || "Discharged", archivedBy: user ? user.id : "sa1" } : r));
            setEditRes(null); setModal(null);
          }
        }
      } }, er.archived ? "\u2705 Unarchive Resident" : "\u{1F4E6} Archive Resident"), /* @__PURE__ */ React.createElement(Btn, { full: true, v: "outline", color: C.red, onClick: () => {
        const n = fullName(er.name);
        if (!confirm("\u26A0\uFE0F WARNING \u26A0\uFE0F\n\nYou are about to PERMANENTLY DELETE:\n" + n + "\n\nThis will erase ALL records including medications, MAR history, behavior logs, incident reports, vitals, and appointments.\n\nThis action CANNOT be undone.\n\nAre you sure you want to continue?")) return;
        if (!confirm("\u{1F6AB} SECOND CONFIRMATION \u{1F6AB}\n\nYou are about to permanently delete " + n + " and ALL associated data.\n\nHave you considered using Archive instead? Archiving preserves all records and allows restoration later.\n\nDo you still want to PERMANENTLY DELETE " + n + "?")) return;
        const typed = prompt("\u{1F512} FINAL CONFIRMATION\n\nType the resident's full name exactly as shown below to confirm permanent deletion:\n\n" + n);
        if (typed !== n) { alert("Name did not match. Deletion cancelled."); return; }
        setResidents((p) => p.filter((r) => r.id !== er.id));
        setEditRes(null); setModal(null);
        alert(n + " has been permanently deleted.");
      } }, "Permanently Delete")))));
    }
    if (modal && modal.startsWith("sk_")) {
      const sk = (SKILLS_BY_RES[selRes] || [])[parseInt(modal.split("_")[1])];
      if (!sk) return null;
      return /* @__PURE__ */ React.createElement(Modal, { title: sk.name, onClose: () => { setOpenPromptKey(null); setModal(null); } }, /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px", paddingTop: 6 } }, /* @__PURE__ */ React.createElement(Card, null, sk.steps.map((st, i) => {
        const key = sk.name + "_" + i;
        const val = skillPrompts[key] || "Independent";
        const isOpen = openPromptKey === key;
        const promptColor = val === "Independent" ? C.grn : val === "Indirect Verbal" || val === "Verbal" ? C.blue : val === "Gesture" || val === "Visual" || val === "Positional" ? C.org : val === "Model" || val === "Partial Physical" ? "#ff9500" : val === "Full Physical" ? C.red : val === "Incomplete" ? C.red : C.tx3;
        return /* @__PURE__ */ React.createElement("div", { key: i, style: { borderBottom: i < sk.steps.length - 1 ? "0.5px solid " + C.sepL : "none" } },
          /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, padding: "10px 12px" } },
            /* @__PURE__ */ React.createElement("div", { style: { width: 22, height: 22, borderRadius: 11, background: promptColor + "20", border: "1.5px solid " + promptColor, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700, color: promptColor, flexShrink: 0 } }, i + 1),
            /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13, flex: 1 } }, st),
            /* @__PURE__ */ React.createElement("button", { onClick: () => setOpenPromptKey(isOpen ? null : key), style: { padding: "6px 10px", borderRadius: 8, border: "2px solid " + promptColor, background: promptColor + "15", color: promptColor, fontWeight: 700, fontSize: 12, cursor: "pointer", fontFamily: C.font, display: "flex", alignItems: "center", gap: 4, whiteSpace: "nowrap" } },
              val, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 9 } }, isOpen ? "\u25B2" : "\u25BC")
            )
          ),
          isOpen && /* @__PURE__ */ React.createElement("div", { style: { padding: "6px 10px 10px 10px", background: C.alt, display: "flex", flexWrap: "wrap", gap: 6 } },
            PROMPTS.map((pr) => {
              const pc = pr === "Independent" ? C.grn : pr === "Indirect Verbal" || pr === "Verbal" ? C.blue : pr === "Gesture" || pr === "Visual" || pr === "Positional" ? C.org : pr === "Model" || pr === "Partial Physical" ? "#ff9500" : pr === "Full Physical" ? C.red : pr === "Incomplete" ? C.red : C.tx3;
              return /* @__PURE__ */ React.createElement("button", { key: pr, onClick: () => { setSkillPrompts((p) => ({ ...p, [key]: pr })); setOpenPromptKey(null); }, style: { padding: "7px 12px", borderRadius: 8, border: "2px solid " + (val === pr ? pc : pc + "40"), background: val === pr ? pc : pc + "15", color: val === pr ? "#fff" : pc, fontWeight: 700, fontSize: 12, cursor: "pointer", fontFamily: C.font } }, pr);
            })
          )
        );
      })), /* @__PURE__ */ React.createElement("div", { style: { padding: "12px 0 4px" } }, /* @__PURE__ */ React.createElement(Btn, { full: true, color: C.teal, onClick: () => {
        const session = { id: "ss" + Date.now(), resId: selRes, skillName: sk.name, d: today, t: new Date().toLocaleTimeString([], { hour: "numeric", minute: "2-digit" }), by: user.id, prompts: sk.steps.map((_, i) => skillPrompts[sk.name + "_" + i] || "Independent") };
        setSkillSessions((p) => [...p, session]);
        const keys = {};
        sk.steps.forEach((_, i) => { keys[sk.name + "_" + i] = "Independent"; });
    ...
    if (modal === "memos") {
      return /* @__PURE__ */ React.createElement(Modal, { title: "Memos", onClose: () => setModal(null), wide: true }, /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px", paddingTop: 8 } }, memos.map((m, i) => {
        ...
        const isHealthAlert = m.text && (m.text.includes("HEALTH ALERT") || m.text.includes("ER") || m.text.includes("Emergency Room"));
        return /* @__PURE__ */ React.createElement(Card, { key: m.id, style: { marginBottom: 10, border: isHealthAlert && !acked ? "2.5px solid " + C.red : !acked ? "1.5px solid " + C.red + "40" : "1px solid " + C.sepL, overflow: "hidden", borderRadius: 14, boxShadow: isHealthAlert && !acked ? "0 4px 20px " + C.red + "30" : "none" } },
          isHealthAlert && !acked && /* @__PURE__ */ React.createElement("div", { style: { background: C.red, padding: "8px 14px", display: "flex", alignItems: "center", gap: 8 } },
            /* @__PURE__ */ React.createElement("div", { style: { width: 10, height: 10, borderRadius: 5, background: "#fff", animation: "memoFlash 0.6s ease-in-out infinite alternate", flexShrink: 0 } }),
            /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13, fontWeight: 800, color: "#fff", letterSpacing: 0.5, textTransform: "uppercase" } }, "\u{1F6A8} HEALTH ALERT \u2014 ACTION REQUIRED"),
            /* @__PURE__ */ React.createElement("div", { style: { width: 10, height: 10, borderRadius: 5, background: "#fff", animation: "memoFlash 0.6s ease-in-out infinite alternate", flexShrink: 0 } })
          ),
          /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 12px", background: isHealthAlert && !acked ? C.red + "06" : "transparent" } },
            !acked && !isHealthAlert && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 4, marginBottom: 6 } }, /* @__PURE__ */ React.createElement("div", { style: { width: 8, height: 8, borderRadius: 4, background: C.red, animation: "memoFlash 1s ease-in-out infinite alternate" } }), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, fontWeight: 700, color: C.red } }, "REQUIRES ACKNOWLEDGMENT")),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: isHealthAlert ? 13 : 14, color: isHealthAlert && !acked ? C.tx : C.tx, lineHeight: 1.6, marginBottom: 6, whiteSpace: "pre-wrap", fontWeight: isHealthAlert && !acked ? 500 : 400 } }, m.text), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3, marginBottom: 6 } }, "Posted ", m.d, " by ", gn(m.by)), m.acks.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { marginBottom: 4 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: C.tx3, fontWeight: 600 } }, "Acknowledged (", m.acks.length, "): "), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: C.grn } }, m.acks.map((a) => staffInit(gn(a))).join(", "))), (isAM || isS) && (() => {
    ...
    if (modal === "newMemo") {
      ...
      const MEMO_TYPES = [{ id: "medication", label: "Medication", icon: "\u{1F48A}", color: C.grn }, { id: "health", label: "Health", icon: "\u{1FA7A}", color: C.red }, { id: "behavioral", label: "Behavioral", icon: "\u{1F9E0}", color: C.org }, { id: "staff", label: "Staff", icon: "\u{1F465}", color: C.blue }, { id: "house", label: "House", icon: "\u{1F3E0}", color: C.teal }, { id: "resident", label: "Resident", icon: "\u{1F464}", color: C.pur }, { id: "appointment", label: "Appointment", icon: "\u{1F4C5}", color: "#5856d6" }, { id: "other", label: "Other", icon: "\u{1F4CC}", color: C.tx3 }];
      return /* @__PURE__ */ React.createElement(Modal, { title: "New Memo", onClose: () => {
        setMemoForm({ name: "", text: "", type: "other", homeIds: [], resId: "", urgent: true });
        setModal(null);
      }, wide: true }, /* @__PURE__ */ React.createElement("div", { style: { paddingTop: 6 } }, /* @__PURE__ */ React.createElement(Inp, { label: "Memo Title *", value: memoForm.name, onChange: (v) => setMemoForm((p) => ({ ...p, name: v })), ph: "Brief title for this memo (e.g. Ibuprofen Added - Sarah M.)" }), /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px", marginBottom: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tx3, marginBottom: 4 } }, "RESIDENT / PARTICIPANT (optional)"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 4 } }, /* @__PURE__ */ React.createElement("button", { onClick: () => setMemoForm((p) => ({ ...p, resId: "" })), style: { padding: "6px 12px", borderRadius: 8, border: "1.5px solid " + (!memoForm.resId ? C.blue : C.sepL), background: !memoForm.resId ? C.blue + "12" : "transparent", color: !memoForm.resId ? C.blue : C.tx3, fontSize: 11, fontWeight: !memoForm.resId ? 700 : 500, cursor: "pointer", fontFamily: C.font } }, "General / All"), residents.filter((r) => !r.archived && (isA || (user.homes || []).some((h) => r.home === h))).map((r) => /* @__PURE__ */ React.createElement("button", { key: r.id, onClick: () => setMemoForm((p) => ({ ...p, resId: r.id })), style: { padding: "6px 12px", borderRadius: 8, border: "1.5px solid " + (memoForm.resId === r.id ? C.pur : C.sepL), background: memoForm.resId === r.id ? C.pur + "12" : "transparent", color: memoForm.resId === r.id ? C.pur : C.tx3, fontSize: 11, fontWeight: memoForm.resId === r.id ? 700 : 500, cursor: "pointer", fontFamily: C.font } }, r.first || fullName(r.name))))), /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px", marginBottom: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tx3, marginBottom: 4 } }, "MEMO TYPE"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 4 } }, MEMO_TYPES.map((mt) => /* @__PURE__ */ React.createElement("button", { key: mt.id, onClick: () => setMemoForm((p) => ({ ...p, type: mt.id })), style: { padding: "6px 12px", borderRadius: 8, border: "1.5px solid " + (memoForm.type === mt.id ? mt.color : C.sepL), background: memoForm.type === mt.id ? mt.color + "12" : "transparent", color: memoForm.type === mt.id ? mt.color : C.tx3, fontSize: 11, fontWeight: memoForm.type === mt.id ? 700 : 500, cursor: "pointer", fontFamily: C.font } }, mt.icon, " ", mt.label)))), /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px", marginBottom: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tx3, marginBottom: 4 } }, "SEND TO"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 4 } }, /* @__PURE__ */ React.createElement("button", { onClick: () => setMemoForm((p) => ({ ...p, homeIds: p.homeIds.length === memoHomes.length ? [] : memoHomes.map((h) => h.id) })), style: { padding: "6px 12px", borderRadius: 8, border: "1.5px solid " + (memoForm.homeIds.length === memoHomes.length ? C.blue : C.sepL), background: memoForm.homeIds.length === memoHomes.length ? C.blue + "12" : "transparent", color: memoForm.homeIds.length === memoHomes.length ? C.blue : C.tx3, fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: C.font } }, "All Homes"), memoHomes.map((h) => {
        ...
        const memoNumStr = "MEMO-" + today.slice(0,4) + "-" + String(memos.length + 1).padStart(3,"0");
        const memoTitle = memoForm.name.trim() || (typeIcon + " " + memoForm.text.trim().slice(0,60));
        setMemos((p) => [{ id: "mm" + Date.now(), memoNum: memoNumStr, name: memoTitle, text: memoTitle + "\n\n" + memoForm.text.trim(), by: user.id, d: today, t: (/* @__PURE__ */ new Date()).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" }), acks: [user.id], urgent: memoForm.urgent, memoType: memoForm.type, homeIds: targetHomes, resId: memoForm.resId || null, ackRecords: [] }, ...p]);
    ...
    if (modal === "erVisit" && erVisit) {
      ...
      const NOTIF_OPTS = ["Manager/Supervisor", "Emergency Contact", "Conservator", "Regional Center Coordinator", "Primary Care Physician", "On-Call Nurse", "Other"];
      const setEv = (fn) => setErVisit(typeof fn === "function" ? fn(erVisit) : fn);
      const toggleNotif = (v) => setEv((p) => ({ ...p, notified: p.notified.includes(v) ? p.notified.filter((x) => x !== v) : [...p.notified, v] }));
      const saveVisit = () => {
        const timeDisp = ev.time || "";
        const visitLabel = ev.visitType + " \u2014 " + (res ? fullName(res.name) : "?");
        const note = "\uD83D\uDEA8 " + visitLabel + "\nDate: " + ev.date + (timeDisp ? " | Time: " + timeDisp : "") + "\nLocation: " + ev.location + "\nReason: " + ev.reason + (ev.transport ? "\nTransport: " + ev.transport : "") + (ev.companionedBy ? "\nAccompanied by: " + ev.companionedBy : "") + (ev.diagnosis ? "\nDiagnosis: " + ev.diagnosis : "") + (ev.treatment ? "\nTreatment: " + ev.treatment : "") + (ev.newMeds ? "\nNew Medications: " + ev.newMeds : "") + "\nAdmitted: " + ev.admitted + (ev.discharged ? " | Discharged: " + ev.discharged : "") + (ev.followUp ? "\nFollow-Up Instructions: " + ev.followUp : "") + (ev.followUpAppt ? "\nFollow-Up Appt: " + ev.followUpAppt : "") + (ev.restrictions ? "\nRestrictions: " + ev.restrictions : "") + "\nNotified: " + (ev.notified.join(", ") || "None documented") + (ev.staffNotes ? "\nNotes: " + ev.staffNotes : "") + "\nDocumented by: " + gn(user.id);
        if (res) setResidents((p) => p.map((r) => r.id === res.id ? { ...r, medChanges: [{ d: ev.date, type: ev.visitType, note: ev.reason + (ev.diagnosis ? ". Dx: " + ev.diagnosis : "") + (ev.treatment ? ". Tx: " + ev.treatment : "") + ".", sev: "high" }, ...r.medChanges] } : r));
        setDocs((p) => [{ id: "d" + Date.now(), resId: ev.resId, type: "Activity", d: ev.date, t: timeDisp, user: gn(user.id), text: note, by: user.id }, ...p]);
        const savedEv = { ...ev, note };
        setModal(null);
        setErReportPrompt({ ev: savedEv, note, res });
        setTimeout(() => setModal("erReportChoice"), 100);
      };
      return /* @__PURE__ */ React.createElement(Modal, { title: ev.visitType === "ER Visit" ? "\uD83D\uDEA8 ER Visit" : "\uD83C\uDFE5 Urgent Care Visit", onClose: () => setModal(null), wide: true }, /* @__PURE__ */ React.createElement("div", { style: { paddingTop: 6 } }, /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px 10px" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, fontWeight: 700, color: C.tx3, marginBottom: 6 } }, "VISIT TYPE"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8 } }, ["ER Visit", "Urgent Care Visit"].map((vt) => /* @__PURE__ */ React.createElement("button", { key: vt, onClick: () => setEv((p) => ({ ...p, visitType: vt })), style: { flex: 1, padding: "12px 8px", borderRadius: 12, border: "2px solid " + (ev.visitType === vt ? (vt === "ER Visit" ? C.red : C.org) : C.sepL), background: ev.visitType === vt ? (vt === "ER Visit" ? C.red : C.org) + "12" : C.card, cursor: "pointer", fontFamily: C.font } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 22, marginBottom: 4 } }, vt === "ER Visit" ? "\uD83D\uDEA8" : "\uD83C\uDFE5"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 700, color: ev.visitType === vt ? (vt === "ER Visit" ? C.red : C.org) : C.tx } }, vt))))), res && /* @__PURE__ */ React.createElement("div", { style: { margin: "0 14px 10px", padding: 10, background: C.alt, borderRadius: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tx3, marginBottom: 2 } }, "CLIENT"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 700 } }, fullName(res.name)), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3 } }, "DOB: " + res.dob + " | UCI: " + (res.uci || "\u2014"))), /* @__PURE__ */ React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, padding: "0 14px 10px" } }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 600, color: C.tx3, marginBottom: 4 } }, "DATE \u23F1 Auto-stamped"), /* @__PURE__ */ React.createElement("input", { type: "date", value: ev.date, onChange: (e) => setEv((p) => ({ ...p, date: e.target.value })), style: { width: "100%", padding: "9px 10px", borderRadius: 10, border: "1.5px solid " + C.grn + "60", fontSize: 13, fontFamily: C.font, boxSizing: "border-box", background: C.grn + "06" } })), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 600, color: C.tx3, marginBottom: 4 } }, "TIME \u23F1 Auto-stamped"), /* @__PURE__ */ React.createElement("input", { type: "time", value: ev.time, onChange: (e) => setEv((p) => ({ ...p, time: e.target.value })), style: { width: "100%", padding: "9px 10px", borderRadius: 10, border: "1.5px solid " + C.grn + "60", fontSize: 13, fontFamily: C.font, boxSizing: "border-box", background: C.grn + "06" } }))), /* @__PURE__ */ React.createElement(Inp, { label: "Location / Facility *", value: ev.location, onChange: (v) => setEv((p) => ({ ...p, location: v })), ph: "e.g., Doctors Medical Center, Modesto" }), /* @__PURE__ */ React.createElement(Inp, { label: "Reason for Visit *", value: ev.reason, onChange: (v) => setEv((p) => ({ ...p, reason: v })), ph: "e.g., Fall, chest pain, seizure..." }), /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px 10px" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 600, color: C.tx3, marginBottom: 6 } }, "MODE OF TRANSPORT"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 6 } }, ["Ambulance (911)", "Private Vehicle", "Facility Van", "Rideshare/Taxi", "Other"].map((t) => /* @__PURE__ */ React.createElement("button", { key: t, onClick: () => setEv((p) => ({ ...p, transport: t })), style: { padding: "6px 12px", borderRadius: 8, border: "1.5px solid " + (ev.transport === t ? C.blue : C.sepL), background: ev.transport === t ? C.blue + "10" : C.card, color: ev.transport === t ? C.blue : C.tx, fontSize: 12, fontWeight: ev.transport === t ? 700 : 500, cursor: "pointer", fontFamily: C.font } }, t)))), /* @__PURE__ */ React.createElement(Inp, { label: "Staff Who Accompanied Client", value: ev.companionedBy, onChange: (v) => setEv((p) => ({ ...p, companionedBy: v })), ph: "Staff name" }), /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px 10px" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tx3, marginBottom: 6 } }, "WHO WAS NOTIFIED? \u2714"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 6 } }, NOTIF_OPTS.map((n) => /* @__PURE__ */ React.createElement("button", { key: n, onClick: () => toggleNotif(n), style: { display: "flex", alignItems: "center", gap: 10, padding: "9px 12px", borderRadius: 10, border: "1.5px solid " + (ev.notified.includes(n) ? C.grn : C.sepL), background: ev.notified.includes(n) ? C.grn + "10" : C.card, cursor: "pointer", fontFamily: C.font, textAlign: "left" } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 16, color: ev.notified.includes(n) ? C.grn : C.sepL } }, ev.notified.includes(n) ? "\u2611" : "\u2610"), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13, fontWeight: ev.notified.includes(n) ? 700 : 500, color: ev.notified.includes(n) ? C.grn : C.tx } }, n))))), /* @__PURE__ */ React.createElement("div", { style: { padding: "6px 14px 10px", fontSize: 12, fontWeight: 700, color: C.blue, borderTop: "0.5px solid " + C.sepL } }, "VISIT OUTCOME"), /* @__PURE__ */ React.createElement(Inp, { label: "Diagnosis / Findings", value: ev.diagnosis, onChange: (v) => setEv((p) => ({ ...p, diagnosis: v })), ph: "e.g., Grade II sprained ankle, no fracture" }), /* @__PURE__ */ React.createElement(Inp, { label: "Treatment Provided", value: ev.treatment, onChange: (v) => setEv((p) => ({ ...p, treatment: v })), ph: "e.g., X-ray, wound care, IV fluids..." }), /* @__PURE__ */ React.createElement(Inp, { label: "New Medications Prescribed", value: ev.newMeds, onChange: (v) => setEv((p) => ({ ...p, newMeds: v })), ph: "e.g., Ibuprofen 400mg PRN, or None" }), /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px 10px" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 600, color: C.tx3, marginBottom: 6 } }, "CLIENT ADMITTED TO HOSPITAL?"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8 } }, ["No", "Yes"].map((v) => /* @__PURE__ */ React.createElement("button", { key: v, onClick: () => setEv((p) => ({ ...p, admitted: v })), style: { flex: 1, padding: "10px", borderRadius: 10, border: "2px solid " + (ev.admitted === v ? (v === "Yes" ? C.red : C.grn) : C.sepL), background: ev.admitted === v ? (v === "Yes" ? C.red : C.grn) + "12" : C.card, color: ev.admitted === v ? (v === "Yes" ? C.red : C.grn) : C.tx, fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: C.font } }, v))), ev.admitted === "Yes" && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 6, padding: "8px 10px", background: C.red + "08", borderRadius: 8, fontSize: 12, color: C.red, fontWeight: 600 } }, "\uD83D\uDEA8 Hospital admission requires a Special Incident Report (SIR)")), ev.admitted === "No" && /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px 10px" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 600, color: C.tx3, marginBottom: 4 } }, "DISCHARGE TIME"), /* @__PURE__ */ React.createElement("input", { type: "time", value: ev.discharged, onChange: (e) => setEv((p) => ({ ...p, discharged: e.target.value })), style: { width: "100%", padding: "9px 10px", borderRadius: 10, border: "1px solid " + C.sepL, fontSize: 13, fontFamily: C.font, boxSizing: "border-box" } })), /* @__PURE__ */ React.createElement(Inp, { label: "Follow-Up Instructions", value: ev.followUp, onChange: (v) => setEv((p) => ({ ...p, followUp: v })), ph: "e.g., Ice 20 min every 2 hrs, keep elevated..." }), /* @__PURE__ */ React.createElement(Inp, { label: "Follow-Up Appointment", value: ev.followUpAppt, onChange: (v) => setEv((p) => ({ ...p, followUpAppt: v })), ph: "e.g., PCP on 03/20/2026, or N/A" }), /* @__PURE__ */ React.createElement(Inp, { label: "Activity Restrictions", value: ev.restrictions, onChange: (v) => setEv((p) => ({ ...p, restrictions: v })), ph: "e.g., No community outings today, limited walking..." }), /* @__PURE__ */ React.createElement(Inp, { label: "Additional Notes", value: ev.staffNotes, onChange: (v) => setEv((p) => ({ ...p, staffNotes: v })), ph: "Any other observations..." }), /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 14px 4px" } }, /* @__PURE__ */ React.createElement(Btn, { full: true, color: ev.visitType === "ER Visit" ? C.red : C.org, onClick: saveVisit, disabled: !ev.location || !ev.reason }, "\u2705 Save Visit & File Report"))));
    }
    if (modal === "erReportChoice" && erReportPrompt) {
      ...
      return /* @__PURE__ */ React.createElement(Modal, { title: "File Incident Report", onClose: () => { setErReportPrompt(null); setModal(null); }, wide: true }, /* @__PURE__ */ React.createElement("div", { style: { padding: "14px 14px 6px" } }, /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 12px", background: C.grn + "10", borderRadius: 10, border: "1px solid " + C.grn + "30", marginBottom: 14 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, fontWeight: 700, color: C.grn } }, "\u2705 Visit logged \u2014 added to medical changes & activity log"), res && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3, marginTop: 2 } }, fullName(res.name) + " | " + ev.date + " | " + ev.visitType)), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 700, color: C.tx, marginBottom: 8 } }, "Which incident report form is needed?"), isHospAdmit && /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 10px", background: C.red + "08", borderRadius: 8, marginBottom: 10, fontSize: 12, color: C.red, fontWeight: 600 } }, "\uD83D\uDEA8 Hospital admission \u2014 a Special Incident Report (SIR) is required"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 10, marginBottom: 14 } }, [{ type: "sir", label: "\uD83D\uDEA8 Special Incident Report (SIR)", desc: "Required for: hospital admission, serious injury, abuse/neglect, missing person, restraint, death. Submitted to Regional Center within 24 hrs.", color: C.red }, { type: "shared", label: "\uD83D\uDCCB Shared Information Report", desc: "For: ER visits without admission, urgent care, minor injuries, behavioral incidents. Shared with team and Regional Center.", color: C.org }].map(({ type, label, desc, color }) => /* @__PURE__ */ React.createElement("button", { key: type, onClick: () => { const d = mkIrDraft(); d.resId = ev.resId; d.clientName = res ? fullName(res.name) : ""; d.uci = res ? (res.uci || "") : ""; d.dob = res ? (res.dob || "") : ""; d.formType = type; d.dateOcc = ev.date; d.timeOcc = ev.time || ""; d.locType = "Community Care Facility"; d.address = ev.location; d.medTx = true; if (type === "sir") d.incTypes = ["Hospital Admission"]; d.desc = note; d.narrative = note; setIrDraft(d); setIrStep(1); setErReportPrompt(null); setModal(null); setTimeout(() => { setResSubTab("dash"); setTab("home"); }, 50); }, style: { padding: "14px", borderRadius: 12, border: "2px solid " + (suggestedType === type ? color : C.sepL), background: suggestedType === type ? color + "08" : C.card, cursor: "pointer", fontFamily: C.font, textAlign: "left" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 700, color: suggestedType === type ? color : C.tx, marginBottom: 4, display: "flex", alignItems: "center", gap: 6 } }, label, suggestedType === type && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, background: color, color: "#fff", borderRadius: 4, padding: "2px 6px" } }, "RECOMMENDED")), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: C.tx3, lineHeight: 1.4 } }, desc)))), /* @__PURE__ */ React.createElement(Btn, { full: true, v: "outline", color: C.tx3, onClick: () => { setErReportPrompt(null); setModal(null); } }, "Skip \u2014 I\u2019ll File Later")));
    }
    if (modal === "aptEdit" && viewApt) {
      ...
      const specialists = aptDoctors.filter((d) => d.specialty && d.specialty !== "PCP");
      return /* @__PURE__ */ React.createElement(Modal, { title: "Edit Appointment", onClose: () => setModal("aptView"), wide: true }, /* @__PURE__ */ React.createElement("div", { style: { paddingTop: 6 } },
        /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px 8px" } },
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tx3, marginBottom: 6 } }, "APPOINTMENT TYPE"),
          /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 5 } },
            ["Specialist", "Dental", "Behavioral", "Lab", "Primary Care", "Other"].map((t) => {
              const tc = t === "Specialist" ? C.pur : t === "Dental" ? C.teal : t === "Behavioral" ? C.org : C.blue;
              return /* @__PURE__ */ React.createElement("button", { key: t, onClick: () => setEa((p) => ({ ...p, type: t })), style: { padding: "6px 12px", borderRadius: 8, border: "2px solid " + (ea.type === t ? tc : C.sepL), background: ea.type === t ? tc + "12" : C.card, color: ea.type === t ? tc : C.tx3, fontSize: 12, fontWeight: ea.type === t ? 700 : 500, cursor: "pointer", fontFamily: C.font } }, t);
            })
          )
        ),
        ea.type === "Specialist" && specialists.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { margin: "0 14px 10px", padding: "10px 12px", background: C.pur + "06", borderRadius: 10, border: "1px solid " + C.pur + "20" } },
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.pur, marginBottom: 6 } }, "\uD83E\uDE7A SELECT SPECIALIST" + (aptRes ? " \u2014 " + aptRes.first + "\u2019s providers" : "")),
          /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 5 } },
            specialists.map((doc, di) => /* @__PURE__ */ React.createElement("button", { key: di, onClick: () => setEa((p) => ({ ...p, doctor: doc.name, location: doc.location || p.location, visitFreq: doc.visitFreq || p.visitFreq, title: doc.specialty + " Visit" })), style: { display: "flex", alignItems: "center", gap: 10, padding: "8px 10px", borderRadius: 9, border: "1.5px solid " + (ea.doctor === doc.name ? C.pur : C.sepL), background: ea.doctor === doc.name ? C.pur + "12" : C.card, cursor: "pointer", fontFamily: C.font, textAlign: "left" } },
              /* @__PURE__ */ React.createElement("div", { style: { width: 34, height: 34, borderRadius: 8, background: C.pur + "15", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, flexShrink: 0 } }, "\uD83E\uDE7A"),
              /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } },
                /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 700, color: ea.doctor === doc.name ? C.pur : C.tx } }, doc.name),
                /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3 } }, doc.specialty + (doc.location ? " \u2022 " + doc.location : "") + (doc.visitFreq ? " \u2022 " + doc.visitFreq : ""))
              ),
              ea.doctor === doc.name && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 14, color: C.pur } }, "\u2713")
            ))
          )
        ),
        /* @__PURE__ */ React.createElement(Inp, { label: "Title", value: ea.title, onChange: (v) => setEa((p) => ({ ...p, title: v })) }),
        /* @__PURE__ */ React.createElement(Inp, { label: "Doctor / Provider", value: ea.doctor, onChange: (v) => setEa((p) => ({ ...p, doctor: v })) }),
        /* @__PURE__ */ React.createElement(Inp, { label: "Location", value: ea.location, onChange: (v) => setEa((p) => ({ ...p, location: v })) }),
        /* @__PURE__ */ React.createElement(Inp, { label: "Date", value: ea.date, onChange: (v) => setEa((p) => ({ ...p, date: v })), type: "date" }),
        /* @__PURE__ */ React.createElement(Inp, { label: "Time", value: ea.time, onChange: (v) => setEa((p) => ({ ...p, time: v })), type: "time" }),
        /* @__PURE__ */ React.createElement(Inp, { label: "Duration", value: ea.duration, onChange: (v) => setEa((p) => ({ ...p, duration: v })) }),
        /* @__PURE__ */ React.createElement(Inp, { label: "Status", value: ea.status, onChange: (v) => setEa((p) => ({ ...p, status: v })), opts: ["scheduled", "confirmed", "cancelled"] }),
        /* @__PURE__ */ React.createElement(Inp, { label: "Notes", value: ea.notes, onChange: (v) => setEa((p) => ({ ...p, notes: v })), multi: true }),
        /* @__PURE__ */ React.createElement(Inp, { label: "Visit Frequency", value: ea.visitFreq || "", onChange: (v) => setEa((p) => ({ ...p, visitFreq: v })), ph: "e.g., Every 3 months, Annually" }),
        /* @__PURE__ */ React.createElement(Inp, { label: "Last Visit Date", value: ea.lastVisit || "", onChange: (v) => setEa((p) => ({ ...p, lastVisit: v })), ph: "YYYY-MM-DD" }),
        /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 14px" } }, /* @__PURE__ */ React.createElement(Btn, { full: true, onClick: () => {
    ...
    if (modal === "aptView" && viewApt) {
      ...
      } }, "\u{1F6A8}", " Save Alert Note")), aptAlerts.filter((al) => al.aptId === a.id).length > 0 && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8, borderTop: "0.5px solid " + C.pk + "20", paddingTop: 6 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: C.pk, marginBottom: 4 } }, "PREVIOUS ALERT NOTES"), aptAlerts.filter((al) => al.aptId === a.id).map((al) => /* @__PURE__ */ React.createElement("div", { key: al.id, style: { padding: "4px 0", borderBottom: "0.5px solid " + C.sepL, fontSize: 12 } }, /* @__PURE__ */ React.createElement("div", { style: { color: C.tx2, lineHeight: 1.4 } }, al.text), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3 } }, al.d, " ", al.t, " \u2014 ", gn(al.by)))))), a.assignedTo && a.assignedTo.length > 0 && /* @__PURE__ */ React.createElement(Card, { style: { padding: 12, marginTop: 8, border: "1px solid " + C.blue + "20" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.blue, fontWeight: 700, textTransform: "uppercase", marginBottom: 6 } }, "Assigned Staff"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 6 } }, a.assignedTo.map((sid) => /* @__PURE__ */ React.createElement(Pill, { key: sid, text: gn(sid), cl: C.blue })))), a.specialInstructions && /* @__PURE__ */ React.createElement(Card, { style: { padding: 12, marginTop: 8, border: "1px solid " + C.org + "25", background: C.org + "04" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.org, fontWeight: 700, textTransform: "uppercase", marginBottom: 3 } }, "Special Instructions"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, color: C.tx2, lineHeight: 1.5 } }, a.specialInstructions)), a.category && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8, display: "flex", alignItems: "center", gap: 6 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: C.tx3, fontWeight: 600, textTransform: "uppercase" } }, "Category:"), /* @__PURE__ */ React.createElement(Pill, { text: a.category, cl: C.pur }), a.recurring && /* @__PURE__ */ React.createElement(Pill, { text: "🔁 " + a.recurring, cl: C.teal })), a.assignedTo && a.assignedTo.length > 0 && !a.taskCompleted && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 10 } }, /* @__PURE__ */ React.createElement(Btn, { full: true, color: C.grn, onClick: () => {
        setAppointments((p) => p.map((x) => x.id === a.id ? { ...x, taskCompleted: true, taskCompletedBy: user.id, taskCompletedAt: today + " " + (/* @__PURE__ */ new Date()).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" }) } : x));
        setViewApt((prev) => ({ ...prev, taskCompleted: true, taskCompletedBy: user.id }));
      } }, "\u2713 Mark Task Complete")), a.taskCompleted && /* @__PURE__ */ React.createElement(Card, { style: { padding: 10, marginTop: 8, background: C.grn + "08", border: "1px solid " + C.grn + "30" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 16 } }, "\u2705"), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 600, color: C.grn } }, "Task Completed"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3 } }, "By ", gn(a.taskCompletedBy), a.taskCompletedAt ? " \u2014 " + a.taskCompletedAt : "")))), /* @__PURE__ */ React.createElement(Card, { style: { padding: 12, marginTop: 8, border: "1px solid " + C.blue + "20" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, fontWeight: 700, color: C.blue } }, "\u{1F4C1} Appointment Documents"), /* @__PURE__ */ React.createElement("label", { style: { display: "flex", alignItems: "center", gap: 4, padding: "5px 10px", borderRadius: 8, background: C.blue, color: "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer" } }, "\u2B06 Upload", /* @__PURE__ */ React.createElement("input", { type: "file", multiple: true, accept: ".pdf,.doc,.docx,.jpg,.jpeg,.png,.txt,.xlsx", style: { display: "none" }, onChange: (e2) => { Array.from(e2.target.files).forEach((file) => { const reader = new FileReader(); reader.onload = (ev) => { setResFiles((p) => [...p, { id: "rf" + Date.now() + Math.random(), resId: a.resId, aptId: a.id, name: file.name, type: file.type, size: file.size, data: ev.target.result, d: today, by: user.id, category: "Appointment" }]); }; reader.readAsDataURL(file); }); e2.target.value = ""; } }))), resFiles.filter((f) => f.aptId === a.id).length === 0 ? /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", fontSize: 12, color: C.tx3, padding: "8px 0" } }, "No documents attached to this appointment") : resFiles.filter((f) => f.aptId === a.id).map((f, i) => /* @__PURE__ */ React.createElement("div", { key: f.id, style: { display: "flex", alignItems: "center", gap: 8, padding: "6px 0", borderBottom: "0.5px solid " + C.sepL } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 16 } }, f.type && f.type.includes("image") ? "\uD83D\uDDBC\uFE0F" : "\uD83D\uDCC4"), /* @__PURE__ */ React.createElement("div", { style: { flex: 1, fontSize: 12, fontWeight: 600, color: C.tx, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, f.name), /* @__PURE__ */ React.createElement("a", { href: f.data, download: f.name, style: { fontSize: 10, padding: "3px 8px", borderRadius: 6, background: C.blue + "10", color: C.blue, fontWeight: 600, textDecoration: "none" } }, "\u2B07 Download")))), (cm || isS) && /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 0" } }, /* @__PURE__ */ React.createElement(Btn, { full: true, v: "outline", color: tc, onClick: () => {
        setEditApt({ ...a });
        setModal("aptEdit");
      } }, "Edit Appointment"))));
    }
    ...
    if (modal === "specialParam" && specialParamForm && specialParamForm.med) {
      const m = specialParamForm.med;
      const nowTime = (/* @__PURE__ */ new Date()).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" });
      const vitals = m.specialParamVitals || [];
      const needsBP = vitals.includes("bp"), needsHR = vitals.includes("hr"), needsBS = vitals.includes("bs");
      const spf = specialParamForm;
      const bpOk = !needsBP || ((spf.bpSys||"").trim() && (spf.bpDia||"").trim());
      const hrOk = !needsHR || (spf.hr||"").trim();
      const bsOk = !needsBS || (spf.bs||"").trim();
      const instrOk = !m.specialParamInstr || (spf.response||"").trim().length >= 3;
      const isValid = bpOk && hrOk && bsOk && instrOk;
      return /* @__PURE__ */ React.createElement(Modal, { title: "⚠️ Special Parameter Required", onClose: () => { setSpecialParamForm(null); setModal(null); } }, /* @__PURE__ */ React.createElement("div", { style: { paddingBottom: 14 } },
        /* @__PURE__ */ React.createElement("div", { style: { margin: "8px 14px", padding: "14px", background: C.red + "08", borderRadius: 12, border: "1.5px solid " + C.red + "30" } },
          /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 10 } },
            ...
            /* @__PURE__ */ React.createElement("div", null,
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 800, color: C.red } }, "Complete Before Administering"),
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: C.tx3 } }, m.name + " " + m.dose))),
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: C.red, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 6 } }, "Required Task"),
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 700, color: C.tx, lineHeight: 1.5, marginBottom: m.specialParamTimes ? 8 : 0 } }, m.specialParamInstr),
          m.specialParamTimes && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6, padding: "6px 10px", background: C.red + "10", borderRadius: 8, marginTop: 4 } },
            /* @__PURE__ */ React.createElement("span", { style: { fontSize: 14 } }, "⏰"),
            /* @__PURE__ */ React.createElement("div", null,
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, fontWeight: 700, color: C.tx3 } }, "REQUIRED AT"),
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 700, color: C.red } }, m.specialParamTimes)))),
        vitals.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { margin: "0 14px 12px" } },
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: C.tx3, textTransform: "uppercase", marginBottom: 8 } }, "Record Vital Signs"),
          /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 8 } },
            needsBP && /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 12px", background: C.red + "06", borderRadius: 10, border: "1.5px solid " + (bpOk ? C.grn : C.red) + "40" } },
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: C.tx3, marginBottom: 6 } }, "❤️ BLOOD PRESSURE (mmHg)"),
              /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "center" } },
                /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, marginBottom: 3 } }, "SYSTOLIC"), /* @__PURE__ */ React.createElement("input", { type: "number", value: spf.bpSys || "", onChange: (e) => setSpecialParamForm((p) => ({ ...p, bpSys: e.target.value })), placeholder: "120", style: { width: "100%", padding: "10px", borderRadius: 8, border: "1.5px solid " + C.sepL, fontSize: 18, fontWeight: 700, fontFamily: C.font, textAlign: "center", boxSizing: "border-box" } })),
                /* @__PURE__ */ React.createElement("div", { style: { fontSize: 22, color: C.tx3 } }, "/"),
                /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, marginBottom: 3 } }, "DIASTOLIC"), /* @__PURE__ */ React.createElement("input", { type: "number", value: spf.bpDia || "", onChange: (e) => setSpecialParamForm((p) => ({ ...p, bpDia: e.target.value })), placeholder: "80", style: { width: "100%", padding: "10px", borderRadius: 8, border: "1.5px solid " + C.sepL, fontSize: 18, fontWeight: 700, fontFamily: C.font, textAlign: "center", boxSizing: "border-box" } })))),
            needsHR && /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 12px", background: C.red + "06", borderRadius: 10, border: "1.5px solid " + (hrOk ? C.grn : C.red) + "40" } },
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: C.tx3, marginBottom: 6 } }, "💓 HEART RATE (bpm)"),
              /* @__PURE__ */ React.createElement("input", { type: "number", value: spf.hr || "", onChange: (e) => setSpecialParamForm((p) => ({ ...p, hr: e.target.value })), placeholder: "72", style: { width: "100%", padding: "10px", borderRadius: 8, border: "1.5px solid " + C.sepL, fontSize: 18, fontWeight: 700, fontFamily: C.font, textAlign: "center", boxSizing: "border-box" } })),
            needsBS && /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 12px", background: C.red + "06", borderRadius: 10, border: "1.5px solid " + (bsOk ? C.grn : C.red) + "40" } },
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: C.tx3, marginBottom: 6 } }, "🩸 BLOOD SUGAR (mg/dL)"),
              /* @__PURE__ */ React.createElement("input", { type: "number", value: spf.bs || "", onChange: (e) => setSpecialParamForm((p) => ({ ...p, bs: e.target.value })), placeholder: "105", style: { width: "100%", padding: "10px", borderRadius: 8, border: "1.5px solid " + C.sepL, fontSize: 18, fontWeight: 700, fontFamily: C.font, textAlign: "center", boxSizing: "border-box" } })))),
        m.specialParamInstr && /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px", marginBottom: 12 } },
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: C.tx3, textTransform: "uppercase", marginBottom: 6 } }, "Confirmation / Notes *"),
          /* @__PURE__ */ React.createElement("textarea", { value: spf.response || "", onChange: (e) => setSpecialParamForm((p) => ({ ...p, response: e.target.value })), placeholder: "Confirm completion and add notes...", rows: 3, style: { width: "100%", padding: "10px 12px", borderRadius: 10, border: "1.5px solid " + (instrOk ? C.grn : C.red + "40"), fontSize: 13, fontFamily: C.font, boxSizing: "border-box", resize: "none" } })),
        !isValid && /* @__PURE__ */ React.createElement("div", { style: { margin: "0 14px 8px", padding: "8px 12px", background: C.red + "08", borderRadius: 8, fontSize: 12, color: C.red, fontWeight: 600 } }, "🛑 All required fields must be completed first"),
        /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px", display: "flex", gap: 10 } },
          /* @__PURE__ */ React.createElement(Btn, { full: true, v: "outline", color: C.tx3, onClick: () => { setSpecialParamForm(null); setModal(null); } }, "Cancel"),
          /* @__PURE__ */ React.createElement(Btn, { full: true, color: isValid ? C.grn : C.tx3, disabled: !isValid, onClick: () => {
            const { med, sTime, type } = specialParamForm;
            const vitalLog = [needsBP ? "BP: " + spf.bpSys + "/" + spf.bpDia : null, needsHR ? "HR: " + spf.hr + " bpm" : null, needsBS ? "BS: " + spf.bs + " mg/dL" : null].filter(Boolean).join(" | ");
            const fullLog = [vitalLog, spf.response].filter(Boolean).join(" — ");
            const resObj = residents.find((x) => x.id === med.resId);
            const resName = resObj ? fullName(resObj.name) : "?";
            setDocs((p) => [...p, { id: "d" + Date.now(), resId: med.resId, type: "Special Parameter", d: today, t: nowTime, user: user.name, by: user.id, text: "🛑 Special Param — " + med.name + " | " + fullLog + " — " + resName }]);
            if (needsBP && spf.bpSys && spf.bpDia) setBpRecs((p) => [...p, { id: "bp" + Date.now(), resId: med.resId, sys: parseInt(spf.bpSys), dia: parseInt(spf.bpDia), d: today, t: nowTime, by: user.id }]);
            if (needsBS && spf.bs) setBsRecs((p) => [...p, { id: "bs" + Date.now(), resId: med.resId, val: parseFloat(spf.bs), d: today, t: nowTime, by: user.id }]);
            if (needsHR && spf.hr) setDocs((p) => [...p, { id: "dhr" + Date.now(), resId: med.resId, type: "Vitals", d: today, t: nowTime, user: user.name, by: user.id, text: "💓 HR: " + spf.hr + " bpm — " + resName }]);
            setSpecialParamForm(null);
            if (type === "checkin") { setModal(null); }
            else if (type === "prn") {
              if (isAM) { setAdminOvr({ type: "prn", med, staffId: "", prnReason: "", followResult: "", prnRecId: null, confirmed: false }); setModal("adminOvr"); }
              else { setPrnForm({ medId: med.id, reason: "" }); setModal("prnGive"); }
            } else {
              if (isAM) { setAdminOvr({ type: "mar", med, sTime: sTime || "", staffId: "", prnReason: "", followResult: "", prnRecId: null, confirmed: false, qty: String(med.tabsPerDose || 1) }); setModal("adminOvr"); }
              else { setMarRecs((p) => [...p, { med: med.name, d: today, t: nowTime, sTime: sTime || "", by: user.id, qty: med.tabsPerDose || 1 }]); setModal(null); }
            }
          } }, isValid ? "✓ Confirmed — Proceed" : "🛑 Complete All Fields First"))));
    }
    if (modal === "prnGive" && prnForm.medId) {
      ...
      return /* @__PURE__ */ React.createElement(Modal, { title: "Administer PRN Medication", onClose: () => {
      ...
      }, wide: true }, /* @__PURE__ */ React.createElement("div", { style: { paddingTop: 6 } }, /* @__PURE__ */ React.createElement("div", { style: { margin: "0 14px", display: "flex", justifyContent: "center", padding: 12, background: C.blue + "04", borderRadius: 12, marginBottom: 8 } }, /* @__PURE__ */ React.createElement(PillVisual, { pill: m.pill, size: 55 })), /* @__PURE__ */ React.createElement("div", { style: { margin: "0 14px 8px", padding: "10px 12px", background: C.card, borderRadius: 10, border: "1px solid " + C.sepL } }, /* @__PURE__ */ React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 } }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3, fontWeight: 600 } }, "MEDICATION"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 16, fontWeight: 800 } }, m.name)), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3, fontWeight: 600 } }, "DOSAGE"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 16, fontWeight: 800 } }, m.dose)), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3, fontWeight: 600 } }, "ROUTE"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 600 } }, m.route)), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3, fontWeight: 600 } }, "DATE & TIME"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 600 } }, nowDate, /* @__PURE__ */ React.createElement("br", null), nowTime))), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 6, padding: "4px 0", borderTop: "0.5px solid " + C.sepL } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3, fontWeight: 600 } }, "INSTRUCTIONS"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: C.tx2 } }, m.instr)), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 4 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3, fontWeight: 600 } }, "STAFF"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 600 } }, user.name, " (", staffInit(user.name), ")"))), /* @__PURE__ */ React.createElement(Inp, { label: "Reason for giving PRN *", value: prnForm.reason, onChange: (v) => setPrnForm((p) => ({ ...p, reason: v })), multi: true, ph: "e.g., Headache, pain level 6/10, patient requesting..." }), /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 14px" } }, /* @__PURE__ */ React.createElement(Btn, { full: true, color: C.blue, onClick: () => {
        ...
        setPrnRecs((p) => [...p, rec]);
        setPrnFollowUps((p) => [...p, { prnId: id, dueAt: Date.now() + 36e5, by: user.id, dismissed: false }]);
    ...
    if (modal === "adminOvr") {
      ...
      return /* @__PURE__ */ React.createElement(Modal, { title: typeLabel, onClose: () => {
        ...
        if (adminOvr.type === "mar") {
          ...
          setPrnFollowUps((p) => [...p, { prnId: id, dueAt: Date.now() + 36e5, by: adminOvr.staffId, dismissed: false }]);
  ...
  const exportData = () => {
    const data = { v: 1, exported: (/* @__PURE__ */ new Date()).toISOString(), staff, residents, meds, docs, tallies, marRecs, marArchive, prnRecs, memos, assigns, staffShiftAssigns, shiftDuties, appointments, reports, acctItems, homeConfig, photos, walkthroughs, irReports, weightRecs, bpRecs, bsRecs, dutyLogs, rptSubmitted, resFiles, meetings, meetingSchedule, mealCalendar, externalLinks, bipDocs, ippDocs };
  ...
  const importData = (json) => {
    try {
      ...
      if (d.bipDocs) setBipDocs(d.bipDocs);
      if (d.ippDocs) setIppDocs(d.ippDocs);
  ...
  const StaffPage = () => /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement(Hdr, { title: isAM ? isA ? "System Admin" : "Manager Panel" : "Staff", right: isAM && (adminTab === "staff" || adminTab === "residents") ? /* @__PURE__ */ React.createElement("button", { onClick: () => setModal(adminTab === "staff" ? "addStaff" : "addRes"), style: { background: "none", border: "none", cursor: "pointer", color: C.blue, fontSize: 13, fontWeight: 600, fontFamily: C.font } }, "+ Add") : null }), isAM && /* @__PURE__ */ React.createElement("div", { style: { padding: "4px 14px 6px" } }, /* @__PURE__ */ React.createElement(Seg, { opts: [{ l: "\u{1F465} Staff", v: "staff" }, { l: "\u{1F3E0} Residents", v: "residents" }, { l: "\uD83D\uDC8A Refills", v: "refills" }, { l: "\u{1F5FA} Homes", v: "homes" }, { l: "\u2705 Acct", v: "acct" }, ...isA ? [{ l: "\u{1F4CB} Duties", v: "duties" }, { l: "\u{1F4BE} Data", v: "data" }, { l: "\u{1F4CA} Sheets", v: "sheets" }] : []], val: adminTab, set: setAdminTab })), (adminTab === "staff" || !isAM) && /* @__PURE__ */ React.createElement(React.Fragment, null, isAM && /* @__PURE__ */ React.createElement(Sec, { title: "Staff Directory (" + staff.filter((s2) => s2.on && (isA || (s2.homes || []).some((h) => (user.homes || []).includes(h)))).length + " active)" }), !isAM && /* @__PURE__ */ React.createElement(Sec, { title: "Staff Directory" }), /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px" } }, /* @__PURE__ */ React.createElement(Card, null, staff.filter((s2) => isA || (s2.homes || []).some((h) => (user.homes || []).includes(h))).map((s2, i, arr) => {
    ...
    const rl2 = isA ? residents.filter((r) => !r.archived) : residents.filter((r) => (user.homes || []).includes(r.home) && r.home !== "TRAIN" && !r.archived);
    const archivedRes = (isA ? residents : residents.filter((r) => (user.homes || []).includes(r.home))).filter((r) => r.archived);
    return /* @__PURE__ */ React.createElement(React.Fragment, null,
      /* @__PURE__ */ React.createElement(Sec, { title: "Residents (" + rl2.length + ")" }),
      /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px" } },
        /* @__PURE__ */ React.createElement(Card, null,
          rl2.length === 0 ? /* @__PURE__ */ React.createElement("div", { style: { padding: 16, textAlign: "center", color: C.tx3 } }, "No residents") :
          rl2.map((r, i) => {
            const h = HOMES.find((x) => x.id === r.home);
            return /* @__PURE__ */ React.createElement("div", { key: r.id, onClick: () => { setEditRes({ ...r }); setModal("editRes"); }, style: { display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderBottom: i < rl2.length - 1 ? "0.5px solid " + C.sepL : "none", cursor: "pointer" } },
              /* @__PURE__ */ React.createElement(Av, { name: r.name, s: 44, cl: h ? h.color : C.blue }),
              /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } },
                /* @__PURE__ */ React.createElement("div", { style: { fontSize: 15, fontWeight: 600 } }, fullName(r.name), " ", /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: C.tx3 } }, "(", resInit(r.name), ")")),
                /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: C.tx3 } }, "UCI: ", r.uci || "\u2014", " | DOB: ", r.dob),
                /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 4, marginTop: 3 } },
                  h && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 9, padding: "1px 6px", borderRadius: 4, background: h.color + "14", color: h.color, fontWeight: 600 } }, h.name),
                  /* @__PURE__ */ React.createElement("span", { style: { fontSize: 9, padding: "1px 6px", borderRadius: 4, background: C.blue + "10", color: C.blue } }, meds.filter((m) => m.resId === r.id && m.status === "active").length, " meds"),
                  /* @__PURE__ */ React.createElement("span", { style: { fontSize: 9, padding: "1px 6px", borderRadius: 4, background: C.red + "10", color: C.red } }, r.allergies.length, " allergies")
                )
              ),
              /* @__PURE__ */ React.createElement(Ic, { n: "chev", s: 14, c: C.tx3 })
            );
      ...
      archivedRes.length > 0 && /* @__PURE__ */ React.createElement(React.Fragment, null,
        /* @__PURE__ */ React.createElement(Sec, { title: "\uD83D\uDCE6 Archived Residents (" + archivedRes.length + ")" }),
        /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px" } },
          /* @__PURE__ */ React.createElement(Card, null,
            archivedRes.map((r, i) => /* @__PURE__ */ React.createElement(Row, {
              key: r.id,
              left: /* @__PURE__ */ React.createElement("div", { style: { width: 44, height: 44, borderRadius: 22, background: "#e0e0e0", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 } }, "\uD83D\uDCE6"),
              title: fullName(r.name),
              sub: (r.archiveReason || "Archived") + (r.archiveDate ? " \u2014 " + r.archiveDate : ""),
              right: /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#999", background: "#f0f0f0", padding: "2px 6px", borderRadius: 4, whiteSpace: "nowrap" } }, HOMES.find((h) => h.id === r.home) ? HOMES.find((h) => h.id === r.home).name : r.home),
              last: i === archivedRes.length - 1,
              onClick: () => { setEditRes({ ...r }); setModal("editRes"); }
            }))
          )
        )
      )
  ...
  , isA && adminTab === "data" && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement(Sec, { title: "Export Backup" }), /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px" } }, /* @__PURE__ */ React.createElement(Card, { style: { padding: 14 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: C.tx3, marginBottom: 8 } }, staff.length, " data loaded"), /* @__PURE__ */ React.createElement(Btn, { full: true, color: C.grn, onClick: exportData }, "Export Full Backup"))), /* @__PURE__ */ React.createElement(Sec, { title: "Upload Files" }), /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px", marginBottom: 8 } }, /* @__PURE__ */ React.createElement(Card, { style: { padding: 14 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: C.tx3, marginBottom: 8 } }, "Upload files to resident records (PDF, JPG, Excel, Word, etc.)"), /* @__PURE__ */ React.createElement("label", { style: { display: "block", padding: "12px 0", borderRadius: 10, border: "2px dashed " + C.blue + "40", background: C.blue + "04", textAlign: "center", cursor: "pointer" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 600, color: C.blue } }, "\\uD83D\\uDCC1 Tap to Select File"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3, marginTop: 2 } }, "JPG, PDF, Excel, Word accepted"), /* @__PURE__ */ React.createElement("input", { type: "file", accept: ".jpg,.jpeg,.png,.pdf,.xlsx,.xls,.docx,.doc", style: { display: "none" }, onChange: (e) => {
    ...
    const MedRefillRow = ({ m }) => {
      ...
      return /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 12px", borderBottom: "0.5px solid " + C.sepL } },
        /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 10 } },
          /* @__PURE__ */ React.createElement(PillVisual, { pill: m.pill, size: 32 }),
          /* @__PURE__ */ React.createElement("div", { style: { flex: 1, minWidth: 0 } },
            /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" } },
              /* @__PURE__ */ React.createElement("span", { style: { fontSize: 14, fontWeight: 700, color: C.tx } }, m.name, " ", m.dose),
              res && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, padding: "1px 6px", borderRadius: 4, background: C.blue + "10", color: C.blue, fontWeight: 600 } }, fullName(res.name))),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3, marginTop: 1 } }, m.route, " | ", m.freq, " | ", m.doctor)),
          /* @__PURE__ */ React.createElement("div", { style: { textAlign: "right", flexShrink: 0 } },
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 22, fontWeight: 800, color: pillsLeft <= 7 ? C.red : pillsLeft <= 14 ? C.org : C.grn, lineHeight: 1 } }, pillsLeft),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, fontWeight: 600 } }, "PILLS LEFT"))),
        /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 10, marginTop: 6, flexWrap: "wrap" } },
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11 } },
            /* @__PURE__ */ React.createElement("span", { style: { color: C.tx3, fontSize: 10, fontWeight: 600 } }, "STARTED "),
            /* @__PURE__ */ React.createElement("span", { style: { fontWeight: 700, color: C.tx } }, m.startDate)),
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11 } },
            /* @__PURE__ */ React.createElement("span", { style: { color: C.tx3, fontSize: 10, fontWeight: 600 } }, "NEXT FILL "),
            /* @__PURE__ */ React.createElement("span", { style: { fontWeight: 700, color: C.pur } }, nextFill.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }))),
          expiry && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11 } },
            /* @__PURE__ */ React.createElement("span", { style: { color: C.tx3, fontSize: 10, fontWeight: 600 } }, "EXPIRES "),
            /* @__PURE__ */ React.createElement("span", { style: { fontWeight: 700, color: isExpired ? C.red : daysToExpiry <= 30 ? C.org : C.tx } }, isExpired ? "EXPIRED" : expiry.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }), !isExpired && daysToExpiry <= 90 ? " (" + daysToExpiry + "d)" : ""))));
    };
    return /* @__PURE__ */ React.createElement(React.Fragment, null,
      /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 14px" } },
        /* @__PURE__ */ React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 4 } },
          /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 12px", background: C.red + "08", borderRadius: 10, border: "1.5px solid " + C.red + "25", textAlign: "center" } },
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 28, fontWeight: 800, color: C.red } }, critical.length),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: C.red } }, "CRITICAL (≤7 pills)")),
          /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 12px", background: C.org + "08", borderRadius: 10, border: "1.5px solid " + C.org + "25", textAlign: "center" } },
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 28, fontWeight: 800, color: C.org } }, low.length),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: C.org } }, "LOW (8–14 pills)")),
          /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 12px", background: C.red + "05", borderRadius: 10, border: "1.5px solid " + C.red + "20", textAlign: "center" } },
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 28, fontWeight: 800, color: C.red } }, expired.length),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: C.red } }, "EXPIRED")),
          /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 12px", background: C.org + "05", borderRadius: 10, border: "1.5px solid " + C.org + "20", textAlign: "center" } },
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 28, fontWeight: 800, color: C.org } }, expiringSoon.length),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: C.org } }, "EXP ≤30 DAYS")))),
      (critical.length > 0 || expired.length > 0) && /* @__PURE__ */ React.createElement(React.Fragment, null,
        /* @__PURE__ */ React.createElement(Sec, { title: "🚨 Needs Immediate Attention" }),
        /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px" } },
          /* @__PURE__ */ React.createElement(Card, { style: { overflow: "hidden" } },
            [...expired, ...critical].map((m, i, arr) => /* @__PURE__ */ React.createElement(MedRefillRow, { key: m.id, m }))))),
      low.length > 0 && /* @__PURE__ */ React.createElement(React.Fragment, null,
        /* @__PURE__ */ React.createElement(Sec, { title: "⚠️ Running Low" }),
        /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px" } },
          /* @__PURE__ */ React.createElement(Card, { style: { overflow: "hidden" } },
            low.map((m) => /* @__PURE__ */ React.createElement(MedRefillRow, { key: m.id, m }))))),
      expiringSoon.length > 0 && /* @__PURE__ */ React.createElement(React.Fragment, null,
        /* @__PURE__ */ React.createElement(Sec, { title: "📅 Expiring Soon" }),
        /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px" } },
          /* @__PURE__ */ React.createElement(Card, { style: { overflow: "hidden" } },
            expiringSoon.map((m) => /* @__PURE__ */ React.createElement(MedRefillRow, { key: m.id, m }))))),
      /* @__PURE__ */ React.createElement(Sec, { title: "All Medications with Pill Counts (" + allMedsWithCounts.length + ")" }),
      /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px" } },
        allMedsWithCounts.length === 0
          ? /* @__PURE__ */ React.createElement(Card, { style: { padding: 20, textAlign: "center" } }, /* @__PURE__ */ React.createElement("div", { style: { color: C.tx3 } }, "No medications with pill counts set"))
          : /* @__PURE__ */ React.createElement(Card, { style: { overflow: "hidden" } },
              sorted.map((m) => /* @__PURE__ */ React.createElement(MedRefillRow, { key: m.id, m })))));
  })(),
  isAM && adminTab === "homes" && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement(Sec, { title: "Home Configuration" }), /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px" } }, HOMES.map((hm) => {
    const cfg = homeConfig[hm.id] || { ips: [], teams: [] };
    return /* @__PURE__ */ React.createElement(Card, { key: hm.id, style: { marginBottom: 10, overflow: "hidden", borderLeft: "4px solid " + hm.color } }, /* @__PURE__ */ React.createElement("div", { style: { padding: "12px 14px", borderBottom: "0.5px solid " + C.sepL } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 16, fontWeight: 700, color: hm.color } }, hm.full), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3 } }, residents.filter((r2) => r2.home === hm.id && !r2.archived).length, " residents | ", staff.filter((s2) => s2.on && (s2.homes || []).includes(hm.id)).length, " staff")), /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 14px" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tx3, marginBottom: 4 } }, "TEAMS CHANNELS (", (cfg.teams || []).length, ")"), (cfg.teams || []).map((tc2, ti) => /* @__PURE__ */ React.createElement("div", { key: ti, style: { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "4px 0", borderBottom: "0.5px solid " + C.sepL } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12 } }, tc2.name, " ", /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: tc2.on ? C.grn : C.red } }, tc2.on ? "Active" : "Off")), /* @__PURE__ */ React.createElement("button", { onClick: () => setHomeConfig((p) => ({ ...p, [hm.id]: { ...p[hm.id], teams: p[hm.id].teams.map((t2, j) => j === ti ? { ...t2, on: !t2.on } : t2) } })), style: { fontSize: 10, color: tc2.on ? C.red : C.grn, background: "none", border: "none", cursor: "pointer", fontFamily: C.font } }, tc2.on ? "Disable" : "Enable"))), /* @__PURE__ */ React.createElement(Btn, { sm: true, v: "outline", color: hm.color, onClick: () => {
  ...
  const tabs = isVA ? [{ id: "home", l: "Home", i: "home" }, { id: "assign", l: "Assign", i: "clip" }, { id: "memo", l: "Memos", i: "memo", badge: unacked, flash: unacked > 0 }, { id: "duties", l: "Duties", i: "check" }] : [{ id: "home", l: "Home", i: "home" }, { id: "res", l: "Residents", i: "users" }, { id: "memo", l: "Memos", i: "memo", badge: unacked, flash: unacked > 0 }, ...(isAM || isS) ? [{ id: "assign", l: "Assign", i: "clip" }] : [], ...((perms.duties || isAM) && !isCCSStaff) ? [{ id: "duties", l: "Duties", i: "check", flash: isAM ? false : !dutiesDone }] : [], ...perms.staff || perms.admin ? [{ id: "staff", l: isAM ? isA ? "Admin" : "Manage" : "Staff", i: isAM ? "shield" : "person" }] : []];
  return /* @__PURE__ */ React.createElement("div", { style: { fontFamily: C.font, background: "linear-gradient(180deg, #eef2f9 0%, " + C.bg + " 120px)", minHeight: "100vh", paddingBottom: 78, maxWidth: 500, margin: "0 auto" } }, /* @__PURE__ */ React.createElement("style", null, "@keyframes su{from{transform:translateY(100%)}to{transform:translateY(0)}}@keyframes memoFlash{from{opacity:1}to{opacity:0.3}}@keyframes flash{0%,100%{opacity:1}50%{opacity:.3}}@keyframes pulse{0%,100%{transform:scale(1)}50%{transform:scale(1.05)}}@keyframes shimmer{0%{background-position:-200% 0}100%{background-position:200% 0}}input:focus,select:focus,textarea:focus{outline:none;border-color:" + C.blue + "!important;box-shadow:0 0 0 3px rgba(0,122,255,.12)!important}*{box-sizing:border-box;-webkit-tap-highlight-color:transparent;-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale}html{scroll-behavior:smooth;-webkit-overflow-scrolling:touch}body{overscroll-behavior:none}::-webkit-scrollbar{width:0;display:none}button{-webkit-appearance:none;touch-action:manipulation}select{-webkit-appearance:none;appearance:none}input[type=time]{-webkit-appearance:none}"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 5, padding: "5px 14px", background: "linear-gradient(90deg, rgba(52,199,89,.06), rgba(0,122,255,.04))", borderBottom: "0.5px solid rgba(52,199,89,.12)" } }, /* @__PURE__ */ React.createElement(Ic, { n: "shield", s: 11, c: C.grn }), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, fontWeight: 600, color: C.grn, letterSpacing: 0.3 } }, "HIPAA COMPLIANT"), /* @__PURE__ */ React.createElement(Ic, { n: "lock", s: 9, c: C.grn })), user && !isAM && !isS && !isVA && memos.filter((m2) => !m2.acks.includes(user.id) && (!m2.homeIds || m2.homeIds.length === 0 || m2.homeIds.some((hid) => (user.homes || []).includes(hid)))).length > 0 && (() => {
    ...
    const curMemo = urgentUnacked[0];
    const memoIdx = memos.filter((m2) => !m2.homeIds || m2.homeIds.length === 0 || m2.homeIds.some((hid) => (user.homes || []).includes(hid))).length - urgentUnacked.length;
    const totalForUser = memoIdx + urgentUnacked.length;
    const isRead = readMemos.includes(curMemo.id);
    const ackTimestamp = new Date().toLocaleString("en-US", { month: "numeric", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit" });
    return /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,.92)", zIndex: 9999, display: "flex", flexDirection: "column" } },
      /* @__PURE__ */ React.createElement("div", { style: { background: C.red, padding: "10px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 } },
        /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8 } },
          /* @__PURE__ */ React.createElement("span", { style: { fontSize: 18 } }, "\u{1F514}"),
          /* @__PURE__ */ React.createElement("div", null,
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: 800, color: "#fff" } }, curMemo.name || "Memo Requires Your Review"),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "rgba(255,255,255,.75)" } }, curMemo.memoNum || "", " \u2022 Posted ", curMemo.d, " ", curMemo.t || "", " \u2022 By ", gn(curMemo.by))
          )
        ),
        /* @__PURE__ */ React.createElement("div", { style: { background: "rgba(255,255,255,.2)", borderRadius: 10, padding: "4px 10px" } },
          /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, fontWeight: 700, color: "#fff" } }, urgentUnacked.length, " remaining")
        )
      ),
      /* @__PURE__ */ React.createElement("div", { style: { background: "rgba(255,255,255,.08)", padding: "8px 16px", flexShrink: 0, display: "flex", alignItems: "center", gap: 8 } },
        /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, color: "rgba(255,255,255,.7)" } }, "\u{1F4CB}"),
        /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, color: "rgba(255,255,255,.85)", fontWeight: 500 } }, "Read the complete memo below before acknowledging. Scroll to the bottom to unlock acknowledgment.")
      ),
      /* @__PURE__ */ React.createElement("div", {
        id: "memo-scroll-" + curMemo.id,
        ref: (el) => {
          if (el && el.scrollHeight <= el.clientHeight + 20) {
            setReadMemos((p) => p.includes(curMemo.id) ? p : [...p, curMemo.id]);
          }
        },
        onScroll: (e) => {
          const el = e.target;
        ...
        /* @__PURE__ */ React.createElement("div", { style: { background: C.card, borderRadius: 16, padding: "20px", maxWidth: 540, margin: "0 auto", boxShadow: "0 4px 24px rgba(0,0,0,.3)" } },
          curMemo.name && /* @__PURE__ */ React.createElement("div", { style: { fontSize: 18, fontWeight: 800, color: C.tx, marginBottom: 6 } }, curMemo.name),
          /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 12 } },
            curMemo.memoNum && /* @__PURE__ */ React.createElement(Pill, { text: curMemo.memoNum, cl: C.pur }),
            curMemo.memoType && /* @__PURE__ */ React.createElement(Pill, { text: curMemo.memoType.charAt(0).toUpperCase() + curMemo.memoType.slice(1), cl: C.blue }),
            curMemo.urgent && /* @__PURE__ */ React.createElement(Pill, { text: "\u26A0 Urgent", cl: C.red })
          ),
          /* @__PURE__ */ React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14, padding: "10px 12px", background: C.alt, borderRadius: 10 } },
            /* @__PURE__ */ React.createElement("div", null,
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, fontWeight: 700, textTransform: "uppercase", marginBottom: 2 } }, "Date Posted"),
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 600 } }, curMemo.d, " ", curMemo.t || "")
            ),
            /* @__PURE__ */ React.createElement("div", null,
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: C.tx3, fontWeight: 700, textTransform: "uppercase", marginBottom: 2 } }, "Posted By"),
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 600 } }, gn(curMemo.by))
            )
          ),
          /* @__PURE__ */ React.createElement("div", { style: { borderTop: "1.5px solid " + C.sepL, paddingTop: 14, marginBottom: 14 } },
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: C.tx3, textTransform: "uppercase", marginBottom: 8 } }, "Memo Contents"),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, color: C.tx, lineHeight: 1.8, whiteSpace: "pre-wrap" } }, curMemo.text)
          ),
          /* @__PURE__ */ React.createElement("div", { style: { borderTop: "1px solid " + C.sepL, paddingTop: 12, marginBottom: 8 } },
            /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 12px", background: isRead ? C.grn + "08" : C.org + "08", borderRadius: 10, border: "1px solid " + (isRead ? C.grn : C.org) + "25", fontSize: 12, color: isRead ? C.grn : C.org, fontWeight: 600, textAlign: "center" } },
              isRead ? "\u2713 You have read this memo in full. You may now acknowledge." : "\u{1F4DC} Continue scrolling to read the full memo before acknowledging."
            )
          )
        ),
        /* @__PURE__ */ React.createElement("div", { style: { height: 20 } })
      ),
      /* @__PURE__ */ React.createElement("div", { style: { padding: "16px 20px", background: C.card, borderTop: "1px solid " + C.sepL, flexShrink: 0, maxWidth: 540, width: "100%", margin: "0 auto", boxSizing: "border-box" } },
        !isRead && /* @__PURE__ */ React.createElement("div", { style: { marginBottom: 10, fontSize: 12, color: C.org, fontWeight: 600, textAlign: "center" } }, "\u{1F4DC} Scroll to the bottom of the memo above to unlock acknowledgment"),
        /* @__PURE__ */ React.createElement("button", {
          disabled: !isRead,
          onClick: () => {
            const now = new Date();
            const ts = now.toLocaleString("en-US", { month: "numeric", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit" });
            const ackRecord = { id: "ack" + Date.now(), memoId: curMemo.id, memoNum: curMemo.memoNum || curMemo.id, memoDate: curMemo.d, memoName: curMemo.name || curMemo.text.slice(0, 60), memoText: curMemo.text, staffId: user.id, staffName: (user.first || "") + " " + (user.last || ""), timestamp: ts };
            setAckLog((p) => [...p, ackRecord]);
            setMemos((p) => p.map((mx) => mx.id === curMemo.id ? { ...mx, acks: [...mx.acks, user.id], ackRecords: [...(mx.ackRecords || []), ackRecord] } : mx));
          },
          style: { width: "100%", padding: "14px", borderRadius: 12, border: "none", background: isRead ? C.grn : C.tx3, color: "#fff", fontSize: 15, fontWeight: 700, cursor: isRead ? "pointer" : "default", fontFamily: C.font, opacity: isRead ? 1 : 0.5 }
        }, "\u2713 I have read and understand this memo in full"),
        /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8, fontSize: 11, color: C.tx3, textAlign: "center" } }, "Your acknowledgment will be recorded with your name, date, and timestamp.")
      )
    );
    ...
    const hRes = residents.filter((r) => r.home === selHome && !r.archived);
    ...
    return /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement(Hdr, { title: "Select Home", right: /* @__PURE__ */ React.createElement("button", { onClick: () => setUser(null), style: { background: "none", border: "none", cursor: "pointer" } }, /* @__PURE__ */ React.createElement(Ic, { n: "out", s: 18, c: C.red })) }), /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px", display: "grid", gap: 8 } }, uHomes.map((h) => {
      const cnt = residents.filter((r) => r.home === h.id && !r.archived).length;
    ...
    const MEMO_TYPE_LABELS = { medication: { label: "Medication", icon: "\u{1F48A}", color: C.grn }, health: { label: "Health", icon: "\u{1FA7A}", color: C.red }, behavioral: { label: "Behavioral", icon: "\u{1F9E0}", color: C.org }, staff: { label: "Staff", icon: "\u{1F465}", color: C.blue }, house: { label: "House", icon: "\u{1F3E0}", color: C.teal }, resident: { label: "Resident", icon: "\u{1F464}", color: C.pur }, appointment: { label: "Appointment", icon: "\u{1F4C5}", color: "#5856d6" }, other: { label: "Other", icon: "\u{1F4CC}", color: C.tx3 } };
    const visibleMemos = memos.filter((m) => !m.homeIds || m.homeIds.length === 0 || m.homeIds.some((hid) => isAM ? true : (user.homes || []).includes(hid)));
    return /* @__PURE__ */ React.createElement("div", null,
      /* @__PURE__ */ React.createElement(Hdr, { title: "Memos", right: cm || isS ? /* @__PURE__ */ React.createElement("button", { onClick: () => setModal("newMemo"), style: { background: "none", border: "none", cursor: "pointer", color: C.blue, fontSize: 14, fontWeight: 600, fontFamily: C.font } }, "+ New") : null }),
      (cm || isS) ? /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px" } },
        HOMES.filter((h) => isA ? true : (user.homes || []).includes(h.id)).map((hm) => {
          const homeMemos = visibleMemos.filter((m) => m.homeIds && m.homeIds.includes(hm.id));
          if (homeMemos.length === 0) return null;
          const homeResidents = residents.filter((r) => r.home === hm.id && !r.archived);
          return /* @__PURE__ */ React.createElement("div", { key: hm.id, style: { marginBottom: 18 } },
            /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 8, padding: "8px 12px", background: hm.color + "10", borderRadius: 12, borderLeft: "4px solid " + hm.color } },
              /* @__PURE__ */ React.createElement("span", { style: { fontSize: 18 } }, "\u{1F3E0}"),
              /* @__PURE__ */ React.createElement("div", null,
                /* @__PURE__ */ React.createElement("div", { style: { fontSize: 15, fontWeight: 800, color: hm.color } }, hm.full),
                /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3 } }, homeMemos.length, " memo", homeMemos.length !== 1 ? "s" : "")
              )
            ),
            (() => {
              const generalMemos = homeMemos.filter((m) => !m.resId);
              const residentMemos = homeResidents.map((r) => ({ r, memos: homeMemos.filter((m) => m.resId === r.id) })).filter((x) => x.memos.length > 0);
              const renderMemoGroup = (groupMemos, groupKey) => {
                const byType = {};
                groupMemos.forEach((m) => { const t = m.memoType || "other"; if (!byType[t]) byType[t] = []; byType[t].push(m); });
                return Object.entries(byType).map(([type, typeMemos]) => {
                  const tInfo = MEMO_TYPE_LABELS[type] || MEMO_TYPE_LABELS.other;
                  return /* @__PURE__ */ React.createElement(Card, { key: groupKey + "-" + type, style: { marginBottom: 8, overflow: "hidden" } },
                    /* @__PURE__ */ React.createElement("div", { style: { padding: "8px 12px", background: tInfo.color + "08", borderBottom: "0.5px solid " + C.sepL, display: "flex", alignItems: "center", justifyContent: "space-between" } },
                      /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6 } },
                        /* @__PURE__ */ React.createElement("span", { style: { fontSize: 14 } }, tInfo.icon),
                        /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13, fontWeight: 700, color: tInfo.color } }, tInfo.label, " (", typeMemos.length, ")")
                      ),
                      (() => {
                        const groupSheetKey = groupKey + "-" + type + "-sheet";
                        return /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6 } },
                          /* @__PURE__ */ React.createElement("input", { value: memoSheetUrls[groupSheetKey] || "", onChange: (e) => setMemoSheetUrls((p) => ({ ...p, [groupSheetKey]: e.target.value })), placeholder: "Ack. sheet URL...", style: { width: 160, padding: "4px 8px", borderRadius: 6, border: "1px solid " + (memoSheetUrls[groupSheetKey] ? "#34a85350" : C.sepL), fontSize: 10, fontFamily: C.font, boxSizing: "border-box" } }),
                          memoSheetUrls[groupSheetKey] ? /* @__PURE__ */ React.createElement("a", { href: memoSheetUrls[groupSheetKey], target: "_blank", rel: "noopener noreferrer", style: { padding: "4px 8px", borderRadius: 6, background: "#34a853", color: "#fff", fontSize: 10, fontWeight: 700, textDecoration: "none", whiteSpace: "nowrap" } }, "\u{1F4CA}") : /* @__PURE__ */ React.createElement("span", { style: { fontSize: 9, color: C.tx3 } }, "\u{1F4CA} Sheet")
                        );
                      })()
                    ),
                    typeMemos.map((m, mi) => {
                      const acks = m.ackRecords || [];
                      const pending = staff.filter((s2) => s2.on && (s2.homes || []).includes(hm.id) && !m.acks.includes(s2.id));
                      const sheetKey = m.id;
                      return /* @__PURE__ */ React.createElement("div", { key: m.id, style: { borderBottom: mi < typeMemos.length - 1 ? "0.5px solid " + C.sepL : "none" } },
                        /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 12px", cursor: "pointer", display: "flex", alignItems: "flex-start", gap: 8 }, onClick: () => setExpandedMemo(expandedMemo === m.id ? null : m.id) },
                          /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } },
                            /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" } },
                              m.memoNum && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 9, fontWeight: 700, color: C.pur, background: C.pur + "10", padding: "2px 6px", borderRadius: 5 } }, m.memoNum),
                              /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13, fontWeight: 700 } }, m.name || m.text.slice(0, 60))
                            ),
                            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3, marginTop: 2 } }, m.d, " ", m.t || "", " \u2022 By ", gn(m.by)),
                            /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 4, marginTop: 4, flexWrap: "wrap" } },
                              /* @__PURE__ */ React.createElement(Pill, { text: "\u2713 " + m.acks.length + " acked", cl: C.grn }),
                              pending.length > 0 && /* @__PURE__ */ React.createElement(Pill, { text: "\u23F3 " + pending.length + " pending", cl: C.red })
                            )
                          ),
                          /* @__PURE__ */ React.createElement(Ic, { n: expandedMemo === m.id ? "back" : "chev", s: 14, c: C.tx3 })
                        ),
                        expandedMemo === m.id && /* @__PURE__ */ React.createElement("div", { style: { padding: "0 12px 12px", background: C.alt, borderTop: "0.5px solid " + C.sepL } },
                          /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 0 6px", fontSize: 12, color: C.tx, whiteSpace: "pre-wrap", lineHeight: 1.6, borderBottom: "0.5px solid " + C.sepL, marginBottom: 10 } }, m.text),
                          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tx3, textTransform: "uppercase", marginBottom: 6 } }, "Acknowledgment Log"),
                          acks.length === 0 ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: C.tx3, padding: "6px 0" } }, "No acknowledgments yet") :
                          acks.map((rec, ri) => /* @__PURE__ */ React.createElement("div", { key: ri, style: { display: "flex", justifyContent: "space-between", padding: "5px 0", borderBottom: ri < acks.length - 1 ? "0.5px solid " + C.sepL : "none" } },
                            /* @__PURE__ */ React.createElement("div", null,
                              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 600 } }, rec.staffName),
                              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3 } }, "ID: ", rec.staffId)
                            ),
                            /* @__PURE__ */ React.createElement("div", { style: { textAlign: "right" } },
                              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: C.grn, fontWeight: 600 } }, "\u2713 Acknowledged"),
                              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3 } }, rec.timestamp)
                            )
                          )),
                          pending.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8 } },
                            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.red, textTransform: "uppercase", marginBottom: 4 } }, "Pending (", pending.length, ")"),
                            /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 4 } }, pending.map((s2) => /* @__PURE__ */ React.createElement(Pill, { key: s2.id, text: (s2.first || "") + " " + (s2.last || ""), cl: C.red })))
                          ),
                          /* @__PURE__ */ React.createElement("div", { style: { marginTop: 12, padding: "10px 12px", background: "#34a85308", borderRadius: 10, border: "1px solid #34a85330" } },
                            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: "#34a853", marginBottom: 6 } }, "\u{1F4CA} Acknowledgment Sheet Link"),
                            /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6 } },
                              /* @__PURE__ */ React.createElement("input", { value: memoSheetUrls[sheetKey] || "", onChange: (e) => setMemoSheetUrls((p) => ({ ...p, [sheetKey]: e.target.value })), placeholder: "Paste Google Sheets URL for this memo's acknowledgment log...", style: { flex: 1, padding: "8px 10px", borderRadius: 8, border: "1px solid " + (memoSheetUrls[sheetKey] ? "#34a85350" : C.sepL), fontSize: 12, fontFamily: C.font, boxSizing: "border-box" } }),
                              memoSheetUrls[sheetKey] && /* @__PURE__ */ React.createElement("a", { href: memoSheetUrls[sheetKey], target: "_blank", rel: "noopener noreferrer", style: { padding: "8px 14px", borderRadius: 8, background: "#34a853", color: "#fff", fontSize: 12, fontWeight: 700, textDecoration: "none", whiteSpace: "nowrap", display: "flex", alignItems: "center", gap: 4 } }, "\u{1F4CA} Open Sheet")
                            ),
                            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: C.tx3, marginTop: 4 } }, "Link to the Google Sheet where acknowledgments for this memo are recorded. Auto-logged fields: Memo #, Date Posted, Memo Name, Staff Full Name, Timestamp, Memo Contents.")
                          )
                        )
                      );
                    })
              ...
              return /* @__PURE__ */ React.createElement(React.Fragment, null,
                residentMemos.map(({ r, memos: rMemos }) =>
                  /* @__PURE__ */ React.createElement("div", { key: r.id, style: { marginBottom: 10 } },
                    /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6, marginBottom: 6 } },
                      /* @__PURE__ */ React.createElement(Av, { name: r.name, s: 28, cl: C.pur }),
                      /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 700 } }, fullName(r.name)),
                      /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: C.tx3 } }, "\u2022 ", rMemos.length, " memo", rMemos.length !== 1 ? "s" : "")
                    ),
                    renderMemoGroup(rMemos, r.id)
                  )
                ),
                generalMemos.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { marginBottom: 10 } },
                  /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, fontWeight: 700, color: C.tx3, marginBottom: 6 } }, "GENERAL MEMOS"),
                  renderMemoGroup(generalMemos, hm.id + "-gen")
                )
              );
            })()
          );
        }),
        visibleMemos.filter((m) => !m.homeIds || m.homeIds.length === 0).length > 0 && /* @__PURE__ */ React.createElement("div", { style: { marginBottom: 18 } },
          /* @__PURE__ */ React.createElement(Sec, { title: "All Homes" }),
          /* @__PURE__ */ React.createElement(Card, null, visibleMemos.filter((m) => !m.homeIds || m.homeIds.length === 0).map((m, i, arr) => {
            const acked = m.acks.includes(user.id);
            return /* @__PURE__ */ React.createElement("div", { key: m.id, style: { padding: "10px 12px", borderBottom: i < arr.length - 1 ? "0.5px solid " + C.sepL : "none" } },
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 600 } }, m.name || m.text.slice(0, 60)),
              /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3, marginTop: 2 } }, m.d, " \u2022 ", m.acks.length, " acknowledged")
            );
          }))
        )
      ) : /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px" } },
        /* @__PURE__ */ React.createElement(Card, null, visibleMemos.length === 0 ? /* @__PURE__ */ React.createElement("div", { style: { padding: 16, textAlign: "center", color: C.tx3, fontSize: 13 } }, "No memos") : visibleMemos.map((m, i) => {
          const acked = m.acks.includes(user.id);
          return /* @__PURE__ */ React.createElement("div", { key: m.id, style: { padding: "12px 14px", borderBottom: i < visibleMemos.length - 1 ? "0.5px solid " + C.sepL : "none", cursor: "pointer", background: !acked ? C.red + "04" : "transparent" }, onClick: () => setModal("memos") },
            /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 10 } },
              !acked && /* @__PURE__ */ React.createElement("div", { style: { width: 10, height: 10, borderRadius: 5, background: C.red, animation: "memoFlash 1s ease-in-out infinite alternate", flexShrink: 0 } }),
              /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } },
                /* @__PURE__ */ React.createElement("div", { style: { fontSize: 14, fontWeight: acked ? 400 : 600, color: acked ? C.tx2 : C.tx } }, m.name || m.text.slice(0, 70)),
                /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3, marginTop: 2 } }, m.d, " | ", gn(m.by))
              ),
              acked ? /* @__PURE__ */ React.createElement(Pill, { text: "Acknowledged", cl: C.grn }) : /* @__PURE__ */ React.createElement(Pill, { text: "Action Required", cl: C.red })
            )
          );
        }))
      )
    );
  })(), tab === "assign" && (isAM || isS || isVA) && (() => {
    const nonCCSHomes = HOMES.filter((h) => h.id !== "CCS" && h.id !== "TRAIN" && (isA || (user.homes || []).includes(h.id)));
    const activeHub = assignHubTab || (nonCCSHomes[0] ? nonCCSHomes[0].id : null);
    const hubHome = HOMES.find((h) => h.id === activeHub);
    const next10 = Array.from({ length: 10 }, (_, i) => { const d = new Date(today + "T12:00:00"); d.setDate(d.getDate() + i); return d.toISOString().split("T")[0]; });
    const SHIFT_COLORS = { AM: C.org, PM: C.blue, NOC: C.pur };
    const SHIFT_LABELS = { AM: "\uD83C\uDF05 AM", PM: "\u2600\uFE0F PM", NOC: "\uD83C\uDF19 NOC" };
    const toggleDA = (date, shift, staffId, resId) => {
      setDailyAssigns((prev) => {
        const day = { ...(prev[date] || {}) };
        const sh = { ...(day[shift] || {}) };
        const cur = sh[resId] || [];
        sh[resId] = cur.includes(staffId) ? cur.filter((x) => x !== staffId) : [...cur, staffId];
        day[shift] = sh;
        return { ...prev, [date]: day };
      });
    };
    const hubResidents = residents.filter((r) => r.home === activeHub && !r.archived);
    const hubStaff = staff.filter((s2) => s2.on && (s2.homes || []).includes(activeHub) && s2.role === "Staff");
    return /* @__PURE__ */ React.createElement("div", null,
      /* @__PURE__ */ React.createElement(Hdr, { title: "\uD83D\uDCC5 Staff Assignments" }),
      /* @__PURE__ */ React.createElement("div", { style: { padding: "6px 14px 4px", overflowX: "auto" } },
        /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, minWidth: "max-content" } },
          nonCCSHomes.map((hm) => {
            const active = activeHub === hm.id;
            return /* @__PURE__ */ React.createElement("button", { key: hm.id, onClick: () => { setAssignHubTab(hm.id); setAssignExpandedRes(null); }, style: { padding: "8px 14px", borderRadius: 20, border: "1.5px solid " + (active ? hm.color : C.sepL), background: active ? hm.color : C.card, color: active ? "#fff" : C.tx3, fontSize: 12, fontWeight: active ? 700 : 500, cursor: "pointer", fontFamily: C.font, whiteSpace: "nowrap", flexShrink: 0 } }, hm.name);
          })
        )
      ),
      hubHome && /* @__PURE__ */ React.createElement("div", { style: { padding: "6px 14px 0" } },
        /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 14px", background: hubHome.color + "10", borderRadius: 12, borderLeft: "4px solid " + hubHome.color, marginBottom: 10, display: "flex", alignItems: "center", justifyContent: "space-between" } },
          /* @__PURE__ */ React.createElement("div", null,
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 15, fontWeight: 800, color: hubHome.color } }, hubHome.full),
            /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3 } }, hubResidents.length + " residents \u2022 " + hubStaff.length + " staff available")
          ),
          hubStaff.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 4, justifyContent: "flex-end", maxWidth: 200 } },
            hubStaff.map((s2) => /* @__PURE__ */ React.createElement("span", { key: s2.id, style: { fontSize: 10, padding: "2px 7px", borderRadius: 6, background: C.blue + "10", color: C.blue, fontWeight: 600 } }, s2.first))
          )
        ),
        hubResidents.length === 0
          ? /* @__PURE__ */ React.createElement(Card, { style: { padding: 20, textAlign: "center" } }, /* @__PURE__ */ React.createElement("span", { style: { color: C.tx3 } }, "No residents at this home"))
          : hubResidents.map((res) => {
              const isExpanded = assignExpandedRes === res.id;
              const todayAssigned = Object.values((dailyAssigns[today] || {})).flatMap((sh) => sh[res.id] || []);
              const uniqueToday = [...new Set(todayAssigned)];
              return /* @__PURE__ */ React.createElement(Card, { key: res.id, style: { marginBottom: 8, overflow: "hidden", border: isExpanded ? "2px solid " + hubHome.color + "40" : undefined } },
                /* @__PURE__ */ React.createElement("div", { onClick: () => setAssignExpandedRes(isExpanded ? null : res.id), style: { padding: "12px 14px", display: "flex", alignItems: "center", gap: 10, cursor: "pointer", background: isExpanded ? hubHome.color + "06" : "transparent" } },
                  /* @__PURE__ */ React.createElement(Av, { name: res.name, s: 40, cl: hubHome.color }),
                  /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } },
                    /* @__PURE__ */ React.createElement("div", { style: { fontSize: 15, fontWeight: 700 } }, fullName(res.name)),
                    /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: C.tx3, marginTop: 2 } },
                      uniqueToday.length === 0
                        ? "No staff assigned today"
                        : "Today: " + uniqueToday.map((sid) => { const s2 = staff.find((x) => x.id === sid); return s2 ? s2.first : "?"; }).join(", ")
                    )
                  ),
                  /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6 } },
                    uniqueToday.length > 0 && /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, padding: "2px 7px", borderRadius: 6, background: C.grn + "12", color: C.grn, fontWeight: 700 } }, uniqueToday.length + " today"),
                    /* @__PURE__ */ React.createElement(Ic, { n: isExpanded ? "back" : "chev", s: 14, c: C.tx3 })
                  )
                ),
                isExpanded && /* @__PURE__ */ React.createElement("div", { style: { borderTop: "0.5px solid " + C.sepL } },
                  hubStaff.length === 0
                    ? /* @__PURE__ */ React.createElement("div", { style: { padding: 14, textAlign: "center", fontSize: 12, color: C.tx3 } }, "No staff assigned to this home")
                    : /* @__PURE__ */ React.createElement("div", { style: { overflowX: "auto" } },
                        /* @__PURE__ */ React.createElement("table", { style: { width: "100%", borderCollapse: "collapse", minWidth: 520 } },
                          /* @__PURE__ */ React.createElement("thead", null,
                            /* @__PURE__ */ React.createElement("tr", { style: { background: C.alt } },
                              /* @__PURE__ */ React.createElement("th", { style: { padding: "6px 10px", fontSize: 10, fontWeight: 700, color: C.tx3, textAlign: "left", whiteSpace: "nowrap", width: 60 } }, "SHIFT"),
                              next10.map((date) => {
                                const isToday2 = date === today;
                                const dl = new Date(date + "T12:00:00");
                                return /* @__PURE__ */ React.createElement("th", { key: date, style: { padding: "4px 6px", fontSize: 9, fontWeight: 700, color: isToday2 ? C.blue : C.tx3, textAlign: "center", whiteSpace: "nowrap", background: isToday2 ? C.blue + "08" : "transparent", minWidth: 80 } },
                                  /* @__PURE__ */ React.createElement("div", null, dl.toLocaleDateString("en-US", { weekday: "short" })),
                                  /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: isToday2 ? C.blue : C.tx2 } }, dl.toLocaleDateString("en-US", { month: "numeric", day: "numeric" }))
                                );
                              })
                            )
                          ),
                          /* @__PURE__ */ React.createElement("tbody", null,
                            SHIFTS.map((shift) => {
                              const sc = SHIFT_COLORS[shift];
                              return /* @__PURE__ */ React.createElement("tr", { key: shift, style: { borderTop: "0.5px solid " + C.sepL } },
                                /* @__PURE__ */ React.createElement("td", { style: { padding: "6px 10px", fontSize: 10, fontWeight: 700, color: sc, whiteSpace: "nowrap", verticalAlign: "top" } }, SHIFT_LABELS[shift]),
                                next10.map((date) => {
                                  const assigned = ((dailyAssigns[date] || {})[shift] || {})[res.id] || [];
                                  const isToday2 = date === today;
                                  return /* @__PURE__ */ React.createElement("td", { key: date, style: { padding: "4px 4px", verticalAlign: "top", background: isToday2 ? C.blue + "04" : "transparent" } },
                                    /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 3 } },
                                      hubStaff.map((s2) => {
                                        const on = assigned.includes(s2.id);
                                        return /* @__PURE__ */ React.createElement("button", { key: s2.id, onClick: () => toggleDA(date, shift, s2.id, res.id), style: { padding: "3px 7px", borderRadius: 6, border: "1px solid " + (on ? sc : C.sepL), background: on ? sc : "transparent", color: on ? "#fff" : C.tx3, fontSize: 10, fontWeight: on ? 700 : 400, cursor: "pointer", fontFamily: C.font, whiteSpace: "nowrap" } }, s2.first);
                                      })
                                    )
                                  );
                                })
  ...
  })())), tab === "duties" && !isAM && !isCCSStaff && /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement(Hdr, { title: "Shift Duties", right: /* @__PURE__ */ React.createElement("span", { style: { fontSize: 12, color: shiftDuties.filter((d) => d.done).length === shiftDuties.length ? C.grn : C.org, fontWeight: 600 } }, shiftDuties.filter((d) => d.done).length, "/", shiftDuties.length) }), /* @__PURE__ */ React.createElement("div", { style: { padding: "0 14px" } }, /* @__PURE__ */ React.createElement("div", { style: { height: 6, borderRadius: 3, background: C.sepL, marginBottom: 10, overflow: "hidden" } }, /* @__PURE__ */ React.createElement("div", { style: { height: "100%", borderRadius: 3, background: shiftDuties.filter((d) => d.done).length === shiftDuties.length ? C.grn : C.teal, width: shiftDuties.filter((d) => d.done).length / shiftDuties.length * 100 + "%", transition: "width .3s ease" } })), /* @__PURE__ */ React.createElement(Card, null, shiftDuties.map((d, i) => /* @__PURE__ */ React.createElement("div", { key: d.id, onClick: () => {
    if (!d.done) {
      const now = (/* @__PURE__ */ new Date()).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" });
  ...
  }, style: { padding: "6px 14px", borderRadius: 8, border: "2px solid rgba(255,255,255,.5)", background: "rgba(255,255,255,.15)", color: "#fff", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: C.font } }, "Respond")), renderModal(), /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", bottom: 0, left: 0, right: 0 } },
      /* @__PURE__ */ React.createElement("div", { style: { position: "relative", bottom: 0, left: 0, right: 0, background: "linear-gradient(180deg, rgba(214,235,255,.92), rgba(214,235,255,.98))", backdropFilter: "blur(20px)", borderTop: "0.5px solid " + C.sep, display: "flex", justifyContent: "space-around", padding: "4px 0 env(safe-area-inset-bottom,16px)", zIndex: 100 } }, tabs.map((t) => /* @__PURE__ */ React.createElement("button", { key: t.id, onClick: () => {
    setTab(t.id);
    if (t.id !== "res") {
      setSelRes(null);
      setSelHome(null);
