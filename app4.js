...
let supa = null;
try {
  if (window.supabase && SUPA_URL !== "YOUR_SUPABASE_URL") {
...
// ─── SUPABASE: LOAD ALL DATA ON MOUNT ─────────────────────────────
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (error) { console.warn("Supabase load error:", error); setDbReady(true); return; }
        if (data && data.length > 0) {
          const kv = {};
          data.forEach(row => { try { kv[row.key] = JSON.parse(row.value); } catch(e) { kv[row.key] = row.value; } });
          if (kv.staff)            setStaff(kv.staff);
          if (kv.residents)        setResidents(kv.residents);
          if (kv.meds)             setMeds(kv.meds);
          if (kv.marRecs)          setMarRecs(kv.marRecs);
          if (kv.marArchive)       setMarArchive(kv.marArchive);
          if (kv.tallies)          setTallies(kv.tallies);
          if (kv.docs)             setDocs(kv.docs);
          if (kv.memos)            setMemos(kv.memos);
          if (kv.appointments)     setAppointments(kv.appointments);
          if (kv.prnRecs)          setPrnRecs(kv.prnRecs);
          if (kv.prnFollowUps)     setPrnFollowUps(kv.prnFollowUps);
          if (kv.shiftDuties)      setShiftDuties(kv.shiftDuties);
          if (kv.dutyLogs)         setDutyLogs(kv.dutyLogs);
          if (kv.walkthroughs)     setWalkthroughs(kv.walkthroughs);
          if (kv.irReports)        setIrReports(kv.irReports);
          if (kv.weightRecs)       setWeightRecs(kv.weightRecs);
          if (kv.bpRecs)           setBpRecs(kv.bpRecs);
          if (kv.bsRecs)           setBsRecs(kv.bsRecs);
          if (kv.ippDocs)          setIppDocs(kv.ippDocs);
          if (kv.bipDocs)          setBipDocs(kv.bipDocs);
          if (kv.behDomains)       setBehDomains(kv.behDomains);
          if (kv.rptSubmitted)     setRptSubmitted(kv.rptSubmitted);
          if (kv.resFiles)         setResFiles(kv.resFiles);
          if (kv.meetings)         setMeetings(kv.meetings);
          if (kv.meetingSchedule)  setMeetingSchedule(kv.meetingSchedule);
          if (kv.mealCalendar)     setMealCalendar(kv.mealCalendar);
          if (kv.externalLinks)    setExternalLinks(kv.externalLinks);
          if (kv.acctItems)        setAcctItems(kv.acctItems);
          if (kv.aptAlerts)        setAptAlerts(kv.aptAlerts);
          if (kv.homeConfig)       setHomeConfig(kv.homeConfig);
          if (kv.assigns)          setAssigns(kv.assigns);
          if (kv.staffShiftAssigns) setStaffShiftAssigns(kv.staffShiftAssigns);
          if (kv.progSections)     setProgSections(kv.progSections);
          if (kv.reports)          setReports(kv.reports);
          if (kv.knownLocs)        setKnownLocs(kv.knownLocs);
          if (kv.knownStaffInv)    setKnownStaffInv(kv.knownStaffInv);
          if (kv.knownResPres)     setKnownResPres(kv.knownResPres);
        }
  ...
  // ─── SUPABASE: AUTO-SAVE WHEN DATA CHANGES ─────────────────────
  const saveTimer = useRef(null);
  useEffect(() => {
    if (!supa || !dbReady) return;
    clearTimeout(saveTimer.current);
    saveTimer.current = setTimeout(async () => {
      setDbSyncing(true);
      const rows = [
        { key: "staff",            value: JSON.stringify(staff) },
        { key: "residents",        value: JSON.stringify(residents) },
        { key: "meds",             value: JSON.stringify(meds) },
        { key: "marRecs",          value: JSON.stringify(marRecs) },
        { key: "marArchive",       value: JSON.stringify(marArchive) },
        { key: "tallies",          value: JSON.stringify(tallies) },
        { key: "docs",             value: JSON.stringify(docs) },
        { key: "memos",            value: JSON.stringify(memos) },
        { key: "appointments",     value: JSON.stringify(appointments) },
        { key: "prnRecs",          value: JSON.stringify(prnRecs) },
        { key: "prnFollowUps",     value: JSON.stringify(prnFollowUps) },
        { key: "shiftDuties",      value: JSON.stringify(shiftDuties) },
        { key: "dutyLogs",         value: JSON.stringify(dutyLogs) },
        { key: "walkthroughs",     value: JSON.stringify(walkthroughs) },
        { key: "irReports",        value: JSON.stringify(irReports) },
        { key: "weightRecs",       value: JSON.stringify(weightRecs) },
        { key: "bpRecs",           value: JSON.stringify(bpRecs) },
        { key: "bsRecs",           value: JSON.stringify(bsRecs) },
        { key: "ippDocs",          value: JSON.stringify(ippDocs) },
        { key: "bipDocs",          value: JSON.stringify(bipDocs) },
        { key: "behDomains",       value: JSON.stringify(behDomains) },
        { key: "rptSubmitted",     value: JSON.stringify(rptSubmitted) },
        { key: "resFiles",         value: JSON.stringify(resFiles) },
        { key: "meetings",         value: JSON.stringify(meetings) },
        { key: "meetingSchedule",  value: JSON.stringify(meetingSchedule) },
        { key: "mealCalendar",     value: JSON.stringify(mealCalendar) },
        { key: "externalLinks",    value: JSON.stringify(externalLinks) },
        { key: "acctItems",        value: JSON.stringify(acctItems) },
        { key: "aptAlerts",        value: JSON.stringify(aptAlerts) },
        { key: "homeConfig",       value: JSON.stringify(homeConfig) },
        { key: "assigns",          value: JSON.stringify(assigns) },
        { key: "staffShiftAssigns",value: JSON.stringify(staffShiftAssigns) },
        { key: "progSections",     value: JSON.stringify(progSections) },
        { key: "reports",          value: JSON.stringify(reports) },
        { key: "knownLocs",        value: JSON.stringify(knownLocs) },
        { key: "knownStaffInv",    value: JSON.stringify(knownStaffInv) },
        { key: "knownResPres",     value: JSON.stringify(knownResPres) },
      ];
      try {
        await supa.from("app_data").upsert(rows, { onConflict: "key" });
      } catch(e) { console.warn("Supabase save error:", e); }
      setDbSyncing(false);
    }, 2000);
  }, [staff,residents,meds,marRecs,marArchive,tallies,docs,memos,appointments,
      prnRecs,prnFollowUps,shiftDuties,dutyLogs,walkthroughs,irReports,
      weightRecs,bpRecs,bsRecs,ippDocs,bipDocs,behDomains,rptSubmitted,
      resFiles,meetings,meetingSchedule,mealCalendar,externalLinks,acctItems,
      aptAlerts,homeConfig,assigns,staffShiftAssigns,progSections,reports,
      knownLocs,knownStaffInv,knownResPres,dbReady]);
  // ─────────────────────────────────────────────────────────────────
...
"severity":"low/medium/high/critical"}` }] }) });
  ...
  const [dbReady, setDbReady] = useState(!supa); // if no supabase, start ready
  ...
  if (!user) if (!dbReady) {
    return /* @__PURE__ */ React.createElement("div", { style: { fontFamily: C.font, minHeight: "100vh", background: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" } },
      /* @__PURE__ */ React.createElement("div", { style: { width: 60, height: 60, borderRadius: "50%", border: "4px solid rgba(255,255,255,0.3)", borderTopColor: "#fff", animation: "spin 0.8s linear infinite", marginBottom: 20 } }),
      /* @__PURE__ */ React.createElement("div", { style: { color: "#fff", fontSize: 18, fontWeight: 700 } }, "Team ISR"),
      /* @__PURE__ */ React.createElement("div", { style: { color: "rgba(255,255,255,0.8)", fontSize: 13, marginTop: 6 } }, "Loading your data\u2026"),
  ...
  const renderModal = () => {
              ...
              /* @__PURE__ */ React.createElement("div", { style: { padding: "10px 14px" } },
                activeBIP
                  ? (d.bipLinked
                    ...
                    : /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: C.tx3, fontStyle: "italic" } }, "🧠 BIP on file — set as Active in BIP tab to link to this domain")
                  )
    ...
    if (modal === "editRes" && editRes) {
      ...
      } }, er.archived ? "✅ Unarchive Resident" : "📦 Archive Resident"), /* @__PURE__ */ React.createElement(Btn, { full: true, v: "outline", color: C.red, onClick: () => {
  ...
  return /* @__PURE__ */ React.createElement("div", { style: { fontFamily: C.font, background: "linear-gradient(180deg, #eef2f9 0%, " + C.bg + " 120px)", minHeight: "100vh", paddingBottom: 78, maxWidth: 500, margin: "0 auto" } }, /* @__PURE__ */ React.createElement("style", null, "@keyframes su{from{transform:translateY(100%)}to{transform:translateY(0)}}@keyframes memoFlash{from{opacity:1}to{opacity:0.3}}@keyframes flash{0%,100%{opacity:1}50%{opacity:.3}}@keyframes pulse{0%,100%{transform:scale(1)}50%{transform:scale(1.05)}}@keyframes shimmer{0%{background-position:-200% 0}100%{background-position:200% 0}}input:focus,select:focus,textarea:focus{outline:none;border-color:" + C.blue + "!important;box-shadow:0 0 0 3px rgba(0,122,255,.12)!important}*{box-sizing:border-box;-webkit-tap-highlight-color:transparent;-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale}html{scroll-behavior:smooth;-webkit-overflow-scrolling:touch}body{overscroll-behavior:none}::-webkit-scrollbar{width:0;display:none}button{-webkit-appearance:none;touch-action:manipulation}select{-webkit-appearance:none;appearance:none}input[type=time]{-webkit-appearance:none}"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 5, padding: "5px 14px", background: "linear-gradient(90deg, rgba(52,199,89,.06), rgba(0,122,255,.04))", borderBottom: "0.5px solid rgba(52,199,89,.12)" } }, /* @__PURE__ */ React.createElement(Ic, { n: "shield", s: 11, c: C.grn }), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, fontWeight: 600, color: C.grn, letterSpacing: 0.3 } }, "HIPAA COMPLIANT"), /* @__PURE__ */ React.createElement(Ic, { n: "lock", s: 9, c: C.grn })), user && !isAM && !isS && !isVA && memos.filter((m2) => !m2.acks.includes(user.id) && (!m2.homeIds || m2.homeIds.length === 0 || m2.homeIds.some((hid) => (user.homes || []).includes(hid)))).length > 0 && (() => {
    ...
    if (due) adminPrnFollow(due.prnId);
      ...
      supa && dbSyncing ? /* @__PURE__ */ React.createElement("div", { style: { background: "#007aff", color: "#fff", fontSize: 10, textAlign: "center", padding: "2px 0", letterSpacing: 0.3 } }, "\u2601\uFE0F Saving\u2026") : supa ? /* @__PURE__ */ React.createElement("div", { style: { background: "#34c75920", color: "#34c759", fontSize: 10, textAlign: "center", padding: "2px 0", letterSpacing: 0.3 } }, "\u2705 Synced") : null,
